...
let partnerRuntimeSyncInFlight=false,partnerRuntimeLastSync=0;
...
function applyServerPartnerStory(snapshot){if(!snapshot)return;const year=experiencedYears(),serverNow=Date.parse(snapshot.serverTime||'')||Date.now(),nextAt=Date.parse(snapshot.nextAvailableAt||'')||serverNow,s=state.partnerStory||{};state.partnerStory={...s,id:s.id||`server-fated-${state.name}`,name:snapshot.name||s.name||'',gender:snapshot.gender||s.gender||(state.gender==='男'?'女':'男'),route:snapshot.route||s.route||'qi',personality:snapshot.personality||s.personality||'warm',bond:Number(snapshot.bond)||0,accord:Number(snapshot.accord)||0,flags:snapshot.flags||{},memories:Array.isArray(s.memories)?s.memories:[],chapter:Number(snapshot.chapter)||0,nextYear:year+Math.max(0,(nextAt-serverNow)/900000),ending:snapshot.ending||'',completed:!!snapshot.completed,version:partnerStoryVersion}}
function partnerChoice(label,bond=0,accord=0,flag=''){return {label,bond,accord,flag,path:'balance',rewards:[]}}
function applyServerPartnerRuntime(snapshot){applyServerPartner(snapshot);const p=state.partnerSystem;if(!p?.established||!snapshot?.currentState)return;const year=experiencedYears(),serverNow=Date.parse(snapshot.serverTime||'')||Date.now(),started=Date.parse(snapshot.stateStartedAt||'')||serverNow,until=Date.parse(snapshot.stateUntil||'')||serverNow;p.partner.realm=Number(snapshot.partnerRealm)||p.partner.realm||1;p.partner.cultivationProgress=Number(snapshot.cultivationProgress)||0;p.state={type:snapshot.currentState,startYear:year-(serverNow-started)/900000,endYear:year+(until-serverNow)/900000,previousType:p.state?.type||'',consecutiveRestOrCultivate:1,destinationType:snapshot.currentState==='travel'?'遊歷中':''}}
...
function partnerConfirmName(event){const input=document.querySelector('#partnerNameInput'),name=(input?.value||'').trim().slice(0,8);if(!name)return toast('請先替這位修士記下姓名');state.partnerStory.name=name;event.serverName=name;event.step++;renderEncounterModal();save()}
function partnerResolveChoice(event,index){if(event.reaction)return;const data=partnerStoryData[event.chapter-1],step=data.steps[event.step],raw=step?.choices?.[index];if(!raw)return;event.serverChoices=event.serverChoices||[];event.serverChoices.push(index);const [label,bond=0,accord=0,flag='']=raw,story=state.partnerStory;story.chapterBond=(story.chapterBond||0)+bond;story.chapterAccord=(story.chapterAccord||0)+accord;if(flag)story.flags[flag]=true;story.memories.push({chapter:event.chapter,label,year:experiencedYears()});event.reaction={phase:0,label,flag,playerText:partnerPlayerChoiceText(label),partnerText:partnerChoiceReaction(event,raw,index),narration:flag==='leave',ending:false};renderEncounterModal();save()}
async function partnerFinishChapter(event,choice,result){const story=state.partnerStory;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',snapshot=await playerStateRpc('player_partner_story_complete',{p_channel:channel,p_chapter:event.chapter,p_choices:event.serverChoices||[],p_name:event.serverName||story.name||null,p_request_id:crypto.randomUUID()});applyServerPartnerStory(snapshot)}catch(error){toast(error.message);return}story.chapterBond=0;story.chapterAccord=0;state.encounterHistory.unshift({title:event.title,choice,result,year:experiencedYears(),at:gameNow(),tags:['life','partner']});state.encounterHistory=state.encounterHistory.slice(0,60);state.encounterQueue.shift();render();renderEncounterModal();updateEncounterButton();save();toast(`${event.title}・已收入歲月錄`)}
...
async function partnerResolveEnding(event,index){const accepted=partnerEndingEligible()&&index===0;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_partner_story_ending',{p_channel:channel,p_accept:accepted,p_request_id:crypto.randomUUID()});applyServerPartnerStory(result.story);applyServerPartner(result.partner)}catch(error){toast(error.message);return}const s=state.partnerStory;state.encounterHistory.unshift({title:event.title,choice:s.ending,result:accepted?`${partnerDisplayName()}自此成為你的道侶。`:'你們將這段相逢收進各自的歲月。',year:experiencedYears(),at:gameNow(),tags:['life','partner']});state.encounterQueue.shift();render();renderEncounterModal();updateEncounterButton();save();toast(accepted?'同心結已成・洞府道侶頁開啟':'緣起緣落，皆為長生路')}
...
function partnerResolveRuntime(year=experiencedYears()){const p=state.partnerSystem;if(!p?.established)return;partnerRefreshBuff(year);const due=!p.state||year>=p.state.endYear||gameNow()-partnerRuntimeLastSync>=30000;if(!sessionOnline||partnerRuntimeSyncInFlight||!due)return;partnerRuntimeSyncInFlight=true;const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';playerStateRpc('player_partner_get',{p_channel:channel}).then(snapshot=>{applyServerPartnerRuntime(snapshot);partnerRuntimeLastSync=gameNow();if(currentFeature==='cave'&&currentCaveView==='partner')renderCavePanel('partner');save()}).catch(error=>console.warn('partner runtime sync paused',error)).finally(()=>{partnerRuntimeSyncInFlight=false})}
...
function applyServerPartner(snapshot){if(!snapshot)return;const established=!!snapshot.established;if(!established){if(state.partnerSystem?.established)state.partnerSystem=null;return}const year=experiencedYears(),serverNow=Date.parse(snapshot.serverTime||'')||Date.now(),activeUntil=Date.parse(snapshot.activeUntil||'')||0,cooldownUntil=Date.parse(snapshot.cooldownUntil||'')||0,p=state.partnerSystem||{};p.established=true;p.partneredAtYear=Number(snapshot.partneredAtYear)||p.partneredAtYear||year;p.partner={...(p.partner||{}),name:snapshot.partnerName||p.partner?.name||'道侶',gender:snapshot.partnerGender||p.partner?.gender||'',route:snapshot.partnerRoute||p.partner?.route||'qi',personality:snapshot.personality||p.partner?.personality||'warm',realm:p.partner?.realm||1,cultivationProgress:p.partner?.cultivationProgress||0};p.companionCultivation=p.companionCultivation||{};p.companionCultivation.cooldownEndYear=year+Math.max(0,(cooldownUntil-serverNow)/900000);p.companionCultivation.activeBuff=activeUntil>serverNow&&snapshot.activeRoute?{route:snapshot.activeRoute,startYear:year,endYear:year+(activeUntil-serverNow)/900000,bonus:.03}:null;p.milestones=p.milestones||{firstCompanionCultivationDone:false,firstSeclusionDone:false,firstTravelDone:false};state.partnerSystem=p;if(!p.state)partnerBeginState('rest',true,year)}
async function partnerCompanionPractice(route){const p=state.partnerSystem;if(!p?.established||!['rest','cultivate'].includes(p.state.type))return toast('道侶目前不在洞府，無法共同修煉');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_partner_practice',{p_channel:channel,p_route:route,p_request_id:crypto.randomUUID()});applyServerPartner(result);if(!p.milestones.firstCompanionCultivationDone){p.milestones.firstCompanionCultivationDone=true;partnerLog('初次同修',`${state.name}與${p.partner.name}第一次並肩參悟。`)}save();renderCavePanel('partner');toast(`${partnerPracticeName(route,p.partner.route)}・${partnerRoutes[route]}效率提升3%（96年）`)}catch(error){toast(error.message)}}
