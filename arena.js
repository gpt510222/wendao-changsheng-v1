...
function snapshot(){
  ...
  return {schema_version:2,eligible:eligible(),highest_realm:`${best.name}・${best.text}`,core,
...
async function buy(kind){if(busy)return;const price=kind==='stone'?5000:10,key=kind==='stone'?'spiritStone':'spiritJade',label=kind==='stone'?'靈石':'靈玉';if(state[key]<price)return toast(`${label}不足`);busy=true;try{await rpc('arena_buy_attempt',{p_kind:kind,p_channel:arenaChannel});state[key]-=price;save();render();toast('問道臺挑戰次數 +1');await renderView('opponents')}catch(e){toast(e.message)}finally{busy=false}}
...
async function openArenaPurchase(){
  ...
  async function refresh(){const status=await rpc('arena_status',{p_channel:arenaChannel});layer.querySelector('p').textContent=`今日可戰 ${status.free_remaining+status.bought_available} / 10`;layer.querySelectorAll('[data-arena-buy]').forEach(b=>{const k=b.dataset.arenaBuy,n=status[k+'_bought']||0;b.disabled=n>=5;b.textContent=`${k==='stone'?'5000靈石':'10靈玉'}・增加1次（今日 ${n}/5）`})}
  layer.querySelectorAll('[data-arena-buy]').forEach(b=>b.onclick=async()=>{if(busy)return;b.disabled=true;await buy(b.dataset.arenaBuy);if(layer.isConnected)await refresh().catch(e=>toast(e.message))});
