...
async function buy(kind,{preserveView=false}={}){if(busy)return null;const price=kind==='stone'?5000:10,key=kind==='stone'?'spiritStone':'spiritJade',label=kind==='stone'?'靈石':'靈玉';if(state[key]<price){toast(`${label}不足`);return null}busy=true;try{await rpc('arena_buy_attempt',{p_kind:kind,p_channel:arenaChannel});state[key]-=price;save();toast('問道臺挑戰次數 +1');if(preserveView){const status=await rpc('arena_status',{p_channel:arenaChannel});updateArenaPurchaseState(status);return status}render();await renderView('opponents');return null}catch(e){toast(e.message);return null}finally{busy=false}}
