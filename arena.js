...
const arenaPurchaseRequestIds=new Map();
async function buy(kind,{preserveView=false}={}){if(busy)return null;const price=kind==='stone'?5000:10,key=kind==='stone'?'spiritStone':'spiritJade',label=kind==='stone'?'靈石':'靈玉';if(state[key]<price){toast(`${label}不足`);return null}const requestId=arenaPurchaseRequestIds.get(kind)||crypto.randomUUID();arenaPurchaseRequestIds.set(kind,requestId);busy=true;try{const receipt=await rpc('arena_buy_attempt',{p_kind:kind,p_channel:arenaChannel,p_request_id:requestId});arenaPurchaseRequestIds.delete(kind);if(receipt.wallet){serverResourceWalletRevision=Number(receipt.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(receipt.wallet.resources)}if(receipt.jade){state.spiritJade=Math.max(0,Number(receipt.jade.balance)||0);serverJadeRevision=Number(receipt.jade.revision)||serverJadeRevision}save();toast(receipt.alreadyPurchased?'購買結果已由伺服器恢復':'問道臺挑戰次數 +1');if(preserveView){const status=await rpc('arena_status',{p_channel:arenaChannel});updateArenaPurchaseState(status);return status}render();await renderView('opponents');return null}catch(e){toast(e.message);return null}finally{busy=false}}
