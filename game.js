...
function bodyTrainingOptions(){const realm=bodyRealmIndex()+1,injuryReduction=(bodyPassiveUnlocked(3)?10:0)+(state.caveBodyEnabled?state.caveBodyLevel*2:0),staminaFactor=activeBodyInjury()==='internal'?1.25:1,load=Math.floor(state.bodyTrainingLoad||0),materialFactor=state.caveBodyEnabled?Math.max(.75,1-state.caveBodyLevel*.03):1,material=value=>Math.max(0,Math.ceil(value*materialFactor));return {
  basic:{name:'溫養周身',stamina:Math.ceil(8*staminaFactor),food:material(24*realm),wood:material(8*realm),iron:0,roots:{bone:1,blood:1,organs:1},risk:0,load:-8,description:'穩定淬鍊三項根基並卸除少量負荷，適合每日收功。'},
  bath:{name:'負重行功',stamina:Math.ceil(18*staminaFactor),food:material(42*realm),wood:material(14*realm),iron:material(8*realm),roots:{bone:1.35,blood:1.2,organs:1.15},risk:0,load:12,description:'以負重配合呼吸推動全身，收益均衡，是主要鍛體方式。'},
  extreme:{name:'破限重鍛',stamina:Math.ceil(30*staminaFactor),food:material(50*realm),wood:material(10*realm),iron:material(22*realm),roots:{bone:1.9,blood:1.55,organs:1.2},risk:Math.max(5,8+Math.floor(load*.22)-injuryReduction),load:25,description:'主動逼近肉身極限，進度最快；受傷後療癒可留下破立值。'}
}}
...
function trainBody(kind,batch=false){
  refreshBodyState();refreshBodyTrainingCharges();if(!state.bodyPathOpened)return toast('尚未開啟煉體之路');if(state.bodyLevel>=mortalBodyMaxLevel)return toast('肉身成聖四階已圓滿');if(bodyFoundationsReady())return toast('三項根基已足，請先完成突破');let option=bodyTrainingOptions()[kind];if(!option)return;if(kind==='extreme'&&activeBodyInjury()==='tendon')return toast('筋傷未癒，無法進行破限重鍛');let times=batch?bodyTrainingCapacity(option):Math.min(1,bodyTrainingCapacity(option));if(times<1)return toast((state.bodyTrainingCharges||0)<1?'今日淬體時機已用盡，最多可累積14次':'體力或承壓材料不足');let completed=0,newInjury='';
...
function renderHelp(tab='cultivation'){
  ...
  if(tab==='body'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('二十三境・四階制',['煉體共有23個大境界，每境分一至四階；第18境「無漏金身」一階是煉體飛升門檻。','每日00:00恢復2次鍛體時機，最多累積14次；完成筋骨、氣血、臟腑三項根基後即可晉升一階。','正常節奏約半年抵達無漏金身一階；此後需求大幅提高，進入跨年養成。'])+helpCard('三種鍛法與破立',['溫養周身最安全並卸除負荷；負重行功收益均衡；破限重鍛最快但可能受傷。','主動治癒傷勢會留下破立值；下一次普通突破時轉化為命骨與玄軀獎勵。','體力每分鐘恢復1點，負荷每分鐘消退1點；鍛體室降低材料與受傷風險。'])+helpCard('五道肉身大關',['只有進入第5、10、15、18、23境時需要肉身試煉，其餘階段完成三項根基即可突破。','五道大關依序解鎖玉骨崩拳、擎嶽鎮天、萬劫返身、金身破界、此身即道。','九鎖封天第18關通關且無漏金身達一階後，可選煉體天路；成功飛升取得「金身破界」。','肉身成聖四階圓滿時取得終極稱號「此身即道」。'])+realmHelp();return}
