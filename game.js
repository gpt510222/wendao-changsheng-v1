...
function rate() { const authoritative=sessionOnline&&serverCultivationRate!=null?serverCultivationRate:Math.floor(baseRate()*cultivationMultiplier()*partnerRouteMultiplier('qi'));return Math.max(1,Math.floor(authoritative*auraInfusionMultiplier('qi'))); }
...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime()}catch(error){sessionOnline=false;show('#titleScreen');$('#titleHint').textContent='需要連線伺服器才能進入遊戲';toast(error.message);return}
