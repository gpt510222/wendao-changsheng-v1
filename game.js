...
async function useGenderRebirthMirror(key){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_identity_profile',{p_channel:channel,p_action:'gender',p_value:next,p_request_id:crypto.randomUUID()});applyServerIdentity(result.identity||{});applyServerMailItemBalances(result.itemBalances||{})}catch(error){toast(error.message);return false}if(state.partnerStory)state.partnerStory.gender=state.partnerStory.gender==='男'?'女':'男';if(established)state.partnerSystem.partner.gender=state.partnerSystem.partner.gender==='男'?'女':'男';applyCharacterVisual();toast(established?'陰陽同轉・你與道侶皆已重塑此身':'陰陽輪轉・此身已重新顯化');render();save();return true;
}
