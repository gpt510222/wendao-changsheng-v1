...
function applyServerBody(body={}){if(body.nutrition!=null)state.bodyNutrition=Math.max(0,Number(body.nutrition)||0);if(body.foundations)state.bodyFoundations={bone:Math.max(0,Number(body.foundations.bone)||0),blood:Math.max(0,Number(body.foundations.blood)||0),organs:Math.max(0,Number(body.foundations.organs)||0)};if(body.trainingMode!=null)state.bodyTrainingMode=body.trainingMode;if(body.nextCycleAt!=null)state.bodyTrainingNextAt=Math.max(0,Number(body.nextCycleAt)||0);if(body.trainingLoad!=null)state.bodyTrainingLoad=Math.max(0,Number(body.trainingLoad)||0);if(body.injury!=null)state.bodyInjury=body.injury;if(body.injuryUntil!=null)state.bodyInjuryUntil=Math.max(0,Number(body.injuryUntil)||0);if(body.breakthroughValue!=null)state.bodyBreakthroughValue=Math.max(0,Number(body.breakthroughValue)||0);if(body.trialFailures&&typeof body.trialFailures==='object')state.bodyTrialFailures=body.trialFailures;syncBodyTemperFromFoundations()}
