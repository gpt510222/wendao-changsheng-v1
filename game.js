...
window.WENDAO_BUILD='20260915-135';
...
itemCatalog.immortalResidualSource={name:'殘界源質',image:'assets/qstyle-v2/immortal/item-residual-source-v1.png',description:'世界崩毀後尚未散盡的法則殘質。沒有靈氣，卻能短暫彌合仙界裂痕，是復界與仙域建設的基礎素材。目前不可使用、不可出售。',count:'immortalResidualSourceCount',usable:false,giftable:false,sellable:false,immortalMaterial:true};
itemCatalog.immortalBarrenTrace={name:'荒痕',image:'assets/qstyle-v2/immortal/item-barren-trace-v1.png',description:'自枯靈與荒蕪遺址中剝離的古老戰痕，封存著仙界衰敗前的微弱法則印記，是修復高階仙界設施的關鍵素材。目前不可使用、不可出售。',count:'immortalBarrenTraceCount',usable:false,giftable:false,sellable:false,immortalMaterial:true};
...
const bodyTechniqueCatalog=[
  {id:'body-jade-break',name:'玉骨崩拳',kind:'body',unlockLevel:16,min:1.25,max:1.45,armorPierce:.2,effect:'拳勁貫骨・無視 20% 防禦',description:'玉骨鳴震，勁力凝於一拳貫入護體罡氣，從內部崩開防勢。'},
  {id:'body-mountain-crush',name:'擎嶽鎮天',kind:'body',unlockLevel:36,min:1.55,max:1.78,hits:2,guardBonus:.08,effect:'兩段鎮壓・命中後本場減傷至少提升至 8%',description:'舉身如擎山，先以踏地震勢封住退路，再以全身之力鎮落。'},
  {id:'body-myriad-calamity',name:'萬劫返身',kind:'body',unlockLevel:56,min:1.72,max:1.96,hits:2,lifeSteal:.08,effect:'兩段反震・回復傷害 8% 氣血',description:'讓劫力穿身而過，再以筋骨震回，傷越深，拳勢越沉。'},
  {id:'body-golden-boundary',name:'金身破界',kind:'body',unlockLevel:68,min:2,max:2.28,armorPierce:.35,guardBonus:.12,effect:'破界一擊・無視 35% 防禦並取得 12% 減傷',description:'無漏金身踏碎界限，以血肉之軀正面撞開天門。'},
  {id:'body-dao-incarnate',name:'此身即道',kind:'body',unlockLevel:88,min:2.35,max:2.7,hits:3,armorPierce:.25,lifeSteal:.12,effect:'三段道震・無視 25% 防禦並回復 12% 傷害',description:'舉手投足皆為道則，天地與此身一同落拳。'}
];
...
const bodyRealmPassives=[
  ...
  {realm:'血海',name:'血海潮生',description:'命骨成長提高，肉身試煉氣血更為深厚。'},
  {realm:'靈藏',name:'五藏歸元',description:'溫養周身時額外卸除肉身負荷。'},
  {realm:'周天',name:'肉身周天',description:'三項根基運轉相連，肉身招式命中提高。'},
  ...
  {realm:'渡星',name:'渡星之體',description:'肉身招式命中與生存能力進一步提高。'},
  {realm:'寰甲',name:'寰甲天成',description:'血肉如界甲，受創後更快重整。'},
  {realm:'無量',name:'無量真身',description:'肉身威勢突破凡俗尺度。'},
  {realm:'萬劫',name:'萬劫留痕',description:'療傷轉化的破立值收益提高。'},
  {realm:'諸漏盡',name:'諸漏皆盡',description:'肉身負荷自然消退速度提高。'},
  {realm:'無漏金身',name:'金身破界',description:'達一階即可選擇煉體天路飛升。'},
  {realm:'不滅法體',name:'不滅',description:'肉身試煉氣血上限額外提高。'},
  {realm:'滴血重生',name:'重生',description:'肉身招式附帶更強回復。'},
  {realm:'法天象地',name:'法象',description:'肉身招式專精突破原有上限。'},
  {realm:'天地同軀',name:'同軀',description:'命骨與玄軀共同放大肉身攻勢。'},
  {realm:'肉身成聖',name:'此身即道',description:'四階圓滿後取得終極稱號「此身即道」。'}
];
...
function applyJadeEntitlements(keys=[]){for(const key of keys||[]){if(key.startsWith('outfit:')){const value=key.slice(7);if(!state.ownedMasterworkOutfits.includes(value))state.ownedMasterworkOutfits.push(value)}else if(key.startsWith('true_form:')){const value=key.slice(10);if(!state.ownedMasterworkTrueForms.includes(value))state.ownedMasterworkTrueForms.push(value)}}}
...
async function selectOrBuyOutfit(id){
  ...
  if(!masterworkOutfitOwned(outfit,gender)){
    ...
    if(state.gender!==gender)return toast('無法完成購買');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_jade_service_purchase',{p_channel:channel,p_service:'outfit',p_value:masterworkOutfitKey(gender,outfit.id),p_request_id:crypto.randomUUID()});state.spiritJade=Math.max(0,Number(result?.jade?.balance)||0);serverJadeRevision=Number(result?.jade?.revision)||serverJadeRevision;applyJadeEntitlements(result?.entitlements);toast(`已購得「${outfit.name}」`)}catch(error){toast(error.message);return}
  }
...
async function selectOrBuyTrueForm(id){
  ...
  if(!masterworkTrueFormOwned(form)){
    ...
    try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_jade_service_purchase',{p_channel:channel,p_service:'true_form',p_value:form.id,p_request_id:crypto.randomUUID()});state.spiritJade=Math.max(0,Number(result?.jade?.balance)||0);serverJadeRevision=Number(result?.jade?.revision)||serverJadeRevision;applyJadeEntitlements(result?.entitlements);toast(`已購得「${form.name}」`)}catch(error){toast(error.message);return}
  }
...
const titleCatalog=[
  ...
  {id:'body-ascension',name:'金身破界',image:'assets/qstyle-v2/titles/title-body-ascension-v2.png',kind:'單途飛升',hint:'以無漏金身一階選擇煉體天路並成功飛升。'},
  {id:'body-ultimate',name:'此身即道',image:'assets/qstyle-v2/titles/title-body-ultimate-v1.png',kind:'煉體極境',hint:'將肉身成聖修至四階圓滿。'},
...
defaults.bodyTrainingSystemVersion=4;defaults.bodyTrainingCharges=2;defaults.bodyTrainingChargeUpdatedAt=0;defaults.bodyFoundations={bone:0,blood:0,organs:0};defaults.bodyTrialFailures={};defaults.bodyBreakthroughValue=0;defaults.bodyPassiveUpdatedAt=0;
defaults.bodyNutrition=0;defaults.bodyTrainingMode='';defaults.bodyTrainingNextAt=0;defaults.bodyContinuousTrainingVersion=1;
defaults.auraInfusionPath='';defaults.auraInfusionUntil=0;
...
defaults.immortalResidualSourceCount=0;defaults.immortalBarrenTraceCount=0;
defaults.ascension={version:2,route:'none',replayRoute:'none',rankedRoute:'none',rankedAscendedAt:0,prologueCompleted:false,heartTrialCompleted:false,delusionTrialCompleted:false,roadEndCompleted:false,heartRewardAllocated:false,delusionRewardAllocated:false,roadEndRewardAllocated:false,ascended:false,ascendedAt:0,serverAscensionRank:0,titleId:'',heartAnswers:[],delusionReplies:[],delusionAttemptCount:0,roadEndAttemptCount:0,lastMortalTrainingGround:'spirit',currentRealm:'mortal',immortalArrivalStoryVersion:0,immortalRealm:{version:2,region:'ascension-platform',residualSource:0,barrenTraces:0,inventoryVersion:0,timelineStartedAt:0,expeditionDaily:{date:'',used:0},restorationJob:null,storyFlags:{},platformChapterCompleted:false,investigated:[],firstBattleWon:false,guideStoneStage:0,expeditionUnlocked:false,expedition:{active:false,node:0,runs:0},outerRouteCleared:false,wasteland:{arrivalSeen:false,routeProgress:{dryWind:0,sunken:0,kneeling:0},outpostStage:0,outpostJob:null,bodyStage:0,storyFlags:{}}}};
defaults.serverAscensionAllocations={trueQi:0,rootBone:0,physique:0,agility:0};
defaults.serverBaseCore={};
...
let marketFloorNoticeTimer=null,lastScriptureDayKey='',marketPurchaseOffer=null,marketPurchaseQuantity=1,marketPurchaseRequest={key:'',id:''},currentMailId=null,serverScriptureStockDay='',serverReputationStockDay='';
...
let serverEquipmentRevision=0;
let serverSettlementInFlight=false;
let pendingOfflineVault=null;
let serverResourceWalletRevision=0;
let serverCultivationRate=null;
let serverCaveRevision=0;
let serverProgressionRevision=0;
let serverBodyRevision=0;
let serverSectRevision=0;
let serverJadeRevision=0;
let serverWeavingRevision=0;
let serverAscensionRevision=0;
let serverMoralRevision=0;
let serverEncounterRevision=0,encounterSyncInFlight=false;
let serverAscensionPendingStage='';
let ascensionRouteSyncPromise=null;
...
function characterAsset(){
  const outfit=Number(state.outfit)||1,allowed=outfit!==9||(formalImmortalRealmEnabled&&state.gender==='男'&&normalizeAscension().ascended);
...
function syncTitleUnlocks(){
  if(Array.isArray(state.serverUnlockedTitles)){state.unlockedTitles=[...state.serverUnlockedTitles];if(state.equippedTitle!=='none'&&!state.unlockedTitles.includes(state.equippedTitle))state.equippedTitle='none';return}
...
function unlockTitle(id){state.unlockedTitles=Array.isArray(state.unlockedTitles)?state.unlockedTitles:[];if(!titleCatalog.some(title=>title.id===id)||state.unlockedTitles.includes(id))return false;state.unlockedTitles.push(id);return true}
function ascensionTitlePaths(){return [['qi','spiritLevel',80],['sword','swordLevel',80],['body','bodyLevel',bodyAscensionLevel]].filter(([,level,need])=>(state[level]||0)>=need).map(([path])=>path)}
...
function bodyAttributeGain(newLevel) {
  const realm=Math.floor(Math.max(0,newLevel)/bodyStagesPerRealm),stage=1+Math.floor(realm/4),breakthrough=newLevel%bodyStagesPerRealm===0;
  return {rootBone:3+stage+(breakthrough?2*stage:0),physique:3+stage+(breakthrough?2*stage:0),trueQi:newLevel%4===0?Math.ceil(stage/2):0,agility:newLevel%4===3?Math.ceil(stage/2):0};
}
...
function bodyRealmName(level=state.bodyLevel){const safe=Math.max(0,Math.min(maxBodyLevel,Math.floor(level||0))),realm=bodyRealms[Math.floor(safe/bodyStagesPerRealm)]||bodyRealms.at(-1),stage=['一階','二階','三階','四階・圓滿'][safe%bodyStagesPerRealm];return `${realm}${stage}`}
function pathRealmName(path){return path==='spirit'?realmName(state.spiritLevel,spiritRealms):path==='sword'?realmName(state.swordLevel||0,swordRealms):bodyRealmName(state.bodyLevel)}
function pathResourceLine(path){if(path==='spirit')return `修為 ${formatLargeNumber(state.free)} / ${formatLargeNumber(req(state.spiritLevel))}`;if(path==='sword')return `劍元 ${formatLargeNumber(state.swordEssence)} / ${formatLargeNumber(swordReq(state.swordLevel||0))}`;return `腹中精氣 ${formatLargeNumber(state.bodyNutrition||0)} / ${formatLargeNumber(bodyNutritionCapacity())}`}
...
function renderPrimarySanctum(){
  ...
  else{refreshBodyState();resourceLabel.textContent='腹中精氣';$('#totalQi').textContent=`${formatLargeNumber(state.bodyNutrition||0)} / ${formatLargeNumber(bodyNutritionCapacity())}`;rateLabel.textContent='鍛體狀態';$('#rateText').textContent=state.bodyTrainingMode?(bodyTrainingOptions()[state.bodyTrainingMode]?.name||'修煉中'):'尚未開始'}
}
...
async function answerQiHeartTrial(index){const trial=activeHeartTrial,answer=qiHeartQuestions[trial?.index]?.options[index];if(!trial||!answer)return;trial.answers.push({index,nature:answer[1],alignment:answer[2]});trial.index++;if(trial.index<qiHeartQuestions.length)return renderQiDestination();try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_qi_heart_submit',{p_channel:channel,p_milestone:trial.milestone,p_answers:trial.answers.map(item=>item.index),p_request_id:crypto.randomUUID()});applyServerMoral(result);const settled=state.qiHeartTrials[String(trial.milestone)];activeHeartTrial=null;toast(`問心已定・${qiHeartAspectNames[settled.aspect]}・${qiHeartNames[settled.nature]}心`);renderQiDestination();render();save()}catch(error){trial.index--;trial.answers.pop();toast(error.message);renderQiDestination()}}
...
async function chooseQiCycle(mode){if(!qiCycles[mode]||mode===state.qiCycleMode)return;try{await playerQiRuntime('set_cycle',mode);toast(`周天已轉為${qiCycles[mode].name}`);renderQiDestination();render();save()}catch(error){toast(error.message)}}
async function useQiInsight(kind){if(state.qiInsightCharges<1)return toast('今日靈機已用盡');if(kind==='focus'&&state.qiTribulationFocus>=6)return toast('凝神加護已達 +6% 上限');const beforeFree=state.free,beforeAura=state.aura;try{await playerQiRuntime('claim_insight',kind)}catch(error){toast(error.message);return}if(kind==='aura')toast(`洗脈凝神・靈氣 +${formatLargeNumber(Math.max(0,state.aura-beforeAura))}`);else if(kind==='focus')toast(`觀脈悟道・下次雷劫 +${state.qiTribulationFocus}%`);else toast(`納氣歸元・修為 +${formatLargeNumber(state.free-beforeFree)}`);renderQiDestination();render();save()}
...
function bodyRealmIndex(){return Math.min(bodyRealms.length-1,Math.floor(Math.max(0,state.bodyLevel||0)/bodyStagesPerRealm))}
...
function bodyTrainingOptions(){const factor=state.caveBodyEnabled?Math.max(.75,1-state.caveBodyLevel*.03):1,m=bodyRealmCostMultiplier(),cost=value=>Math.ceil(value*m*factor),injuryReduction=(bodyPassiveUnlocked(3)?10:0)+(state.caveBodyEnabled?state.caveBodyLevel*2:0),load=Math.floor(state.bodyTrainingLoad||0);return {
  basic:{name:'調息養身',minutes:5,nutrition:cost(120),wood:0,iron:0,roots:{bone:1,blood:1.15,organs:1.35},risk:0,load:-12,description:'安穩調息，偏重氣血與臟腑；可長時間修煉且不會受傷。'},
  bath:{name:'負重鍛骨',minutes:10,nutrition:cost(360),wood:cost(120),iron:0,roots:{bone:1.8,blood:1.35,organs:1.2},risk:0,load:14,description:'消耗木材架設負重，主要鍛鍊筋骨，收益穩定。'},
  extreme:{name:'極境熬身',minutes:15,nutrition:cost(600),wood:cost(120),iron:cost(90),roots:{bone:2.4,blood:2.1,organs:1.8},risk:Math.max(5,8+Math.floor(load*.22)-injuryReduction),load:25,description:'以隕鐵逼近肉身極限，三項成長最快；高負荷時可能受傷。'}
}}
...
function bodyTrainingCanPay(option){return state.bodyNutrition>=option.nutrition&&state.wood>=option.wood&&state.meteorIron>=option.iron}
async function prepareBodyNutrition(){const before=state.bodyNutrition||0;try{await playerBodyRpc('player_body_command',{p_action:'feed',p_mode:null});toast(`進食調養・腹中精氣 +${formatLargeNumber((state.bodyNutrition||0)-before)}`);renderBodyDestination('training');render();save()}catch(error){toast(error.message)}}
async function stopBodyTraining(message='已停止持續鍛體'){try{await playerBodyRpc('player_body_command',{p_action:'stop',p_mode:null});if(message)toast(message);renderBodyDestination('training');render();save()}catch(error){toast(error.message)}}
async function startBodyTraining(kind){refreshBodyState();const option=bodyTrainingOptions()[kind];if(!option||!state.bodyPathOpened)return;if(bodyFoundationsReady())return toast('三項根基已足，請先完成突破');if(kind==='extreme'&&activeBodyInjury()==='tendon')return toast('筋傷未癒，無法進行極境熬身');if(!bodyTrainingCanPay(option))return toast('腹中精氣或鍛體材料不足');try{await playerBodyRpc('player_body_command',{p_action:'start',p_mode:kind});toast(`${option.name}已開始・離線期間照常進行`);renderBodyDestination('training');save()}catch(error){toast(error.message)}}
async function processBodyTrainingCycles(){if(!serverBodyRevision||bodySettlementInFlight)return 0;bodySettlementInFlight=true;try{const result=await playerBodyRpc('player_body_settle');if(result.cycles){save();render()}return Number(result.cycles)||0}catch(error){console.warn('body settlement paused',error);return 0}finally{bodySettlementInFlight=false}}
const normalizeBodyPathBeforeContinuous=normalizeBodyPath;normalizeBodyPath=function(){if((state.bodyContinuousTrainingVersion||0)<1){state.bodyNutrition=Math.min(600,Math.max(0,state.bodyTrainingCharges||0)*120);state.bodyContinuousTrainingVersion=1}state.bodyNutrition=Math.max(0,Number(state.bodyNutrition)||0);state.bodyTrainingMode=['basic','bath','extreme'].includes(state.bodyTrainingMode)?state.bodyTrainingMode:'';state.bodyTrainingNextAt=state.bodyTrainingMode?Number(state.bodyTrainingNextAt)||gameNow():0;return normalizeBodyPathBeforeContinuous()};
function bodyHealCost(id=activeBodyInjury()){const base={scratch:{food:140,wood:30,stone:0},internal:{food:420,wood:90,stone:0},tendon:{food:700,wood:180,stone:0}}[id];if(!base)return;const factor=bodyPassiveUnlocked(10)?.8:1;return {food:Math.ceil(base.food*factor),wood:Math.ceil(base.wood*factor),stone:0}}
async function healBodyInjury(){const id=activeBodyInjury();if(!id)return toast('目前沒有傷勢');const before=state.bodyBreakthroughValue||0;try{await playerBodyRpc('player_body_heal');toast(`傷勢已痊癒・破立值 +${Math.max(0,(state.bodyBreakthroughValue||0)-before)}`);renderBodyDestination('body');render();save()}catch(error){toast(error.message)}}
...
function applyServerArtifacts(snapshot={}){state.ownedArtifacts=(Array.isArray(snapshot.owned)?snapshot.owned:[]).filter(id=>artifactCatalog.some(a=>a.id===id));state.equippedArtifact=state.ownedArtifacts.includes(snapshot.equipped)?snapshot.equipped:'';return snapshot}
async function serverArtifactCommand(action,id){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_artifact_command',{p_channel:channel,p_action:action,p_artifact:id,p_request_id:crypto.randomUUID()});applyServerArtifacts(result);save();return result}
async function claimArtifact(id){const artifact=artifactCatalog.find(a=>a.id===id);if(!artifact||artifactClaimPending()<1||state.ownedArtifacts.includes(id))return;if(!await gameConfirm(`確定讓「${artifact.name}」認主？\n取得後鎮界器塚入口會暫時隱去，直至下一道封印於第 12／18 關解開。`,{title:'古器認主',confirmText:'確認認主'}))return;try{await serverArtifactCommand('claim',id);render();$('#artifactModal')?.classList.remove('show');toast(`取得並裝備・${artifact.name}`)}catch(error){toast(error.message)}}
async function toggleArtifact(id){if(battle?.active)return toast('戰鬥中無法更換法寶');if(!state.ownedArtifacts.includes(id))return;const next=state.equippedArtifact===id?'':id;try{await serverArtifactCommand('equip',next);render();openArtifactTomb('equip');toast(state.equippedArtifact===id?`已裝備${artifactCatalog.find(a=>a.id===id).name}`:'已卸下法寶')}catch(error){toast(error.message)}}
...
async function resolveEncounter(index){const event=state.encounterQueue[0],choice=event?.choices?.[index];if(event?.kind==='partner')return partnerResolveChoice(event,index);if(!choice)return;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_encounter_choose',{p_channel:channel,p_event_id:event.id,p_choice:index,p_request_id:crypto.randomUUID()});applyServerEncounter(result);applyServerMoral(result);applyServerMailItemBalances(result.itemBalances||{});render();renderEncounterModal();updateEncounterButton();save();toast(`奇遇已了・${itemCatalog[result.reward?.item]?.name||result.reward?.item} ×${result.reward?.amount||0}`)}catch(error){toast(error.message);await playerEncounterSync().catch(()=>{})}}
function realmName(level, arr) {
  if(arr===bodyRealms)return bodyRealmName(level);
...
const replayableSettlementRpcs=new Set(['player_mainline_finish','player_sect_spar_finish','player_sect_master_finish','player_ascension_battle_finish','player_immortal_battle_finish','player_sword_trial_finish','player_body_trial_finish','player_inventory_sell','player_craft_consumable','player_equipment_forge','player_equipment_sell','player_inventory_consume_bundle','player_permanent_consumable_use','player_book_art_learn','player_book_art_upgrade','player_spirit_tribulation','player_sword_breakthrough','player_body_breakthrough']);
...
function progressionMigrationPayload(){return {cultivationAwakened:state.cultivationAwakened,spiritPathOpened:state.spiritPathOpened,swordPathOpened:state.swordPathOpened,bodyPathOpened:state.bodyPathOpened,spiritLevel:state.spiritLevel,swordLevel:state.swordLevel,bodyLevel:state.bodyLevel,swordTrialWins:state.swordTrialWins}}
function bodyMigrationPayload(){return {nutrition:state.bodyNutrition||0,foundations:state.bodyFoundations||{},trainingLoad:state.bodyTrainingLoad||0}}
function resourceEarningProfile(){return {cultivation_rate:state.cultivationAwakened&&state.spiritPathOpened?Math.floor(rate()/auraInfusionMultiplier('qi')):0,aura_rate:state.cultivationAwakened?auraRate():0,aura_capacity:auraCapacity(),pool_bonus:spiritPoolProductionBonus(),sword_rate:state.cultivationAwakened&&state.swordPathOpened?Math.floor(swordEssenceRate()/auraInfusionMultiplier('sword')*(1+(state.caveSwordEnabled&&state.swordEmbryo?(.08+state.caveSwordLevel*.02):0))):0,food_workers:Math.min(state.workerFood,areaWorkerMax(caveAreas.food)),wood_workers:Math.min(state.workerWood,areaWorkerMax(caveAreas.wood)),iron_workers:Math.min(state.workerMeteorIron,areaWorkerMax(caveAreas.meteorIron)),food_capacity:areaCapacity(caveAreas.food),wood_capacity:areaCapacity(caveAreas.wood),iron_capacity:areaCapacity(caveAreas.meteorIron)}}
function applyServerWalletSnapshot(resources={}){state.free=toBigInt(resources.free);state.swordEssence=toBigInt(resources.swordEssence);for(const key of ['aura','spiritStone','food','wood','meteorIron'])if(resources[key]!=null)state[key]=Math.max(0,Number(resources[key])||0)}
function applyServerProgression(progress={}){for(const key of ['cultivationAwakened','spiritPathOpened','swordPathOpened','bodyPathOpened'])if(progress[key]!=null)state[key]=!!progress[key];for(const key of ['spiritLevel','swordLevel','bodyLevel','swordTrialWins'])if(progress[key]!=null)state[key]=Math.max(0,Math.floor(Number(progress[key])||0));if(['spirit','sword','body'].includes(progress.firstPath))state.firstPath=progress.firstPath}
function applyServerWeaving(result={}){const weaving=result.weaving||result;serverWeavingRevision=Number(weaving.revision)||serverWeavingRevision;if(weaving.bagRank!=null)state.bagRank=Math.max(1,Math.min(19,Number(weaving.bagRank)||1));if(weaving.mendingSilk!=null)state.mendingSilk=Math.max(0,Math.floor(Number(weaving.mendingSilk)||0));const job=weaving.job;state.weavingJob=job?{tier:Number(job.tier)||1,amount:Number(job.amount)||0,startAt:Number(job.startAt)||0,endAt:Number(job.endAt)||0}:null;if(result.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet)}return result}
async function playerWeavingCommand(action,tier=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_weaving_command',{p_channel:channel,p_action:action,p_tier:tier,p_expected_wallet_revision:action==='start'?serverResourceWalletRevision:null,p_expected_weaving_revision:serverWeavingRevision,p_request_id:crypto.randomUUID()});return applyServerWeaving(result)}
function applyServerMoral(result={}){const moral=result.moral||result;serverMoralRevision=Number(moral.revision)||serverMoralRevision;if(moral.righteousness!=null)state.righteousness=Math.max(0,Number(moral.righteousness)||0);if(moral.evilQi!=null)state.evilQi=Math.max(0,Number(moral.evilQi)||0);if(moral.qiHeartTraits)state.qiHeartTraits={guard:Number(moral.qiHeartTraits.guard)||0,benevolent:Number(moral.qiHeartTraits.benevolent)||0,free:Number(moral.qiHeartTraits.free)||0};if(moral.qiHeartTrials)state.qiHeartTrials=moral.qiHeartTrials;if(moral.swordTrialChoices)state.swordTrialChoices=moral.swordTrialChoices;if(result.itemBalances)applyServerMailItemBalances(result.itemBalances);return result}
function applyServerIdentity(identity={}){if(identity.origin&&originProfiles[identity.origin])state.origin=identity.origin;if(identity.name)state.name=String(identity.name);if(['男','女'].includes(identity.gender))state.gender=identity.gender;state.serverOriginCore=identity.baseCore&&typeof identity.baseCore==='object'?identity.baseCore:{};return identity}
async function playerIdentityBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerPartner(await playerStateRpc('player_partner_bootstrap',{p_channel:channel}));applyServerPartnerRuntime(await playerStateRpc('player_partner_get',{p_channel:channel}));applyServerPartnerStory(await playerStateRpc('player_partner_story_bootstrap',{p_channel:channel}));await playerStateRpc('player_identity_bootstrap',{p_channel:channel,p_legacy_origin:state.origin||'家族子弟'});return applyServerIdentity(await playerStateRpc('player_identity_profile',{p_channel:channel,p_action:'get',p_value:null,p_request_id:null}))}
function applyServerTitles(profile={}){state.serverUnlockedTitles=Array.isArray(profile.unlocked)?profile.unlocked.filter(id=>titleCatalog.some(title=>title.id===id)):[];state.unlockedTitles=[...state.serverUnlockedTitles];state.equippedTitle=profile.equipped==='none'||state.unlockedTitles.includes(profile.equipped)?profile.equipped:'none';return profile}
async function playerTitleProfile(action='get',title=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerTitles(await playerStateRpc('player_title_profile',{p_channel:channel,p_action:action,p_title:title}))}
function applyServerCoreAttributes(result={}){const core=result.baseCore&&typeof result.baseCore==='object'?result.baseCore:{};const permanent=state.serverPermanentAttributeBonuses&&typeof state.serverPermanentAttributeBonuses==='object'?state.serverPermanentAttributeBonuses:{},ascension=state.serverAscensionAllocations&&typeof state.serverAscensionAllocations==='object'?state.serverAscensionAllocations:{};for(const key of ['trueQi','rootBone','physique','agility','spiritualPower','comprehension','fortune'])state[key]=Math.max(0,Number(core[key])||0)+(['trueQi','rootBone','physique','agility'].includes(key)?Math.max(0,Number(permanent[key])||0)+Math.max(0,Number(ascension[key])||0):0);state.serverBaseCore={...core};return result}
async function playerCoreAttributesGet(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerCoreAttributes(await playerStateRpc('player_core_attributes_get',{p_channel:channel}))}
async function playerMoralBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerMoral(await playerStateRpc('player_moral_bootstrap',{p_channel:channel,p_legacy_righteousness:Math.max(0,Math.floor(state.righteousness||0)),p_legacy_evil_qi:Math.max(0,Math.floor(state.evilQi||0)),p_legacy_traits:state.qiHeartTraits||{},p_legacy_trials:state.qiHeartTrials||{},p_legacy_sword_choices:state.swordTrialChoices||{}}))}
function applyServerEncounter(result={}){const encounter=result.encounter||result;serverEncounterRevision=Number(encounter.revision)||serverEncounterRevision;if(Number.isFinite(Number(encounter.serverYear))){serverYearsAnchor=Math.max(0,Number(encounter.serverYear));serverYearsAnchoredAt=gameNow()}const partners=(state.encounterQueue||[]).filter(event=>event?.kind==='partner'),partnerHistory=(state.encounterHistory||[]).filter(entry=>(entry?.tags||[]).includes('partner'));state.encounterQueue=[...partners,...(Array.isArray(encounter.queue)?encounter.queue:[])].slice(0,12);state.encounterHistory=[...partnerHistory,...(Array.isArray(encounter.history)?encounter.history:[])].slice(0,60);return result}
async function playerEncounterSync(){if(encounterSyncInFlight)return null;encounterSyncInFlight=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerEncounter(await playerStateRpc('player_encounter_sync',{p_channel:channel}))}finally{encounterSyncInFlight=false}}
function applyServerAscension(result={}){const asc=result.ascension||result;serverAscensionRevision=Number(asc.revision)||serverAscensionRevision;serverAscensionPendingStage=String(asc.pendingReward?.stage||'');const prior=state.serverAscensionAllocations&&typeof state.serverAscensionAllocations==='object'?state.serverAscensionAllocations:{},next={};for(const key of ['trueQi','rootBone','physique','agility']){next[key]=Math.max(0,Math.floor(Number(asc.allocations?.[key])||0));state[key]=Math.max(0,(Number(state[key])||0)+next[key]-(Number(prior[key])||0))}state.serverAscensionAllocations=next;const a=normalizeAscension();for(const key of ['prologueCompleted','heartTrialCompleted','heartRewardAllocated','delusionTrialCompleted','delusionRewardAllocated','roadEndCompleted','roadEndRewardAllocated','ascended'])if(asc[key]!=null)a[key]=!!asc[key];if(asc.route!=null)a.route=asc.route;if(Array.isArray(asc.heartAnswers))a.heartAnswers=asc.heartAnswers.map(Number).slice(0,3);a.pendingReward=asc.pendingReward||null;a.ascendedAt=Math.max(0,Number(asc.ascendedAt)||0);a.rankedAscendedAt=a.ascendedAt;a.rankedRoute=a.ascended&&['qi','sword','body'].includes(a.route)?a.route:'none';a.serverAscensionRank=Math.max(0,Number(asc.serverAscensionRank)||0);a.titleId=String(asc.titleId||'');if(!a.ascended)a.currentRealm='mortal';return result}
async function playerAscensionBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerAscension(await playerStateRpc('player_ascension_bootstrap',{p_channel:channel}))}
async function playerAscensionRoute(route){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerAscension(await playerStateRpc('player_ascension_route',{p_channel:channel,p_route:route,p_request_id:crypto.randomUUID()}))}
async function playerAscensionHeart(answers){if(ascensionRouteSyncPromise)await ascensionRouteSyncPromise;const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerAscension(await playerStateRpc('player_ascension_heart',{p_channel:channel,p_answers:answers,p_request_id:crypto.randomUUID()}))}
async function playerAscensionAllocate(stage,allocation){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerAscension(await playerStateRpc('player_ascension_allocate',{p_channel:channel,p_stage:stage,p_allocation:allocation,p_expected_revision:serverAscensionRevision,p_request_id:crypto.randomUUID()}))}
function applyServerQiRuntime(result={}){const qi=result.qi||result;if(qi.cycle&&qiCycles[qi.cycle])state.qiCycleMode=qi.cycle;if(qi.marks)state.qiFoundationMarks={small:Number(qi.marks.small)||0,origin:Number(qi.marks.origin)||0,still:Number(qi.marks.still)||0};if(qi.insightDay!=null)state.qiInsightDay=String(qi.insightDay||'');if(qi.insightCharges!=null)state.qiInsightCharges=Math.max(0,Math.min(3,Number(qi.insightCharges)||0));if(qi.focus!=null)state.qiTribulationFocus=Math.max(0,Math.min(6,Number(qi.focus)||0));if(qi.cultivationRate!=null)serverCultivationRate=Math.max(0,Math.floor(Number(qi.cultivationRate)||0));if(result.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}serverProgressionRevision=Number(qi.revision)||serverProgressionRevision;return result}
async function playerQiRuntime(action='get',value=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',mutating=action!=='get',result=await playerStateRpc('player_qi_runtime',{p_channel:channel,p_action:action,p_value:value,p_expected_wallet_revision:mutating?serverResourceWalletRevision:null,p_expected_progression_revision:mutating?serverProgressionRevision:null,p_request_id:mutating?crypto.randomUUID():null});return applyServerQiRuntime(result)}
async function importLegacyQiRuntime(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_qi_legacy_import',{p_channel:channel,p_cycle:qiCycles[state.qiCycleMode]?state.qiCycleMode:'small',p_marks:state.qiFoundationMarks||{}});return applyServerQiRuntime(result)}
function applyServerSect(result={}){const sect=result.sect||result;serverSectRevision=Number(sect.revision)||serverSectRevision;if(sect.sectName!=null){state.sect=String(sect.sectName||'');state.sectStar=Math.max(0,Math.min(9,Number(sect.sectStar)||0));state.sectFaction=String(sect.sectFaction||'')}if(sect.sectRank!=null)state.sectRank=Math.max(0,Math.min(4,Number(sect.sectRank)||0));if(sect.actingLeader!=null)state.actingLeader=!!sect.actingLeader;if(sect.sectMerit!=null)state.sectMerit=Math.max(0,Number(sect.sectMerit)||0);if(sect.sectContribution!=null)state.sectContribution=Math.max(0,Number(sect.sectContribution)||0);if(sect.prestige!=null)state.prestige=Math.max(0,Number(sect.prestige)||0);if(sect.sectTask!=null)state.sectTask=String(sect.sectTask||'');if(sect.lastSalaryDay!=null)state.lastSalaryDay=String(sect.lastSalaryDay||'');if(sect.lastPracticeDay!=null)state.lastPracticeDay=String(sect.lastPracticeDay||'');if(sect.lastGreetingDay!=null)state.lastGreetingDay=String(sect.lastGreetingDay||'');const until=Date.parse(sect.practiceUntil||'')||0,transmissionUntil=Date.parse(sect.transmissionUntil||'')||0;state.practiceBuff=until>gameNow()?{active:true,until,remaining:0,total:Math.max(1,until-gameNow())}:{active:false,until:0,remaining:0,total:0};state.transmissionBuff=transmissionUntil>gameNow()?{active:true,until:transmissionUntil,remaining:0,total:Math.max(1,transmissionUntil-gameNow())}:{active:false,until:0,remaining:0,total:0};if(result.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}return result}
const applyServerSectBase=applyServerSect;
applyServerSect=function(result={}){const sect=result.sect||result;if(sect.sectExperienceBonus!=null)state.serverSectExperienceBonus=Math.max(0,Math.min(.5,Number(sect.sectExperienceBonus)||0));if(sect.taskRoutes&&typeof sect.taskRoutes==='object'){state.serverSectTaskRoutes={...sect.taskRoutes};state.sectTaskRouteUnlocks={...sect.taskRoutes}}return applyServerSectBase(result)};
async function playerCombatLoadoutBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerWeaving(await playerStateRpc('player_weaving_bootstrap',{p_channel:channel,p_legacy_bag_rank:state.bagRank||1,p_legacy_mending_silk:state.mendingSilk||0}));await playerAscensionBootstrap();return applyServerCombatLoadout(await playerStateRpc('player_combat_loadout_bootstrap',{p_channel:channel,p_legacy_embryo:state.swordEmbryo||'',p_legacy_moves:state.swordMoves||[]}))}
async function playerSwordStateBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerCombatLoadout(await playerStateRpc('player_sword_state_bootstrap',{p_channel:channel,p_legacy_name:state.swordName||'',p_legacy_nurture:state.swordNurtureLevel||0,p_legacy_intent:state.swordIntent||0,p_legacy_insight:state.swordInsight||0,p_legacy_intent_type:state.swordIntentType||''}))}
async function playerSectBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerSect(await playerStateRpc('player_sect_bootstrap',{p_channel:channel,p_rank:state.sectRank||0,p_acting_leader:!!state.actingLeader}));applyServerSect(await playerStateRpc('player_sect_membership_bootstrap',{p_channel:channel,p_sect_name:state.sect||'',p_sect_star:state.sectStar||0,p_sect_faction:state.sectFaction||''}));applyServerSectArts(await playerStateRpc('player_sect_arts_bootstrap',{p_channel:channel,p_legacy_arts:[...(state.learnedArts||[]).filter(art=>art.sourceSect),...(state.learnedSectMoves||[])],p_legacy_capacity:state.artsCapacity||8}));await playerCombatLoadoutBootstrap();await playerSwordStateBootstrap();return applyServerSectSparStatus(await playerStateRpc('player_sect_spar_status',{p_channel:channel}))}
async function playerSectEconomy(action='get'){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',mutating=action!=='get',result=await playerStateRpc('player_sect_economy',{p_channel:channel,p_action:action,p_expected_wallet_revision:mutating?serverResourceWalletRevision:null,p_expected_sect_revision:mutating?serverSectRevision:null,p_request_id:mutating?crypto.randomUUID():null});return applyServerSect(result)}
async function playerSectProgressBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sect_progress_bootstrap',{p_channel:channel,p_merit:state.sectMerit||0,p_contribution:state.sectContribution||0,p_prestige:state.prestige||0,p_task:state.sectTask||''});return applyServerSect(result)}
async function playerSectProgress(action='get',task=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',mutating=action!=='get';if(mutating)await playerSectProgress();const result=await playerStateRpc('player_sect_progress',{p_channel:channel,p_action:action,p_task:task,p_expected_sect_revision:mutating?serverSectRevision:null,p_request_id:mutating?crypto.randomUUID():null});return applyServerSect(result)}
function applyServerBody(body={}){if(body.nutrition!=null)state.bodyNutrition=Math.max(0,Number(body.nutrition)||0);if(body.stamina!=null){state.bodyStamina=Math.max(0,Number(body.stamina)||0);state.bodyStaminaUpdatedAt=gameNow()}if(body.foundations)state.bodyFoundations={bone:Math.max(0,Number(body.foundations.bone)||0),blood:Math.max(0,Number(body.foundations.blood)||0),organs:Math.max(0,Number(body.foundations.organs)||0)};if(body.trainingMode!=null)state.bodyTrainingMode=body.trainingMode;if(body.nextCycleAt!=null)state.bodyTrainingNextAt=Math.max(0,Number(body.nextCycleAt)||0);if(body.trainingLoad!=null)state.bodyTrainingLoad=Math.max(0,Number(body.trainingLoad)||0);if(body.injury!=null)state.bodyInjury=body.injury;if(body.injuryUntil!=null)state.bodyInjuryUntil=Math.max(0,Number(body.injuryUntil)||0);if(body.breakthroughValue!=null)state.bodyBreakthroughValue=Math.max(0,Number(body.breakthroughValue)||0);if(body.trialFailures&&typeof body.trialFailures==='object')state.bodyTrialFailures=body.trialFailures;syncBodyTemperFromFoundations()}
async function playerBodyRpc(name,extra={}){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc(name,{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID(),...extra});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverBodyRevision=Number(result?.body?.revision)||serverBodyRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerBody(result?.body);return result}
async function playerProgressionCommand(action,path){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_progression_command',{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_progression_revision:serverProgressionRevision,p_request_id:crypto.randomUUID(),p_action:action,p_path:path});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverProgressionRevision=Number(result?.progression?.revision)||serverProgressionRevision;serverCaveRevision=Number(result?.cave_revision)||serverCaveRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerProgression(result?.progression);if(path==='spirit')await playerQiRuntime();await playerCoreAttributesGet();return result}
async function connectServerPlayerState(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';const record=await playerStateRpc('player_state_bootstrap',{p_channel:channel,p_legacy_state:recoverySaveData()});serverPlayerStateRevision=Number(record?.revision)||0;if(!serverPlayerStateRevision)throw new Error('伺服器角色狀態未建立');const wallet=await playerStateRpc('player_resource_wallet_bootstrap',{p_channel:channel,p_resources:resourceWalletMigrationPayload()});serverResourceWalletRevision=Number(wallet?.revision)||0;if(!serverResourceWalletRevision)throw new Error('伺服器資源帳本未建立');applyServerWalletSnapshot(wallet?.resources);const progression=await playerStateRpc('player_progression_bootstrap',{p_channel:channel,p_progress:progressionMigrationPayload()});serverProgressionRevision=Number(progression?.revision)||0;if(!serverProgressionRevision)throw new Error('伺服器修行進度未建立');applyServerProgression(progression);const body=await playerStateRpc('player_body_bootstrap',{p_channel:channel,p_body:bodyMigrationPayload()});serverBodyRevision=Number(body?.revision)||0;if(!serverBodyRevision)throw new Error('伺服器煉體狀態未建立');applyServerBody(body);await playerStateRpc('player_resource_profile_bootstrap',{p_channel:channel,p_profile:resourceEarningProfile()});const cave=await playerStateRpc('player_cave_bootstrap',{p_channel:channel,p_cave:caveMigrationPayload()});serverCaveRevision=Number(cave?.revision)||0;if(!serverCaveRevision)throw new Error('伺服器洞府狀態未建立');const securedCave=await playerStateRpc('player_cave_security_bootstrap',{p_channel:channel,p_config:caveSecurityMigrationPayload()});serverCaveRevision=Number(securedCave?.revision)||serverCaveRevision;const infusion=await playerStateRpc('player_aura_infusion_get',{p_channel:channel});serverCaveRevision=Number(infusion?.revision)||serverCaveRevision;if(infusion?.until){state.auraInfusionPath=infusion.path||'';state.auraInfusionUntil=Date.parse(infusion.until||'')||0}applyServerMailItemBalances((await playerStateRpc('player_inventory_get',{p_channel:channel}))?.itemBalances||{});applyServerMainline(await playerStateRpc('player_mainline_bootstrap',{p_channel:channel,p_legacy_cleared:state.mainlineCleared||0}));applyServerArtifacts(await playerStateRpc('player_artifact_bootstrap',{p_channel:channel,p_legacy_owned:state.ownedArtifacts||[],p_legacy_equipped:state.equippedArtifact||''}));applyServerPermanentConsumables(await playerStateRpc('player_book_arts_bootstrap',{p_channel:channel,p_legacy_arts:(state.learnedArts||[]).filter(art=>art.source==='book')}));applyServerEquipment(await playerStateRpc('player_equipment_bootstrap',{p_channel:channel,p_legacy_items:state.equipmentInventory||[],p_legacy_equipped:state.equippedItems||{}}));applyServerDivineRoaming(await playerStateRpc('player_divine_roaming',{p_channel:channel,p_action:'get',p_stage:null,p_total:null,p_legacy_unlocked:!!state.divineRoamingUnlocked,p_request_id:null}));applyServerCraftingDaily(await playerStateRpc('player_crafting_daily_get',{p_channel:channel}));return playerStateRpc('offline_reward_prepare',{p_channel:channel})}
function applyServerCaveState(cave={}){const numeric=['daoChildTotal','daoChildBought','workerFood','workerWood','workerMeteorIron','foodAreaLevel','woodAreaLevel','meteorIronAreaLevel','spiritPoolLevel','caveCoreLevel','caveCultivationLevel','caveSwordLevel','caveBodyLevel','metalRoot','woodRoot','waterRoot','fireRoot','earthRoot'],flags=['caveCultivationEnabled','caveSwordEnabled','caveBodyEnabled'];for(const key of numeric)if(cave[key]!=null)state[key]=Number(cave[key]);for(const key of flags)if(cave[key]!=null)state[key]=!!cave[key]}
async function playerCaveMutation(rpc,action,key=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc(rpc,{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_cave_revision:serverCaveRevision,p_request_id:crypto.randomUUID(),p_action:action,p_key:key});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverCaveRevision=Number(result?.cave?.revision)||serverCaveRevision;applyServerCaveState(result?.cave);return result}
function playerCaveCommand(action,key=null){return playerCaveMutation('player_cave_command',action,key)}
function playerCaveControl(action,key=null){return playerCaveMutation('player_cave_control',action,key)}
async function claimServerSettlementTicks(){if(!sessionOnline||serverSettlementInFlight)return {ticks:0,rewards:{}};serverSettlementInFlight=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',settled=await playerStateRpc('player_state_claim_elapsed',{p_channel:channel,p_expected_revision:serverPlayerStateRevision,p_request_id:crypto.randomUUID()});serverPlayerStateRevision=Number(settled?.revision)||serverPlayerStateRevision;serverResourceWalletRevision=Number(settled?.wallet_revision)||serverResourceWalletRevision;return {ticks:Math.max(0,Math.floor(Number(settled?.elapsed_ticks??settled?.elapsed_seconds/5)||0)),rewards:settled?.rewards||{}}}catch(error){console.warn('server settlement paused',error);return {ticks:0,rewards:{}}}finally{serverSettlementInFlight=false}}
async function registerFormalPlayer(session,retryAuth=true){
  if(!session?.user?.id)return '';
  const boundUid=localStorage.getItem(accountRecoveryConfig.boundUidKey),hasRecoveryCode=!!storedRecoveryCode(),response=await fetch(`${leaderboardConfig.url}/rest/v1/rpc/player_account_ensure`,{method:'POST',headers:leaderboardHeaders(session.access_token),body:JSON.stringify({p_player_name:(state.name||'無名修士').trim().slice(0,20),p_allow_create:!boundUid&&!hasRecoveryCode})});
  if(response.status===401&&retryAuth)return registerFormalPlayer(await ensureLeaderboardSession(true),false);
  if(!response.ok)throw new Error('player registration failed');const account=await response.json();if(!account?.exists||boundUid&&account.public_uid!==boundUid){clearTransferredDeviceData();throw new Error('account transferred')}currentPlayerUid=account.public_uid||'';if(currentPlayerUid)localStorage.setItem(accountRecoveryConfig.boundUidKey,currentPlayerUid);return currentPlayerUid;
}
...
async function verifyAccountOwnership(){const boundUid=localStorage.getItem(accountRecoveryConfig.boundUidKey),hasRecoveryCode=!!storedRecoveryCode();let session=readLeaderboardSession();if(!state.name||!session?.user?.id||!boundUid&&!hasRecoveryCode||accountOwnershipCheckInFlight||document.hidden)return;accountOwnershipCheckInFlight=true;try{let response=await fetch(`${leaderboardConfig.url}/rest/v1/rpc/player_account_ensure`,{method:'POST',headers:leaderboardHeaders(session.access_token),body:JSON.stringify({p_player_name:(state.name||'無名修士').trim().slice(0,20),p_allow_create:false})});if(response.status===401){session=await ensureLeaderboardSession(true);response=await fetch(`${leaderboardConfig.url}/rest/v1/rpc/player_account_ensure`,{method:'POST',headers:leaderboardHeaders(session.access_token),body:JSON.stringify({p_player_name:(state.name||'無名修士').trim().slice(0,20),p_allow_create:false})})}if(!response.ok)return;const account=await response.json();if(!account?.exists||boundUid&&account.public_uid!==boundUid){clearTransferredDeviceData();return}if(!boundUid&&account.public_uid)localStorage.setItem(accountRecoveryConfig.boundUidKey,account.public_uid)}catch{}finally{accountOwnershipCheckInFlight=false}}
...
async function recoverAccountThroughEdge(recoveryHash,channel,retryAuth=true){let session=await ensureLeaderboardSession(),response=await fetch(`${leaderboardConfig.url}/functions/v1/account-recovery`,{method:'POST',headers:leaderboardHeaders(session.access_token),body:JSON.stringify({recoveryHash,channel})});if(response.status===401&&retryAuth){session=await ensureLeaderboardSession(true);return recoverAccountThroughEdge(recoveryHash,channel,false)}const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.message||'帳號恢復服務暫時無法使用');return {session,data}}
...
function lockAscensionLeaderboardRecord(a=normalizeAscension(),remoteRecord=null){if(!a.ascended)return null;const validRoute=route=>['qi','sword','body'].includes(route),titleRoute={'qi-ascension':'qi','sword-ascension':'sword','body-ascension':'body'}[a.titleId],route=validRoute(a.rankedRoute)?a.rankedRoute:validRoute(remoteRecord?.route)?remoteRecord.route:validRoute(titleRoute)?titleRoute:validRoute(a.route)?a.route:'none',ascendedAt=Math.max(1,Number(a.rankedAscendedAt)||Number(remoteRecord?.ascendedAt)||Number(a.ascendedAt)||Date.now());if(!validRoute(route))return null;a.rankedRoute=route;a.rankedAscendedAt=ascendedAt;return {route,ascendedAt}}
function ascensionLeaderboardKey(a=normalizeAscension()){const record=lockAscensionLeaderboardRecord(a);return record?`${record.ascendedAt}:${record.route}`:''}
...
function leaderboardVersionValue(){return leaderboardConfig.gameVersion}
...
function parseImmortalLeaderboardRecord(record){const match=String(record?.game_version||'').match(/\|I:(\d+):(\d+):(\d+)(?::(\d+):(\d+):(\d+))?(?:\||$)/);return match?{...record,restorationStage:Number(match[1]),exploration:Number(match[2]),expeditionRuns:Number(match[3]),outpostStage:Number(match[4]||0),wastelandProgress:Number(match[5]||0),bodyStage:Number(match[6]||0)}:null}
async function loadOwnLeaderboardRecord(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',record=await playerStateRpc('player_ranking_own',{p_channel:channel}),ascensionRecord=parseAscensionLeaderboardRecord(record),immortalRecord=parseImmortalLeaderboardRecord(record),a=normalizeAscension();if(ascensionRecord&&a.ascended&&!['qi','sword','body'].includes(a.rankedRoute)){lockAscensionLeaderboardRecord(a,ascensionRecord);save()}leaderboardKnownPower=record?Number(record.combat_power):0;leaderboardKnownName=record?.player_name||'';leaderboardKnownAscensionKey=ascensionRecord?`${ascensionRecord.ascendedAt}:${ascensionRecord.route}`:'';leaderboardKnownImmortalKey=immortalRecord?`${immortalRecord.restorationStage}:${immortalRecord.exploration}:${immortalRecord.expeditionRuns}:${immortalRecord.outpostStage}:${immortalRecord.wastelandProgress}:${immortalRecord.bodyStage}`:''}
async function syncLeaderboard(){
  if(leaderboardSyncInFlight||!state.name||!state.cultivationAwakened)return;leaderboardSyncInFlight=true;
  try{const session=await ensureLeaderboardSession();if(leaderboardKnownPower===null)await loadOwnLeaderboardRecord(session);const currentPower=Math.max(0,Math.round(combatPower())),playerName=state.name.trim().slice(0,20)||'無名修士',nameChanged=playerName!==leaderboardKnownName,ascensionKey=ascensionLeaderboardKey(),immortalKey=immortalLeaderboardKey(),ascensionChanged=ascensionKey!==leaderboardKnownAscensionKey,immortalChanged=immortalKey!==leaderboardKnownImmortalKey;if(currentPower<=leaderboardKnownPower&&!nameChanged&&!ascensionChanged&&!immortalChanged)return;if(typeof snapshot!=='function')throw new Error('戰鬥快照尚未就緒');const record=await playerStateRpc('player_ranking_sync',{p_channel:leaderboardConfig.sessionKey.includes('release')?'formal':'test',p_name:playerName,p_game_version:leaderboardVersionValue(),p_snapshot:snapshot()}),rankedPower=Math.max(0,Number(record?.combatPower)||0),rankedAscension=parseAscensionLeaderboardRecord({game_version:record?.gameVersion}),rankedImmortal=parseImmortalLeaderboardRecord({game_version:record?.gameVersion});leaderboardKnownPower=rankedPower;leaderboardKnownName=record?.playerName||playerName;leaderboardKnownAscensionKey=rankedAscension?`${rankedAscension.ascendedAt}:${rankedAscension.route}`:'';leaderboardKnownImmortalKey=rankedImmortal?`${rankedImmortal.restorationStage}:${rankedImmortal.exploration}:${rankedImmortal.expeditionRuns}:${rankedImmortal.outpostStage}:${rankedImmortal.wastelandProgress}:${rankedImmortal.bodyStage}`:''}catch{}finally{leaderboardSyncInFlight=false}
}
...
async function fetchCombatLeaderboard(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return dedupeCombatLeaderboard(await playerStateRpc('player_combat_rankings',{p_channel:channel}))}
async function fetchAscensionLeaderboard(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return playerStateRpc('player_ascension_rankings',{p_channel:channel})}
async function fetchImmortalLeaderboard(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return(await playerStateRpc('player_immortal_rankings',{p_channel:channel})).map(record=>({...record,restorationStage:Number(record.restoration_stage)||0,expeditionRuns:Number(record.expedition_runs)||0,outpostStage:Number(record.outpost_stage)||0,wastelandProgress:Number(record.wasteland_progress)||0,bodyStage:Number(record.body_stage)||0})).slice(0,leaderboardConfig.limit)}
async function refreshOwnAscensionRank(){const a=normalizeAscension();if(!a.ascended)return;try{await playerAscensionBootstrap();save();const label=document.querySelector('.sheet-ascension-rank b');if(label)label.textContent=`第 ${a.serverAscensionRank} 名`}catch{}}
...
async function openImmortalLeaderboard(){closeGameMenu();$('#leaderboardModal')?.remove();const modal=document.createElement('div');modal.id='leaderboardModal';modal.className='leaderboard-modal immortal-leaderboard-modal';modal.innerHTML='<section class="leaderboard-window"><button class="leaderboard-close" aria-label="關閉">×</button><header><small>荒界留名・前五十名</small><h2>萬界復甦錄</h2><p>不問凡間境界，只記錄修士為仙界尋回多少生機</p></header><nav class="leaderboard-tabs"><button data-leaderboard-tab="restoration" class="active">復界榜</button><button data-leaderboard-tab="void">碎虛榜</button></nav><div class="leaderboard-list"><p class="leaderboard-loading">正在感應諸界碑文……</p></div></section>';document.body.appendChild(modal);const close=()=>modal.remove();modal.querySelector('.leaderboard-close').onclick=close;modal.onclick=event=>{if(event.target===modal)close()};const openVoid=()=>{renderLeaderboardTabs(modal,'void');modal.querySelector('.leaderboard-list').innerHTML='<div class="leaderboard-coming"><b>碎虛天榜</b><p>尚無修士破碎虛空</p><small>此榜將記錄離開仙界、踏入虛空的先後與所留道果；目前僅建立榜單，暫無排名內容。</small></div>'};const openRestoration=async()=>{renderLeaderboardTabs(modal,'restoration');const list=modal.querySelector('.leaderboard-list');list.innerHTML='<p class="leaderboard-loading">正在感應諸界碑文……</p>';try{await syncLeaderboard();const records=await fetchImmortalLeaderboard();list.innerHTML=records.length?records.map((record,index)=>`<article class="leaderboard-rank rank-${index+1}"><strong>${index+1}</strong><div><b>${escapeLeaderboardText(record.player_name||'無名仙修')}</b><small>引道碑 ${record.restorationStage}/3・荒原據點 ${record.outpostStage}/3・界軀 ${record.bodyStage} 階</small></div><span><small>荒原留痕</small>${record.wastelandProgress}/9</span></article>`).join(''):'<p class="leaderboard-empty">萬界寂然，尚無仙修在碑上留名</p>'}catch{list.innerHTML='<p class="leaderboard-empty">界碑暫失感應，請稍後再試</p>'}};modal.querySelector('[data-leaderboard-tab="restoration"]').onclick=openRestoration;modal.querySelector('[data-leaderboard-tab="void"]').onclick=openVoid;modal.classList.add('show');openRestoration()}
async function openLeaderboard(){
  if(normalizeAscension().ascended&&normalizeAscension().currentRealm==='immortal')return openImmortalLeaderboard();
...
function cultivationEfficiency() { const value=Math.max(0,effectiveCore('comprehension'));return Math.floor(2+3*Math.sqrt(Math.max(0,value-5))); }
function auraEfficiency() { return Math.floor(2.5*Math.sqrt(Math.max(0,effectiveCore('fortune')))); }
...
function swordEssenceRate(){return state.swordPathOpened?Math.max(1,Math.floor(12.5*pathEfficiency(state.swordLevel||0)*partnerRouteMultiplier('sword')*auraInfusionMultiplier('sword'))):0}
...
function auraInfusionActive(path,now=gameNow()){return state.auraInfusionPath===path&&(state.auraInfusionUntil||0)>now}
function auraInfusionMultiplier(path,now=gameNow()){return auraInfusionActive(path,now)?1.12:1}
function rate() { const authoritative=sessionOnline&&serverCultivationRate!=null?serverCultivationRate:Math.floor(baseRate()*cultivationMultiplier()*partnerRouteMultiplier('qi')),practice=sessionOnline&&buffActive('practiceBuff')?5:1;return Math.max(1,Math.floor(authoritative*practice*auraInfusionMultiplier('qi'))); }
...
async function beginAuraInfusion(path){const labels={qi:'練氣',sword:'淬劍',body:'煉體'},effects={qi:'修為獲取 +12%',sword:'劍元獲取 +12%',body:'每輪筋骨、氣血、臟腑根基收益 +12%'};if(!labels[path])return;const cost=auraInfusionCost();if(state.aura<cost)return toast(`靈氣不足・需要 ${formatLargeNumber(cost)}`);try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_aura_infusion',{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_cave_revision:serverCaveRevision,p_request_id:crypto.randomUUID(),p_path:path});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverCaveRevision=Number(result?.cave_revision)||serverCaveRevision;applyServerWalletSnapshot(result?.wallet?.resources);state.auraInfusionPath=result.path;state.auraInfusionUntil=Date.parse(result.until||'')||0;toast(`靈氣灌體・${effects[path]}・持續4小時`);renderSpiritRootView('pool');render();save()}catch(error){toast(error.message)}}
...
function load() {
  try {
    ...
    if(current&&!Object.prototype.hasOwnProperty.call(current,'bodyContinuousTrainingVersion'))current.bodyContinuousTrainingVersion=0;
...
function applyServerMailItemBalances(balances={}){
  ...
  for(const [key,value] of Object.entries(balances||{})){if(!validItemCounts.has(key))continue;const next=Math.max(0,Math.floor(Number(value)||0)),previous=Math.max(0,Math.floor(Number(state.serverMailItemBalances[key])||0)),delta=next-previous;if(delta)state[key]=Math.max(0,(Number(state[key])||0)+delta);state.serverMailItemBalances[key]=next}
}
function applyServerPermanentConsumables(effects={}){state.dosageLimitBonus=Math.max(0,Math.floor(Number(effects.dosageBonus)||0));if(effects.mindEmbodiment)state.mindEmbodimentUnlocked=true;const learned=new Set(Array.isArray(effects.learnedBooks)?effects.learnedBooks:[]),levels=effects.artLevels&&typeof effects.artLevels==='object'?effects.artLevels:{};state.serverPermanentLearnedBooks=[...learned];state.learnedBookIds=[...learned];state.learnedArts=[...(state.learnedArts||[]).filter(art=>art.source!=='book'),...[...learned].map(id=>{const book=itemCatalog[id]?.techniqueBook;return book?{...book,level:Math.max(1,Math.min(10,Number(levels[id])||1))}:null}).filter(Boolean)];const previous=state.serverPermanentAttributeBonuses&&typeof state.serverPermanentAttributeBonuses==='object'?state.serverPermanentAttributeBonuses:{},bonuses=effects.attributeBonuses&&typeof effects.attributeBonuses==='object'?effects.attributeBonuses:{};for(const key of ['trueQi','rootBone','physique','agility']){const next=Math.max(0,Number(bonuses[key])||0),before=Math.max(0,Number(previous[key])||0);state[key]=Math.max(0,(Number(state[key])||0)+next-before)}state.serverPermanentAttributeBonuses={...bonuses};state.pillUsage={...(state.pillUsage||{}),...(effects.pillUsage||{})};state.brewUsage={...(state.brewUsage||{}),...(effects.brewUsage||{})};return effects}
function applyServerSectArts(snapshot={}){state.artsCapacity=Math.max(8,Math.min(40,Number(snapshot.capacity)||8));const regular=[],skills=[];for(const record of Array.isArray(snapshot.arts)?snapshot.arts:[]){const art=sectTechniqueSet(String(record.sect||''),Number(record.star)||1).find(entry=>entry.slot===Number(record.slot));if(!art)continue;const canonical={...art,level:art.kind==='sectSkill'?1:Math.max(1,Math.min(10,Number(record.level)||1)),legacySectTechnique:false};if(canonical.kind==='sectSkill')skills.push(canonical);else regular.push(canonical)}state.learnedArts=[...(state.learnedArts||[]).filter(art=>art.source==='book'),...regular];state.learnedSectMoves=skills;state.swordMoves=(state.swordMoves||['origin']).filter(id=>combatTechniqueById(id)).slice(0,2);if(!state.swordMoves.length)state.swordMoves=['origin'];return snapshot}
function applyServerCombatLoadout(snapshot={}){state.swordEmbryo=['heavy','spirit','shadow'].includes(snapshot.swordEmbryo)?snapshot.swordEmbryo:'';state.swordMoves=(Array.isArray(snapshot.moves)?snapshot.moves:[]).map(String).filter(id=>combatTechniqueById(id)).slice(0,2);if(snapshot.swordName!=null)state.swordName=String(snapshot.swordName||'').slice(0,12);if(snapshot.swordNurtureLevel!=null)state.swordNurtureLevel=Math.max(0,Math.min(9,Number(snapshot.swordNurtureLevel)||0));if(snapshot.swordIntent!=null)state.swordIntent=Math.max(0,Number(snapshot.swordIntent)||0);if(snapshot.swordInsight!=null)state.swordInsight=Math.max(0,Number(snapshot.swordInsight)||0);if(snapshot.swordIntentType!=null)state.swordIntentType=swordIntents[snapshot.swordIntentType]?snapshot.swordIntentType:'';return snapshot}
async function serverSwordEmbryoSet(embryo,reversion=false){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sword_embryo_set',{p_channel:channel,p_embryo:embryo,p_reversion:reversion,p_request_id:crypto.randomUUID()});applyServerCombatLoadout(result?.loadout||{});applyServerMailItemBalances(result?.itemBalances||{});save();return result}
async function serverSwordStateCommand(action,value=''){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sword_state_command',{p_channel:channel,p_action:action,p_value:String(value),p_request_id:crypto.randomUUID()});applyServerCombatLoadout(result?.sword||{});if(result?.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}save();return result}
async function serverSectArtCommand(action,art){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sect_art_command',{p_channel:channel,p_action:action,p_sect:art.sourceSect,p_slot:art.slot,p_request_id:crypto.randomUUID()});applyServerSectArts(result?.arts||{});await playerCombatLoadoutBootstrap();if(result?.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}save();return result}
async function useServerAttributeConsumable(item,quantity){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_attribute_consumable_use',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_request_id:crypto.randomUUID()});applyServerMailItemBalances(result?.itemBalances||{});applyServerPermanentConsumables(result?.effects||{});return result}
function applyServerMainline(mainline={}){if(mainline.clearedStage!=null)state.mainlineCleared=Math.max(0,Math.min(18,Number(mainline.clearedStage)||0));return mainline}
async function useServerPermanentConsumable(item,quantity=1){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_permanent_consumable_use',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_request_id:crypto.randomUUID()});applyServerMailItemBalances(result?.itemBalances||{});applyServerPermanentConsumables(result?.effects||{});return result}
...
function render() {
  ...
  $('#bodyRealm').textContent=state.bodyPathOpened?bodyRealmName(state.bodyLevel):'尚未開啟';
...
async function upgrade(type) {
  ...
  if((spirit&&state.spiritLevel>=maxSpiritLevel)||(sword&&(state.swordLevel||0)>=mortalSwordMaxLevel)||(!spirit&&!sword&&state.bodyLevel>=mortalBodyMaxLevel))return toast(!spirit&&!sword?'肉身成聖四階已圓滿':sword?'凡間劍途已至劍域十層極限':'已達此道最高境界');
  ...
  if(!spirit&&!sword&&bodyTrialRequired()){state.activePath==='body'?openPrimaryBodyView('bodyTrial'):openExperienceView('bodyTrial');return toast('此境為肉身大關，需先完成試煉')}
  ...
  if(spirit||sword){
    const nextLevel=(spirit?state.spiritLevel:state.swordLevel||0)+1,gain=spirit?spiritAttributeGain(nextLevel):swordAttributeGain(nextLevel);
    try{await playerProgressionCommand('upgrade',type);toast(`已提升至${realmName(spirit?state.spiritLevel:state.swordLevel,spirit?spiritRealms:swordRealms)}`)}catch(error){toast(error.message);return}
  }
  else {
    try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_body_breakthrough',{p_channel:channel,p_expected_progression_revision:serverProgressionRevision,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID()});serverProgressionRevision=Number(result?.progression?.revision)||serverProgressionRevision;serverBodyRevision=Number(result?.body?.revision)||serverBodyRevision;applyServerProgression(result?.progression);applyServerBody(result?.body);await playerCoreAttributesGet()}catch(error){toast(error.message);return}
  ...
  if(!spirit&&!sword)renderBodyDestination('body');
  save();
}
async function openCultivationPath(type){
  ...
  if(type==='spirit'){
    ...
    try{await playerProgressionCommand('open_path','spirit')}catch(error){toast(error.message);return false}if(!(state.swordMoves||[]).length)state.swordMoves=['origin'];toast('已開啟練氣之路・開始累積修為');
  }else{
    ...
    try{await playerProgressionCommand('open_path',type)}catch(error){toast(error.message);return false}if(type==='body'&&!(state.swordMoves||[]).length)state.swordMoves=['body-origin'];toast(`已開啟${meta.name}之路・${meta.realm}${type==='body'?'・獲得起步口糧30':''}`);
  }
...
async function tribulate() {
  ...
  scheduleTribulation(()=>{
    ...
    if(success) {
      syncSectTaskRouteUnlocks('spirit');queueRealmEncounter('spirit',state.spiritLevel);
...
async function startSwordBreakthrough(cost=swordReq(state.swordLevel||0)){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sword_breakthrough',{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_progression_revision:serverProgressionRevision,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverProgressionRevision=Number(result?.progression?.revision)||serverProgressionRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerProgression(result?.progression);await playerCoreAttributesGet()}catch(error){toast(error.message);return}
...
function applyServerResourceRewards(rewards={}){const cultivation=toBigInt(rewards.free||0),sword=toBigInt(rewards.swordEssence||0);state.free+=cultivation;state.totalEarned+=cultivation;state.swordEssence+=sword;for(const key of ['aura','spiritStone','food','wood','meteorIron'])state[key]=Math.max(0,(Number(state[key])||0)+(Number(rewards[key])||0));return {cultivation,sword}}
...
function showPendingOfflineRewards(vault){pendingOfflineVault=vault;const seconds=Math.max(0,Math.floor(Number(vault?.elapsed_ticks)||0))*5,hours=Math.floor(seconds/3600),minutes=Math.floor(seconds%3600/60),secs=seconds%60,labels={free:'修為',swordEssence:'劍元',aura:'靈氣',spiritStone:'靈石',food:'食物',wood:'木材',meteorIron:'隕鐵'},rows=Object.entries(labels).map(([key,label])=>[label,toBigInt(vault?.rewards?.[key]||0)]).filter(([,amount])=>amount>0n);$('#offlineDuration').textContent=`離線時間 ${String(hours).padStart(2,'0')}:${String(minutes).padStart(2,'0')}:${String(secs).padStart(2,'0')}`;$('#offlineRewardList').innerHTML=rows.length?rows.map(([label,amount])=>`<div><span>${label}</span><b>+${formatLargeNumber(amount)}</b></div>`).join(''):'<p>本次沒有可領取的資源。</p>';const button=$('#offlineRewardClaim');button.disabled=false;button.textContent='領取離線收益';$('#offlineModal').classList.remove('hidden')}
async function claimOfflineRewards(){const vault=pendingOfflineVault,button=$('#offlineRewardClaim');if(!vault?.vault_id||button.disabled)return;button.disabled=true;button.textContent='領取中…';try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',receipt=await playerStateRpc('offline_reward_claim',{p_channel:channel,p_vault_id:vault.vault_id,p_request_id:crypto.randomUUID()}),rewards=receipt?.rewards||{},seconds=Math.max(0,Math.floor(Number(receipt?.elapsed_ticks)||0))*5,before=rewardSnapshot();applyServerResourceRewards(rewards);serverResourceWalletRevision=Number(receipt?.wallet_revision)||serverResourceWalletRevision;pendingOfflineVault=null;showOfflineRewards(before,seconds);button.textContent='已領取';save();render()}catch(error){button.disabled=false;button.textContent='重新領取';toast(error.message)}}
...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();await playerEncounterSync();await playerIdentityBootstrap();await playerAscensionBootstrap();await playerTitleProfile();await playerImmortalBootstrap();await playerCoreAttributesGet();await playerMoralBootstrap();sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await playerSectShopStatus();await syncJadeGrants({notify:false})}catch(error){sessionOnline=false;show('#titleScreen');$('#titleHint').textContent='需要連線伺服器才能進入遊戲';toast(error.message);return}
...
function swordNurtureCost(){const next=Math.max(1,(state.swordNurtureLevel||0)+1);return {iron:40+25*next+5*next*next,wood:20+12*next+2*next*next,stone:Math.ceil(150*Math.pow(next,1.42)),insight:2+Math.floor((next-1)/5)}}
...
async function chooseSwordEmbryo(id){
  ...
  try{await serverSwordEmbryoSet(id,false);if(!state.swordName)await serverSwordStateCommand('rename',`${state.name||''}之劍`.slice(0,12)||'無名靈劍');toast(`已凝聚${embryo.name}`);renderSwordDestination('sword');render();save()}catch(error){toast(error.message)}
}
async function renameSword(){const input=$('#swordNameInput');if(!input||!state.swordEmbryo)return;const name=input.value.trim().slice(0,12);if(!name)return toast('請輸入劍名');try{await serverSwordStateCommand('rename',name);toast(`本命劍定名為「${name}」`);renderSwordDestination('sword')}catch(error){toast(error.message)}}
async function nurtureSword(){
  if(!state.swordEmbryo)return toast('尚未凝聚本命劍');if(state.swordNurtureLevel>=swordNurtureMax())return toast('本命劍已養成圓滿');if(state.swordNurtureLevel>=swordNurtureLimit())return toast(`通過試劍境第 ${state.swordNurtureLevel*10} 關後開放下一階養劍`);const cost=swordNurtureCost();if(state.meteorIron<cost.iron||state.wood<cost.wood||state.spiritStone<cost.stone||state.swordInsight<cost.insight)return toast('養劍所需的感悟、隕鐵、木材或靈石不足');
  try{await serverSwordStateCommand('nurture');toast(`養劍完成・本命劍提升至${state.swordNurtureLevel}階`);renderSwordDestination('sword');render()}catch(error){toast(error.message)}
}
async function chooseSwordIntent(id){
  ...
  if(!await gameConfirm(`${intent.description}\n\n領悟後第一版本暫時無法更換。`,{title:'領悟劍意',confirmText:'領悟'}))return;try{await serverSwordStateCommand('intent',id);toast(`已領悟${intent.name}`);renderSwordDestination('sword');render()}catch(error){toast(error.message)}
}
async function setSwordMove(id,slot){
  const move=combatTechniqueById(id);if(!move||slot<0||slot>1)return;if(move.embryo&&move.embryo!==state.swordEmbryo)return toast(`此招只屬於${swordEmbryos[move.embryo].name}，目前尚未掌握`);if(!combatTechniqueAvailable(move))return toast(move.kind==='sectSkill'?'尚未習得此門派招式':`通過試劍境第${swordTechniqueUnlockStage(id)}關後解鎖`);
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_combat_move_set',{p_channel:channel,p_move:id,p_slot:slot,p_request_id:crypto.randomUUID()});applyServerCombatLoadout(result);toast(`${move.name}已設為第${slot+1}式`);renderArtsPanel('moves');save()}catch(error){toast(error.message)}
}
...
function renderBodyExperienceView(view,inner){
  refreshBodyState();refreshBodyTrainingCharges();const injuryId=activeBodyInjury(),injury=bodyInjuries[injuryId],nextIsRealm=bodyTrialRequired(),sessionNeed=bodySessionNeed(),foundationRows=[['bone','筋骨'],['blood','氣血'],['organs','臟腑']];
  ...
  if(state.bodyLevel>=mortalBodyMaxLevel){inner.innerHTML='<div class="realm-lock"><b>肉身成聖・四階圓滿</b><small>二十三境已全部走完；此身不假外物，自成一道。</small></div>';return}
  if(view==='body'){
    const heal=bodyHealCost(injuryId),ready=bodyFoundationsReady(),mode=bodyTrainingOptions()[state.bodyTrainingMode],remaining=mode?Math.max(0,state.bodyTrainingNextAt-gameNow()):0;inner.innerHTML=`<section class="body-dashboard"><div class="body-seal">體</div><div><small>當前肉身</small><h2>${bodyRealmName(state.bodyLevel)}</h2><p>以食物化為腹中精氣，選定鍛法後會持續修煉，離線期間照常推進。</p></div><div class="body-charge-summary"><b>${mode?`${mode.name}進行中`:'目前未鍛體'}</b><small>${mode?`下一循環約 ${formatDuration(remaining)}`:'前往鍛體頁選擇修煉方式'}</small></div><div class="body-foundation-grid">${foundationRows.map(([key,label])=>{const value=Math.min(sessionNeed,state.bodyFoundations[key]||0);return `<span><small>${label}</small><b>${value.toFixed(1)} / ${sessionNeed}</b><i><em style="width:${value/sessionNeed*100}%"></em></i></span>`}).join('')}</div><div class="body-meter"><span>腹中精氣 <b>${formatLargeNumber(state.bodyNutrition||0)} / ${formatLargeNumber(bodyNutritionCapacity())}</b></span><i><em style="width:${Math.min(100,(state.bodyNutrition||0)/bodyNutritionCapacity()*100)}%"></em></i><small>由食物補充；耗盡時持續鍛體會暫停</small></div><div class="body-meter body-load"><span>肉身負荷 <b>${Math.floor(state.bodyTrainingLoad)} / 100</b></span><i><em style="width:${state.bodyTrainingLoad}%"></em></i><small>負荷每分鐘自然消退1點</small></div><div class="body-condition ${injury?'injured':''}"><b>${injury?injury.name:'肉身無傷'}</b><small>${injury?`${injury.description}・剩餘 ${formatDuration(state.bodyInjuryUntil-gameNow())}`:'目前可進行所有鍛體方式。'}</small>${injury?`<button id="healBodyBtn">療傷・食物 ${formatLargeNumber(heal.food)}／木材 ${formatLargeNumber(heal.wood)}</button>`:''}</div><button id="goBodyAction" class="jade-button">${!ready?'前往鍛體':nextIsRealm?'查看肉身試煉':`突破至${bodyRealmName(state.bodyLevel+1)}`}</button></section>`;if(injury)$('#healBodyBtn').onclick=healBodyInjury;$('#goBodyAction').onclick=()=>!ready?renderBodyDestination('training'):nextIsRealm?renderBodyDestination('bodyTrial'):upgrade('body');return;
  }
  if(view==='training'){
    const options=bodyTrainingOptions(),active=state.bodyTrainingMode;inner.innerHTML=`<section class="body-training-head"><h2>持續鍛體</h2><p>先以食物補足腹中精氣，再選擇一種鍛法；完成根基、資源不足或主動停止時結束。</p><div class="body-nutrition-card"><span><small>腹中精氣</small><b>${formatLargeNumber(state.bodyNutrition||0)} / ${formatLargeNumber(bodyNutritionCapacity())}</b><em>持有食物 ${formatLargeNumber(state.food)}</em></span><button id="prepareBodyNutrition" ${(state.bodyNutrition||0)>=bodyNutritionCapacity()||state.food<1?'disabled':''}>進食補滿</button></div><div class="body-foundation-grid">${foundationRows.map(([key,label])=>{const value=Math.min(sessionNeed,state.bodyFoundations[key]||0);return `<span><small>${label}</small><b>${value.toFixed(1)} / ${sessionNeed}</b><i><em style="width:${value/sessionNeed*100}%"></em></i></span>`}).join('')}</div></section><div class="body-training-grid">${Object.entries(options).map(([id,item])=>{const disabled=!bodyTrainingCanPay(item)||id==='extreme'&&injuryId==='tendon',rootText=foundationRows.map(([key,label])=>`${label}+${item.roots[key]}`).join('・');return `<article class="${active===id?'active':''}"><h3>${item.name}</h3><p>${item.description}</p><small>${item.minutes}分鐘一循環・精氣 ${formatLargeNumber(item.nutrition)}${item.wood?`・木材 ${formatLargeNumber(item.wood)}`:''}${item.iron?`・隕鐵 ${formatLargeNumber(item.iron)}`:''}</small><b>${rootText}・負荷 ${item.load>0?'+':''}${item.load}${item.risk?`・受傷率 ${item.risk}%`:''}</b><button data-body-training="${id}" ${disabled||active===id?'disabled':''}>${active===id?'修煉中':'開始持續鍛體'}</button></article>`}).join('')}</div>${active?'<button id="stopBodyTraining" class="text-button body-stop-training">停止目前鍛體</button>':''}`;$('#prepareBodyNutrition').onclick=prepareBodyNutrition;$$('[data-body-training]').forEach(button=>button.onclick=()=>startBodyTraining(button.dataset.bodyTraining));if(active)$('#stopBodyTraining').onclick=()=>{stopBodyTraining();renderBodyDestination('training')};return;
  }
...
async function chooseSwordTrialPath(path){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerMoral(await playerStateRpc('player_sword_trial_moral_choice',{p_channel:channel,p_stage:stage,p_path:path,p_request_id:crypto.randomUUID()}))}catch(error){toast(error.message);return}
  state.swordTrialPendingChoice=0;$('#swordPathChoice')?.classList.add('hidden');if($('#battleResultClose'))$('#battleResultClose').disabled=false;if(liveChoice)$('#battleResultText').textContent+=` 此戰凝成${swordPaths[path].name}之悟。`;render();if(currentFeature==='experience')renderExperiencePanel('trial');else if(currentFeature==='swordPrimary')renderPrimarySwordPanel('trial');save();
}
...
async function startSwordTrial(){
  clearSwordTrialAdvance();
  if((state.swordTrialWins||0)>=swordTrialMaxStage)return toast('試劍境前九十關已全數通關');
  const stage=(state.swordTrialWins||0)+1;if(!state.swordEmbryo||stage>swordTrialMaxStage)return toast('目前試劍境關卡尚未開放');let ticket;try{await window.syncSwordTrialProfile?.();const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';ticket=await playerStateRpc('player_sword_trial_begin',{p_channel:channel,p_request_id:crypto.randomUUID()})}catch(error){toast(error.message);return}clearTimeout(battleTimer);startBgm('swordTrial');const player=battlePlayerStats(),generated=npcCoreFromPower(Number(ticket.enemy_power)||swordTrialPower(stage),{id:900000+stage,seedScope:'sword-trial'}),core=generated.core,enemy={combatPower:generated.combatPower,core,maxHp:combatHealth(core.rootBone),attack:Math.max(12,core.trueQi*5),defense:Math.max(0,core.physique*20),evasion:combatEvasion(core.agility),accuracy:combatAccuracy(core.spiritualPower),crit:combatCritical(core.spiritualPower)};
  battle={active:true,resolved:false,mode:'swordTrial',swordTrialAttemptId:ticket.attempt_id,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp,name:'劍道幻影',npc:{id:`sword-trial-${state.swordTrialWins}`},race:'human'},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');setBattleBackground('swordTrial');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className='battle-silhouette silhouette-human';$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent='劍道幻影';$('#battleLog').innerHTML=`<p><b>${state.name}</b>執起本命劍「${state.swordName}」，劍道幻影應念而生。</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700);
}
async function startBodyTrial(){
  refreshBodyState();const nextLevel=state.bodyLevel+1,nextIsRealm=bodyTrialRequired(nextLevel),requirement=bodyBreakthroughRequirement(nextLevel);if(!nextIsRealm||!bodyFoundationsReady()||!bodyBreakthroughMaterialsReady(requirement))return toast('根基、產線等級或突破物資尚未備妥');let ticket;try{await window.syncSwordTrialProfile?.();const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';ticket=await playerStateRpc('player_body_trial_begin',{p_channel:channel,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID()})}catch(error){toast(error.message);return}const autoEquipped=ensureBodyTrialTechnique();clearTimeout(battleTimer);startBgm('bodyTrial');const player=bodyTrialPlayerStats(),targetRounds=5+bodyTrialLevels.indexOf(nextLevel),enemy=bodyTrialEnemyStats(player,targetRounds),failures=Number(ticket.failures)||0,guaranteed=!!ticket.guaranteed;
  battle={active:true,resolved:false,mode:'bodyTrial',bodyTrialAttemptId:ticket.attempt_id,round:1,completedRounds:0,targetRounds,bodyTrialLevel:nextLevel,bodyRequirement:requirement,guaranteedBodyTrial:guaranteed,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp,name:'煉體試煉化身',npc:{id:`body-trial-${state.bodyLevel}`},race:'human'},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');setBattleBackground('bodyTrial');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className='battle-silhouette silhouette-human';$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent='煉體試煉化身';$('#battleLog').innerHTML=`<p><b>${state.name}</b>踏入試煉，必須以肉身撐過 ${targetRounds} 回合。${autoEquipped?`未裝配肉身招式，已自動將「${autoEquipped.name}」設為第一式。`:''}${guaranteed?'前兩次失敗已使肉身適應此境威壓，本次必能守住最後一息。':''}</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700);
}

const divineRoamingStoneCost=20000,divineRoamingJadeCost=35,mindEmbodimentJadeCost=15,divineRoamingAttemptMs=900000;
let divineServerSyncInFlight=false;
function applyServerDivineRoaming(result={}){const roaming=result.roaming||result;state.divineRoamingUnlocked=!!roaming.unlocked;state.divineRoamingDay=String(roaming.day||'');state.divineRoamingUsed=Math.max(0,Number(roaming.used)||0);const job=roaming.job;state.divineRoamingJob=job?{stageId:Number(job.stageId),total:Number(job.total),completed:Number(job.completed),nextAt:Number(job.nextAt)||0,returnRequested:!!job.returnRequested,...(job.pendingStageId?{pendingStageId:Number(job.pendingStageId),pendingTotal:Number(job.pendingTotal)||0}:{})}:null;const harvest={};for(const [key,amount] of Object.entries(roaming.harvest||{})){const item=Object.values(itemCatalog).find(entry=>entry.count===key);harvest[`state:${key}`]={key,name:item?.name||key,type:'state',amount:Math.max(0,Number(amount)||0)}}state.divineRoamingHarvest=harvest;if(result.itemBalances)applyServerMailItemBalances(result.itemBalances);if(result.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}return result}
async function syncServerDivineRoaming(action='get',stage=null,total=null){if(divineServerSyncInFlight)return null;divineServerSyncInFlight=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_divine_roaming',{p_channel:channel,p_action:action,p_stage:stage,p_total:total,p_legacy_unlocked:!!state.divineRoamingUnlocked,p_request_id:action==='get'?null:crypto.randomUUID()});applyServerDivineRoaming(result);save();return result}finally{divineServerSyncInFlight=false}}
...
function processDivineRoaming(){const job=state.divineRoamingJob;if(sessionOnline&&job&&job.nextAt<=gameNow()&&!divineServerSyncInFlight)syncServerDivineRoaming().then(()=>{if($('#divineRoamingModal.show'))openDivineHarvest()}).catch(error=>console.warn('divine roaming sync paused',error));return false}
...
async function requestDivineReturn(){processDivineRoaming();const job=state.divineRoamingJob;if(!job||job.completed>=job.total)return toast('神念目前已經歸體');if(job.returnRequested)return toast('已在等待本次遠遊結束');try{await syncServerDivineRoaming('return');const selected=+$('.divine-roaming-window[data-divine-selected-stage]')?.dataset.divineSelectedStage;if(selected)openDivineRoamingStage(selected);else openDivineHarvest();toast('已傳念召回・本次結束後神念回歸')}catch(error){toast(error.message)}}
...
async function startDivineRoaming(stage,total){processDivineRoaming();const daily=divineRoamingDaily(),job=state.divineRoamingJob,active=job&&job.completed<job.total,maxCount=Math.max(0,daily.remaining-(active?1:0));if(active&&job.stageId===stage.id)return toast('神念目前正在此地圖遠遊');if(total>(active?maxCount:daily.remaining))return toast('今日神念遠遊次數不足');try{await syncServerDivineRoaming('start',stage.id,total);openDivineRoamingStage(stage.id);toast(active?`本次結束後將轉往${stage.name}`:`神念已前往${stage.name}`)}catch(error){toast(error.message)}}
...
async function claimDivineHarvest(){const entries=Object.values(state.divineRoamingHarvest||{});if(!entries.length)return;const deltas=entries.map(x=>[x.key,x.amount]);if(!canStoreBagCounts(deltas))return toast('角色儲物袋容量不足');try{await syncServerDivineRoaming('claim');render();openDivineHarvest();toast('遠遊收益已由伺服器提取至角色儲物袋')}catch(error){toast(error.message)}}
...
function recoverFeaturePanel(page){
  cancelAnimationFrame(featureRecoveryFrame);featureRecoveryFrame=requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    else if(page.startsWith('immortal-'))renderImmortalFeature(page);
    else if(page==='mainline')renderMainlinePage();
    else description.textContent=descriptions[page]||'此頁面暫時無法顯示，請重新切換頁籤。';
  }));
}
const immortalInvestigations={
  array:{name:'熄滅古陣',description:'陣紋中仍有幾縷不屬於靈氣的微光。'},
  stele:{name:'無字殘碑',description:'碑面無字，背面卻留著一道向內斬開的裂痕。'},
  gate:{name:'傾塌界門',description:'界門下有枯靈殘兵徘徊，守著通往古臺外圍的斷路。'}
};
const immortalSuppressionBase={attack:.5,defense:.6,hp:.7,accuracy:.8,evasion:.8};
function immortalState(){const im=normalizeAscension().immortalRealm;if(!im.inventoryVersion){state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(state.immortalResidualSourceCount)||0))+Math.max(0,Math.floor(Number(im.residualSource)||0));state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(state.immortalBarrenTraceCount)||0))+Math.max(0,Math.floor(Number(im.barrenTraces)||0));im.inventoryVersion=1}if(!im.storyFlags||typeof im.storyFlags!=='object')im.storyFlags={};const base=defaults.ascension.immortalRealm.wasteland,saved=im.wasteland&&typeof im.wasteland==='object'?im.wasteland:{};im.wasteland={...base,...saved,routeProgress:{...base.routeProgress,...(saved.routeProgress||{})},storyFlags:{...(saved.storyFlags||{})}};Object.defineProperties(im,{residualSource:{configurable:true,enumerable:true,get:()=>state.immortalResidualSourceCount||0,set:value=>state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(value)||0))},barrenTraces:{configurable:true,enumerable:true,get:()=>state.immortalBarrenTraceCount||0,set:value=>state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(value)||0))}});return im}
const immortalExpeditionDailyLimit=3,immortalRestorationDurations=[4*3600000,20*3600000,48*3600000],wastelandRestorationDurations=[8*3600000,24*3600000,72*3600000];
function immortalTimelineDay(){const im=immortalState();if(!im.timelineStartedAt)im.timelineStartedAt=gameNow();return Math.max(1,Math.floor((gameNow()-im.timelineStartedAt)/86400000)+1)}
function immortalExpeditionDaily(){const im=immortalState(),today=dateKey(),daily=im.expeditionDaily&&typeof im.expeditionDaily==='object'?im.expeditionDaily:{date:'',used:0};if(today&&daily.date!==today){daily.date=today;daily.used=0}daily.used=Math.max(0,Math.min(immortalExpeditionDailyLimit,Math.floor(Number(daily.used)||0)));im.expeditionDaily=daily;return {...daily,remaining:Math.max(0,immortalExpeditionDailyLimit-daily.used)}}
function immortalRestorationJob(){const im=immortalState(),job=im.restorationJob;return job&&Number.isFinite(job.completesAt)&&job.targetStage===im.guideStoneStage+1?job:null}
function wastelandUnlocked(){const im=immortalState();return im.guideStoneStage>=3&&im.outerRouteCleared&&Object.values(im.storyFlags||{}).filter(Boolean).length>=7}
function wastelandState(){return immortalState().wasteland}
function wastelandExploration(){const w=wastelandState(),routeTotal=Object.values(w.routeProgress).reduce((sum,value)=>sum+Math.min(3,Math.max(0,Number(value)||0)),0);return Math.min(100,routeTotal*8+(w.outpostStage||0)*7+(w.bodyStage||0)*3)}
function wastelandRestorationJob(){const w=wastelandState(),job=w.outpostJob;return job&&Number.isFinite(job.completesAt)&&job.targetStage===w.outpostStage+1?job:null}
function immortalAdaptation(){const im=immortalState(),w=im.wasteland;return Math.min(100,[0,3,6,10][Math.max(0,Math.min(3,im.guideStoneStage||0))]+(w.bodyStage||0)*5+(w.outpostStage||0)*2)}
function immortalSuppression(){const response=immortalAdaptation()/100;return Object.fromEntries(Object.entries(immortalSuppressionBase).map(([key,base])=>[key,base+(1-base)*response]))}
function immortalEffectiveStats(){const stats=battlePlayerStats(),ratio=immortalSuppression();return {...stats,maxHp:Math.round(stats.maxHp*ratio.hp),attack:stats.attack*ratio.attack,qiAttack:stats.qiAttack*ratio.attack,bodyAttack:stats.bodyAttack*ratio.attack,defense:stats.defense*ratio.defense,accuracy:stats.accuracy*ratio.accuracy,evasion:stats.evasion*ratio.evasion}}
function immortalEffectiveCombatPower(){const ratio=immortalSuppression(),core={rootBone:effectiveCore('rootBone'),trueQi:effectiveCore('trueQi'),physique:effectiveCore('physique'),agility:effectiveCore('agility'),spiritualPower:effectiveCore('spiritualPower')};return Math.round(core.rootBone*combatPowerWeights.rootBone*ratio.hp+core.trueQi*combatPowerWeights.trueQi*ratio.attack+core.physique*combatPowerWeights.physique*ratio.defense+core.agility*combatPowerWeights.agility*ratio.evasion+core.spiritualPower*combatPowerWeights.spiritualPower*ratio.accuracy)}
function immortalExploration(){const im=immortalState();return Math.min(100,im.investigated.length*15+(im.guideStoneStage||0)*10+(im.outerRouteCleared?25:0))}
function immortalHeader(eyebrow,title,description){const im=immortalState(),storyCount=Object.values(im.storyFlags||{}).filter(Boolean).length,inWasteland=im.region==='wuxi-wasteland';return `<header class="immortal-page-head"><small>${eyebrow}</small><h2>${title}</h2><p>${description}</p><div class="immortal-resource-row"><span>仙界歷程 <b>第 ${immortalTimelineDay()} 日</b></span><span>殘界源質 <b>${im.residualSource}</b></span><span>荒痕 <b>${im.barrenTraces}</b></span><span>${inWasteland?'荒原探索':'探索'} <b>${inWasteland?wastelandExploration():immortalExploration()}%</b></span>${inWasteland?`<span>界軀 <b>${im.wasteland.bodyStage} 階</b></span>`:`<span>古臺碑錄 <b>${storyCount} / 7</b></span>`}</div></header>`}
function playImmortalStory({chapter,title,lines,onDone,background}){const modal=ensureAscensionModal(),im=immortalState(),storyBackground=background||(im.region==='wuxi-wasteland'?'assets/qstyle-v2/ascension/immortal-wuxi-wasteland-v1.png':'assets/qstyle-v2/ascension/immortal-realm-barren-v1.png');modal.classList.add('show');playAscensionStory({theme:'immortalBarren',chapter,title,background:storyBackground,className:'immortal-arrival',lines,onDone:()=>{modal.classList.remove('show');onDone?.();save();render();resumeWorldBgm();queueImmortalMilestoneStory()}})}
let immortalMilestoneStoryCooling=false;
function immortalMilestoneStories(){const im=immortalState(),player=state.name||'修士',exploration=immortalExploration();return [
  {id:'platform-surveyed',ready:im.investigated.length>=3,chapter:'飛升古臺・異變',title:'碑下之聲',lines:[ascensionLine('旁白','三處痕跡被串在一起時，無字殘碑忽然傳來極輕的震動。那不是聲音，更像一段被石頭記住的恐懼。','none'),ascensionLine('殘碑之聲','不要……讓它抵達虛空。門不是被敵人打碎的，是我們自己關上的。','none'),ascensionLine(player,'「它」是誰？'),ascensionLine('天路之主','這就是仙界留下的第一個問題。繼續復界；死去的世界，會用恢復的部分回答你。','master')]},
  {id:'first-expedition-return',ready:(im.expedition?.runs||0)>=1,chapter:'遠征・首次歸返',title:'未熄的營火',lines:[ascensionLine('旁白','你回到古臺，從廢棄營地帶回的灰燼忽然泛起一點暗紅。它沒有熱，卻證明那群前人曾在此等待過誰。','none'),ascensionLine(player,'他們等的人沒有回來。'),ascensionLine('天路之主','也可能回來了，只是已經不再是他們認識的那個人。通往虛空的路，從來不只考驗力量。','master')]},
  {id:'second-dawn',ready:immortalTimelineDay()>=2,chapter:'仙界歷程・第二日',title:'無風之夜',lines:[ascensionLine('旁白','仙界沒有日月。直到第二次「天色」轉暗，你才發現古臺外每隔一段距離，便有一枚面向虛空跪倒的石像。','none'),ascensionLine(player,'它們是在朝拜？'),ascensionLine('天路之主','不。它們是在抵抗某種從虛空回望仙界的東西。只是抵抗得太久，最後連自己為何跪著都忘了。','master')]},
  {id:'exploration-seventy-five',ready:exploration>=75,chapter:'飛升古臺・深痕',title:'斷路同鳴',lines:[ascensionLine('旁白','探索覆蓋古臺大半後，所有斷裂陣紋同時亮起一瞬。光芒沒有向外延伸，反而全部指向引道碑底部。','none'),ascensionLine('天路之主','古臺不是迎接飛升者的門。它原本是一座撤離陣。仙界之人曾試圖從這裡逃向凡間。','master'),ascensionLine(player,'所以九鎖封天，也許不只是凡間在防仙界。'),ascensionLine('天路之主','現在你開始看見兩邊都不願留下的真相了。','master')]},
  {id:'guide-second-stage',ready:(im.guideStoneStage||0)>=2,chapter:'復界・碑心甦醒',title:'陌生的座標',lines:[ascensionLine('旁白','接續的碑紋沒有指向無息荒原，而是在更高處勾出一個殘缺圓環。圓環之外，只有一道反覆收縮的黑線。','none'),ascensionLine(player,'那是虛空？'),ascensionLine('天路之主','是入口留下的傷口。有人成功破碎虛空，也有人從那道傷口裡回來。仙界的毀滅，或許始於後者。','master')]},
  {id:'third-dawn',ready:immortalTimelineDay()>=3,chapter:'仙界歷程・第三日',title:'第一縷風',lines:[ascensionLine('旁白','第三日，荒蕪仙界第一次起風。風從修復中的引道碑穿過，帶來極淡的泥土氣味。','none'),ascensionLine(player,'這裡還有活著的土地。'),ascensionLine('天路之主','或者，是土地察覺有人還沒有放棄它。無息荒原正在回應古臺。','master')]},
  {id:'platform-complete',ready:(im.guideStoneStage||0)>=3&&im.outerRouteCleared,chapter:'飛升古臺・終章',title:'死界的第一條路',lines:[ascensionLine('旁白','引道碑的光沿著古臺裂縫流動，穿過傾塌界門，最終在荒野中留下一條灰青色道路。','none'),ascensionLine('天路之主','飛升古臺已經完成它的新使命。從今日起，它不再只是凡間抵達仙界的終點，而是仙界重新出發的起點。','master'),ascensionLine(player,'下一站，無息荒原。然後沿著這些斷路，一直走到虛空。'),ascensionLine('天路之主','記住今日的風。若有一天你真的破碎虛空，至少要分得清——門外吹來的，還是不是同一個世界的風。','master'),ascensionLine('系統','飛升古臺篇・完成。下一仙域：無息荒原。','none')],complete:true}
]}
function queueImmortalMilestoneStory(){if(immortalMilestoneStoryCooling||immortalArrivalPending||battle?.active)return;window.setTimeout(async()=>{if(immortalMilestoneStoryCooling||currentFeature?.startsWith('immortal-')!==true||$('#ascensionModal')?.classList.contains('show'))return;const im=immortalState(),story=immortalMilestoneStories().find(entry=>entry.ready&&!im.storyFlags[entry.id]);if(!story)return;immortalMilestoneStoryCooling=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',snapshot=await playerStateRpc('player_immortal_story_ack',{p_channel:channel,p_story:story.id,p_request_id:crypto.randomUUID()});applyServerImmortal(snapshot);save();playImmortalStory({chapter:story.chapter,title:story.title,lines:story.lines,onDone:()=>{if(story.complete)toast('飛升古臺篇完成・無息荒原已開放');window.setTimeout(()=>immortalMilestoneStoryCooling=false,800)}})}catch{immortalMilestoneStoryCooling=false}},260)}
function renderImmortalChronicle(){const im=immortalState(),stories=immortalMilestoneStories(),next=$('#featureDescription .immortal-next-region');if(!next)return;const section=document.createElement('section');section.className='immortal-chronicle';section.innerHTML=`<header><b>古臺碑錄</b><small>${Object.values(im.storyFlags).filter(Boolean).length} / ${stories.length}</small></header><div>${stories.map((story,index)=>`<button data-replay-immortal-story="${story.id}" ${im.storyFlags[story.id]?'':'disabled'}><i>${String(index+1).padStart(2,'0')}</i><span>${im.storyFlags[story.id]?story.title:'碑文尚未顯現'}</span></button>`).join('')}</div>`;next.before(section);section.querySelectorAll('[data-replay-immortal-story]:not(:disabled)').forEach(button=>button.onclick=()=>{const story=stories.find(entry=>entry.id===button.dataset.replayImmortalStory);if(story)playImmortalStory({chapter:`重溫・${story.chapter}`,title:story.title,lines:story.lines})})}
const wastelandRoutes={dryWind:{name:'枯風谷',description:'沿著沒有風的谷道回收凝固荒痕。',reward:'荒痕 ×2、殘界源質 ×1'},sunken:{name:'沉界坑',description:'下探世界塌陷處，剝離較完整的殘界源質。',reward:'殘界源質 ×4'},kneeling:{name:'跪像原',description:'穿過面向虛空的石像群，尋找無聲城座標。',reward:'荒痕 ×1、殘界源質 ×2'}};
async function setImmortalRegion(region){const im=immortalState();if(region==='wuxi-wasteland'&&!wastelandUnlocked())return toast('完成古臺碑錄 7/7、引道碑與外圍遠征後開放');const firstArrival=region==='wuxi-wasteland'&&!im.wasteland.arrivalSeen;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',snapshot=await playerStateRpc('player_immortal_region_set',{p_channel:channel,p_region:region,p_request_id:crypto.randomUUID()});applyServerImmortal(snapshot);save()}catch(error){return toast(error.message)}renderAscensionEntrances();if(currentFeature==='immortal-domain'){renderImmortalDomain();if(region==='ascension-platform')renderImmortalChronicle()}else renderImmortalFeature(currentFeature);if(firstArrival)playImmortalStory({chapter:'無息荒原・序章',title:'風停在這裡',lines:[ascensionLine('旁白','引道碑照出的灰青道路在身後熄滅。前方沒有風，萬千跪像卻都朝著同一道虛空裂痕低首。','none'),ascensionLine(state.name||'修士','它們不是在向我跪。'),ascensionLine('天路之主','它們跪的是曾從虛空回來的東西。先在荒原立足，再決定要不要循著它們的目光走下去。','master'),ascensionLine('系統','第二仙域「無息荒原」開放。枯風谷探索路線已發現。','none')]})}
function immortalRegionTabs(){const im=immortalState(),unlocked=wastelandUnlocked();return `<nav class="immortal-region-tabs"><button data-immortal-region="ascension-platform" class="${im.region==='ascension-platform'?'active':''}">飛升古臺</button><button data-immortal-region="wuxi-wasteland" class="${im.region==='wuxi-wasteland'?'active':''}" ${unlocked?'':'disabled'}>無息荒原${unlocked?'':'・封鎖'}</button></nav>`}
function bindImmortalRegionTabs(){$$('[data-immortal-region]').forEach(button=>button.onclick=()=>setImmortalRegion(button.dataset.immortalRegion))}
function renderWastelandDomain(){const w=wastelandState(),progress=w.routeProgress,second=progress.dryWind>=3,third=w.outpostStage>=1,objective=progress.dryWind<3?'完成枯風谷三次探索':!w.outpostStage?'於復界頁修復聚源井':progress.sunken<3?'探索沉界坑':progress.kneeling<3?'追查跪像原':'等待無聲城深層區域開放';$('#featureDescription').innerHTML=`<section class="immortal-play-page wuxi-page">${immortalRegionTabs()}${immortalHeader('荒蕪仙界・第二仙域','無息荒原','風與聲音都在此地失去歸處。修復據點、鍛成界軀，才能靠近遠方的虛空傷口。')}<div class="wuxi-hero"><img src="assets/qstyle-v2/ascension/immortal-wuxi-wasteland-v1.png" alt="無息荒原"><span>無聲城外圍・虛空裂望台尚未開放</span></div><div class="immortal-objective"><small>目前目標</small><b>${objective}</b></div><div class="immortal-card-grid">${Object.entries(wastelandRoutes).map(([id,route],index)=>{const unlocked=id==='dryWind'||id==='sunken'&&second||id==='kneeling'&&third;return `<article class="immortal-action-card ${progress[id]>=3?'complete':''}"><i>0${index+1}</i><div><b>${route.name}</b><p>${route.description}</p><small>${unlocked?`探索 ${progress[id]} / 3・${route.reward}`:id==='sunken'?'完成枯風谷後發現':'修復聚源井後發現'}</small></div><button data-wasteland-route="${id}" ${unlocked?'':'disabled'}>${progress[id]>=3?'再次探索':unlocked?'前往遠征':'未發現'}</button></article>`}).join('')}</div><aside class="immortal-next-region"><small>更深區域</small><b>無聲城</b><span>第一階段暫未開放・跪像原的座標將指向城門</span></aside></section>`;bindImmortalRegionTabs();$$('[data-wasteland-route]:not(:disabled)').forEach(button=>button.onclick=()=>{w.selectedRoute=button.dataset.wastelandRoute;save();toggleFeature($('.immortal-feature-tab[data-page="immortal-expedition"]'))})}
function renderImmortalFeature(page){if(page==='immortal-domain'){renderImmortalDomain();if(immortalState().region==='ascension-platform')renderImmortalChronicle()}else if(page==='immortal-expedition')renderImmortalExpedition();else if(page==='immortal-path')renderImmortalPath();else if(page==='immortal-restoration')renderImmortalRestoration();queueImmortalMilestoneStory()}
function renderImmortalDomain(){const im=immortalState();if(im.region==='wuxi-wasteland')return renderWastelandDomain();const done=id=>im.investigated.includes(id);$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalRegionTabs()}${immortalHeader('荒蕪仙界・第一仙域','飛升古臺','仙界沒有靈氣。先讀懂這座殘臺留下的痕跡，才知道路斷在何處。')}<div class="immortal-objective"><small>目前目標</small><b>${!done('array')?'調查熄滅古陣':!done('stele')?'調查無字殘碑':!im.firstBattleWon?'清除界門下的枯靈殘兵':!im.outerRouteCleared?'由遠征深入飛升古臺外圍':'修復引道碑，尋找無息荒原的方向'}</b></div><div class="immortal-card-grid">${Object.entries(immortalInvestigations).map(([id,item],index)=>`<article class="immortal-action-card ${done(id)?'complete':''}"><i>0${index+1}</i><div><b>${item.name}</b><p>${item.description}</p></div><button data-investigate="${id}" ${id==='gate'&&!done('array')&&!done('stele')?'disabled':''}>${done(id)?'重看痕跡':id==='gate'?'接近界門':'調查'}</button></article>`).join('')}</div><aside class="immortal-next-region ${wastelandUnlocked()?'revealed':''}"><small>下一仙域</small><b>無息荒原</b><span>${wastelandUnlocked()?'道路已重接・可由上方仙域選擇前往':'迷霧封鎖・完成古臺碑錄 7/7、外圍遠征與引道碑修復'}</span></aside></section>`;bindImmortalRegionTabs();$$('[data-investigate]').forEach(button=>button.onclick=()=>investigateImmortalSite(button.dataset.investigate))}
function investigateImmortalSite(id){const im=immortalState(),seen=im.investigated.includes(id),record=async()=>{if(seen)return renderImmortalDomain();try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_immortal_investigate',{p_channel:channel,p_site:id,p_request_id:crypto.randomUUID()});applyServerImmortal(result.immortal||{});save();if(id==='array'&&result.firstVisit)toast('取得殘界源質 ×3');renderImmortalDomain()}catch(error){toast(error.message)}};if(id==='array'){playImmortalStory({chapter:'飛升古臺・其一',title:'熄滅古陣',lines:[ascensionLine('旁白','你俯身觸碰古陣。沒有靈氣回應，陣紋深處卻滲出幾縷灰白微光。','none'),ascensionLine('天路之主','那不是靈氣，是世界崩毀後尚未散盡的「殘界源質」。在仙界，修復與前行都要靠它。','master'),ascensionLine(state.name||'修士','原來仙界不是沒有力量，只是力量早已換了形態。','player')],onDone:record})}else if(id==='stele'){playImmortalStory({chapter:'飛升古臺・其二',title:'無字殘碑',lines:[ascensionLine('旁白','碑上無字。你繞到背面，才看見裂痕並非風化，而是從仙界內部向外斬出。','none'),ascensionLine('天路之主','仙界的枯竭不是天災。有人斬斷了諸域之間的循環，也把通往虛空的路一併藏了起來。','master'),ascensionLine(state.name||'修士','所以復甦仙界與破碎虛空，其實是同一條路。','player')],onDone:record})}else{if(!im.investigated.includes('array')||!im.investigated.includes('stele'))return toast('先讀完古陣與殘碑留下的線索');if(seen)return playImmortalStory({chapter:'飛升古臺・戰痕',title:'傾塌界門',lines:[ascensionLine('旁白','枯靈殘兵已散，只留下一道由內向外延伸的足跡。古臺外圍的斷路仍在等待你。','none')],onDone:renderImmortalDomain});playImmortalStory({chapter:'飛升古臺・其三',title:'傾塌界門',lines:[ascensionLine('旁白','你踏近界門，沉在石縫裡的甲片同時震動。乾枯人影從沙塵裡站起。','none'),ascensionLine('天路之主','凡間之力在此會被天地壓制。別用舊有戰力估量這一戰——先學會在沒有靈氣的地方活下來。','master'),ascensionLine('系統','進入戰鬥：枯靈殘兵','none')],onDone:()=>startImmortalBattle('domain')})}}
function renderImmortalPath(){const ratio=immortalSuppression(),adapt=immortalAdaptation(),im=immortalState(),w=im.wasteland,bodyCosts=[[4,2],[8,4],[14,7]],cost=bodyCosts[w.bodyStage];$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('凡道為基・仙途初啟','界應','界應代表你與仙界法則的適應程度。它不覆蓋凡間成長，只改變凡間力量在仙界能發揮多少。')}<div class="immortal-power"><small>凡間原始戰力</small><b>${formatCombatPower(combatPower())}</b><i>仙界實效戰力</i><strong>${formatCombatPower(immortalEffectiveCombatPower())}</strong></div><div class="immortal-adaptation"><div><span>界應</span><b>${adapt} / 100</b></div><progress max="100" value="${adapt}"></progress><p>${adapt?'引道碑、荒原據點與界軀正逐步讓這片天地承認你。':'仙界排斥所有來自凡間的既有法則。修復引道碑後，可取得最初的界應。'}</p></div>${wastelandUnlocked()?`<section class="wasteland-body"><header><small>仙界專屬成長</small><b>界軀・${w.bodyStage} / 3 階</b></header><p>以荒原殘質重塑肉身對仙界的承受方式。每階提供界應 +5，不取代凡間煉體。</p>${cost?`<div><span>殘界源質 <b>${im.residualSource} / ${cost[0]}</b></span><span>荒痕 <b>${im.barrenTraces} / ${cost[1]}</b></span></div><button data-upgrade-wasteland-body ${im.residualSource>=cost[0]&&im.barrenTraces>=cost[1]?'':'disabled'}>淬成第 ${w.bodyStage+1} 階界軀</button>`:'<strong>初階界軀已圓滿・後續階位將於更深仙域開放</strong>'}</section>`:''}<div class="immortal-ratio-grid">${[['攻擊','attack'],['防禦','defense'],['氣血','hp'],['命中','accuracy'],['閃避','evasion']].map(([label,key])=>`<span>${label}<b>${Math.round(ratio[key]*100)}%</b></span>`).join('')}</div><aside class="immortal-rule-note"><b>壓制規則</b><p>只作用於仙界戰鬥。凡間面板、修行成果與排行榜戰力完全保留；日後凡間三線仍可成長，返回仙界後再按界應比例換算。</p><small>長期目標・界應 100 → 仙界壓制完全解除 → 尋找破碎虛空之法</small></aside></section>`;$('#featureDescription').querySelector('[data-upgrade-wasteland-body]')?.addEventListener('click',upgradeWastelandBody)}
async function upgradeWastelandBody(){const im=immortalState(),w=im.wasteland,cost=[[4,2],[8,4],[14,7]][w.bodyStage];if(!cost)return;if(im.residualSource<cost[0]||im.barrenTraces<cost[1])return toast('淬鍊界軀所需材料不足');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_immortal_body_upgrade',{p_channel:channel,p_request_id:crypto.randomUUID()});applyServerImmortal(result);save();const stage=immortalState().wasteland.bodyStage;playImmortalStory({chapter:'仙途・界軀初成',title:`界軀第 ${stage} 階`,lines:[ascensionLine('旁白','殘界源質滲入經脈，荒痕則像細小刻刀，將不屬於仙界的部分一寸寸改寫。','none'),ascensionLine('天路之主','不是捨棄凡身，而是讓凡間走出的路，學會承受另一個世界。','master'),ascensionLine('系統',`界軀提升至第 ${stage} 階・界應 +5。`,'none')],onDone:renderImmortalPath})}catch(error){toast(error.message)}}
const immortalRestorationStages=[{name:'辨認碑基',cost:'殘界源質 ×2'},{name:'接續斷紋',cost:'荒痕 ×1、隕鐵 ×300'},{name:'喚醒引道',cost:'殘界源質 ×4、荒痕 ×2、木材 ×500'}];
const immortalRestorationCosts=[[['immortalResidualSource','immortalResidualSourceCount',2]],[['immortalBarrenTrace','immortalBarrenTraceCount',1],['meteorIron','meteorIron',300]],[['immortalResidualSource','immortalResidualSourceCount',4],['immortalBarrenTrace','immortalBarrenTraceCount',2],['wood','wood',500]]];
function immortalRestorationCostHtml(index){return immortalRestorationCosts[index].map(([itemId,count,need])=>{const item=itemCatalog[itemId],fallback=count==='meteorIron'?['隕鐵','assets/qstyle-v2/meteor-iron-cutout.png']:['木材','assets/qstyle-v2/wood-cutout.png'],name=item?.name||fallback[0],image=item?.image||fallback[1],owned=Math.max(0,Math.floor(Number(state[count])||0));return `<span class="immortal-material-cost ${owned>=need?'ready':''}"><img src="${image}" alt="${name}"><em>${name}</em><b>${formatLargeNumber(owned)} / ${formatLargeNumber(need)}</b></span>`}).join('')}
function renderImmortalRestoration(){const im=immortalState();if(im.region==='wuxi-wasteland')return renderWastelandRestoration();const stage=im.guideStoneStage,job=immortalRestorationJob(),remaining=job?Math.max(0,job.completesAt-gameNow()):0,ready=job&&remaining<=0;$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('殘界待甦・第一修復','引道碑','修復工程按現實時間推進，離開仙界或關閉遊戲仍會持續。')}<div class="guide-stone-visual stage-${stage}"><i></i><b>${stage>=3?'引道微明':job?'修復進行中':stage?'殘紋復燃':'死寂碑身'}</b><span>${stage} / 3</span></div>${job?`<div class="immortal-timed-job ${ready?'ready':''}"><small>第 ${job.targetStage} 階工程</small><b>${ready?'工程已完成':`剩餘 ${formatDuration(remaining)}`}</b><span>${ready?'領取後才會套用本階修復效果。':'材料已投入・離線期間照常施工'}</span><button data-claim-restoration ${ready?'':'disabled'}>${ready?'完成並領取':'施工中'}</button></div>`:''}<div class="restoration-stages">${immortalRestorationStages.map((item,index)=>`<article class="${stage>index?'complete':stage===index?'current':''}"><i>${stage>index?'✓':index+1}</i><div><b>${item.name}</b><span>${stage>index?'已完成':item.cost}</span>${stage===index&&!job?`<div class="immortal-material-costs">${immortalRestorationCostHtml(index)}</div>`:''}</div>${stage===index&&!job?`<button data-restore-stage="${index}">開始・${formatDuration(immortalRestorationDurations[index])}</button>`:''}</article>`).join('')}</div>${stage>=3?'<div class="immortal-complete-note"><b>界應 +10</b><span>引道碑已將你的存在寫入這片天地。無息荒原的方位若隱若現。</span></div>':'<p class="immortal-hint">同一時間只能進行一項復界工程；使用現實可信時間判定。</p>'}</section>`;$('#featureDescription').querySelector('[data-restore-stage]')?.addEventListener('click',startGuideStoneRestoration);$('#featureDescription').querySelector('[data-claim-restoration]')?.addEventListener('click',claimGuideStoneRestoration)}
const wastelandOutpostStages=[{name:'聚源井',description:'穩住荒原裂隙，開放跪像原並提高界應。',cost:[5,2]},{name:'鎮風柱',description:'替遠征標記歸路，建立第一處安全邊界。',cost:[9,4]},{name:'引路烽',description:'讓無聲城在荒霧中顯出輪廓。',cost:[15,7]}];
function renderWastelandRestoration(){const im=immortalState(),w=im.wasteland,stage=w.outpostStage,job=wastelandRestorationJob(),remaining=job?Math.max(0,job.completesAt-gameNow()):0,ready=job&&remaining<=0;$('#featureDescription').innerHTML=`<section class="immortal-play-page wuxi-page">${immortalHeader('無息荒原・立足之地','荒原據點','這不是把仙界恢復成從前，而是在死去的規則上建立一個能活下去的起點。')}<div class="wasteland-outpost stage-${stage}"><i></i><b>${stage>=3?'引路烽微明':job?'據點施工中':stage?'聚源井已甦':'荒地無標'}</b><span>${stage} / 3</span></div>${job?`<div class="immortal-timed-job ${ready?'ready':''}"><small>第 ${job.targetStage} 階據點工程</small><b>${ready?'工程已完成':`剩餘 ${formatDuration(remaining)}`}</b><span>現實時間施工・離線照常推進</span><button data-claim-wasteland-restoration ${ready?'':'disabled'}>${ready?'完成並領取':'施工中'}</button></div>`:''}<div class="restoration-stages">${wastelandOutpostStages.map((item,index)=>`<article class="${stage>index?'complete':stage===index?'current':''}"><i>${stage>index?'✓':index+1}</i><div><b>${item.name}</b><span>${stage>index?'已完成':item.description}</span>${stage===index&&!job?`<small>殘界源質 ${im.residualSource}/${item.cost[0]}・荒痕 ${im.barrenTraces}/${item.cost[1]}</small>`:''}</div>${stage===index&&!job?`<button data-start-wasteland-restoration ${im.residualSource>=item.cost[0]&&im.barrenTraces>=item.cost[1]?'':'disabled'}>開始・${formatDuration(wastelandRestorationDurations[index])}</button>`:''}</article>`).join('')}</div><p class="immortal-hint">同一時間只能建造一項荒原據點工程。</p></section>`;$('#featureDescription').querySelector('[data-start-wasteland-restoration]')?.addEventListener('click',startWastelandRestoration);$('#featureDescription').querySelector('[data-claim-wasteland-restoration]')?.addEventListener('click',claimWastelandRestoration)}
async function startWastelandRestoration(){if(!requireTrustedTime())return;const w=wastelandState(),item=wastelandOutpostStages[w.outpostStage];if(!item||wastelandRestorationJob())return;try{await playerImmortalRestoration('outpost','start');renderWastelandRestoration();toast(`${item.name}工程已開始`)}catch(error){toast(error.message)}}
async function claimWastelandRestoration(){if(!requireTrustedTime())return;const job=wastelandRestorationJob();if(!job)return;try{await playerImmortalRestoration('outpost','claim');const w=wastelandState(),item=wastelandOutpostStages[w.outpostStage-1];playImmortalStory({chapter:'復界・無息荒原',title:`${item.name}重立`,lines:[ascensionLine('旁白',w.outpostStage===1?'第一滴灰白源質從井壁凝出時，附近跪像同時抬起了半寸。':'殘破的據點再次亮起微光，荒原終於有了一處可以辨認的方向。','none'),ascensionLine('天路之主',w.outpostStage===1?'它們聽見的不是水聲，而是世界重新流動的聲音。跪像原的路已經出現。':'每修回一處，仙界便少一分死寂；但虛空也會更容易看見你。','master'),ascensionLine('系統',`${item.name}完成・界應 +2。`,'none')],onDone:renderWastelandRestoration})}catch(error){toast(error.message)}}
async function startGuideStoneRestoration(){if(!requireTrustedTime())return;const stage=immortalState().guideStoneStage;if(immortalRestorationJob())return toast('目前已有復界工程進行中');try{await playerImmortalRestoration('guide','start');renderImmortalRestoration();toast(`引道碑第 ${stage+1} 階工程已開始`)}catch(error){toast(error.message)}}
async function claimGuideStoneRestoration(){if(!requireTrustedTime())return;const job=immortalRestorationJob();if(!job)return;const stage=immortalState().guideStoneStage;try{await playerImmortalRestoration('guide','claim');const scripts=[['碑基之下','你剝去碑基上的死灰，發現石中埋著與天路同源的古紋。','這座碑不是路標。它曾把新來者的存在，寫入仙界法則。'],['斷紋重接','凡間隕鐵一觸碑身便迅速黯淡，卻替殘紋搭起短暫的橋。','凡物在仙界會衰敗，但並非毫無作用。你的舊路，仍能成為新路的材料。'],['引道微明','最後一道殘紋亮起。無靈荒野沒有風，遠方沙塵卻第一次向兩側分開。','碑接受了你的存在。界應初生，而路的盡頭指向一片名為「無息荒原」的死地。']][stage];playImmortalStory({chapter:`復界・第 ${stage+1} 階`,title:scripts[0],lines:[ascensionLine('旁白',scripts[1],'none'),ascensionLine('天路之主',scripts[2],'master'),...(stage===2?[ascensionLine('系統','取得界應 10。仙界戰力壓制略微緩和。','none')]:[])],onDone:()=>{toast(stage===2?'引道碑修復完成・界應 +10':'引道碑修復進度提升');renderImmortalRestoration()}})}catch(error){toast(error.message)}}
const immortalExpeditionNodes=[['古臺外緣','越過傾塌界門，確認撤退路線。'],['枯骨石林','從石化巨木間回收殘界源質。'],['殘兵伏擊','擊散伏在風沙中的第二具枯靈殘兵。'],['廢棄營地','調查早期飛升者留下的臨時營地。'],['斷門界碑','抵達外圍盡頭，標記通往無息荒原的方向。']];
function renderImmortalExpedition(){const im=immortalState();if(im.region==='wuxi-wasteland')return renderWastelandExpedition();const ex=im.expedition,daily=immortalExpeditionDaily();if(!im.expeditionUnlocked){$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('踏荒尋跡・尚未開放','遠征','先調查飛升古臺的三處異常，並清除傾塌界門下的守衛。')}<div class="immortal-lock-card"><b>飛升古臺外圍</b><span>完成仙域調查後開啟固定五節點遠征</span></div></section>`;return}$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('踏荒尋跡・第一路線','飛升古臺外圍','每日可開始三次遠征，額度於現實 00:00 恢復；已在途中的遠征不受跨日影響。')}<div class="immortal-daily-quota"><span>今日遠征</span><b>${daily.used} / ${immortalExpeditionDailyLimit}</b><small>剩餘 ${daily.remaining} 次</small></div><div class="expedition-route">${immortalExpeditionNodes.map((node,index)=>`<article class="${ex.active&&index===ex.node?'current':index<ex.node||!ex.active&&im.outerRouteCleared?'complete':''}"><i>${index+1}</i><div><b>${node[0]}</b><span>${node[1]}</span></div></article>`).join('')}</div><button class="immortal-primary-action" data-expedition-action ${!ex.active&&!daily.remaining?'disabled':''}>${ex.active?'繼續前進':daily.remaining?im.outerRouteCleared?'再次遠征':'整備並出發':'今日遠征次數已盡'}</button><small class="immortal-run-count">完成次數 ${ex.runs||0}・每日 00:00 恢復</small></section>`;$('#featureDescription').querySelector('[data-expedition-action]').onclick=advanceImmortalExpedition}
async function advanceImmortalExpedition(){const im=immortalState(),ex=im.expedition;if(!ex.active){if(!requireTrustedTime())return;const daily=immortalExpeditionDaily();if(!daily.remaining)return toast('今日三次遠征額度已用完，現實 00:00 恢復');try{await playerImmortalExpeditionCommand('start')}catch(error){return toast(error.message)}return playImmortalStory({chapter:'遠征・飛升古臺外圍',title:'離開古臺',lines:[ascensionLine('天路之主','遠征不是一路殺到底。記住你從哪裡來，也要知道什麼時候帶著所得回去。','master'),ascensionLine(state.name||'修士','先找出這片荒地仍剩下什麼，再決定下一步。','player')],onDone:renderImmortalExpedition})}if(ex.node===2)return playImmortalStory({chapter:'遠征・第三節點',title:'殘兵伏擊',lines:[ascensionLine('旁白','石林盡頭傳來甲片摩擦聲。另一具枯靈殘兵堵住了前路。','none'),ascensionLine('系統','進入戰鬥：枯靈殘兵','none')],onDone:()=>startImmortalBattle('expedition')});const node=ex.node;try{await playerImmortalExpeditionCommand('advance')}catch(error){return toast(error.message)}if(node===1)return playImmortalStory({chapter:'遠征・第二節點',title:'枯骨石林',lines:[ascensionLine('旁白','你從石化巨木的根部剝下三枚灰白晶片。它們沒有靈氣，卻讓周圍裂縫短暫閉合。','none'),ascensionLine('系統','取得殘界源質 ×3。','none')],onDone:renderImmortalExpedition});if(node===3)return playImmortalStory({chapter:'遠征・第四節點',title:'廢棄營地',lines:[ascensionLine('旁白','營地早已無人，只留下一句刻在石上的話：「若仙界已死，就替它決定下一次要活成什麼。」','none'),ascensionLine(state.name||'修士','前人沒能走完，但至少留下了方向。','player'),ascensionLine('系統','回收殘界源質 ×1。','none')],onDone:renderImmortalExpedition});playImmortalStory({chapter:'遠征・終點',title:'斷門界碑',lines:[ascensionLine('旁白','界碑背後，荒原延伸到視線之外。碑腳刻著四字：無息荒原。','none'),ascensionLine('天路之主','這只是仙界的第一寸路。修好引道碑，它會替你穿過封鎖，指出下一域真正的入口。','master')],onDone:()=>{toast('飛升古臺外圍通關・無息荒原線索已發現');renderImmortalExpedition()}})}
function wastelandRouteUnlocked(id){const w=wastelandState();return id==='dryWind'||id==='sunken'&&w.routeProgress.dryWind>=3||id==='kneeling'&&w.outpostStage>=1}
function renderWastelandExpedition(){const im=immortalState(),w=im.wasteland,daily=immortalExpeditionDaily(),selected=wastelandRouteUnlocked(w.selectedRoute)?w.selectedRoute:'dryWind';w.selectedRoute=selected;$('#featureDescription').innerHTML=`<section class="immortal-play-page wuxi-page">${immortalHeader('無息荒原・分路遠征','荒原探索','每天三次遠征額度與飛升古臺共用。每條路完成三次探索後，才會留下穩定路標。')}<div class="immortal-daily-quota"><span>今日遠征</span><b>${daily.used} / ${immortalExpeditionDailyLimit}</b><small>剩餘 ${daily.remaining} 次・現實 00:00 恢復</small></div><div class="wasteland-route-picker">${Object.entries(wastelandRoutes).map(([id,route])=>`<button data-select-wasteland-route="${id}" class="${selected===id?'active':''}" ${wastelandRouteUnlocked(id)?'':'disabled'}><b>${route.name}</b><small>${wastelandRouteUnlocked(id)?`${w.routeProgress[id]} / 3`:'尚未發現'}</small></button>`).join('')}</div><article class="wasteland-route-detail"><small>目前路線</small><b>${wastelandRoutes[selected].name}</b><p>${wastelandRoutes[selected].description}</p><span>本次收穫・${wastelandRoutes[selected].reward}</span></article><button class="immortal-primary-action" data-run-wasteland-route ${daily.remaining?'':'disabled'}>${daily.remaining?'開始本次探索':'今日遠征次數已盡'}</button></section>`;$$('[data-select-wasteland-route]:not(:disabled)').forEach(button=>button.onclick=()=>{w.selectedRoute=button.dataset.selectWastelandRoute;save();renderWastelandExpedition()});$('#featureDescription').querySelector('[data-run-wasteland-route]')?.addEventListener('click',runWastelandRoute)}
async function runWastelandRoute(){if(!requireTrustedTime())return;const im=immortalState(),w=im.wasteland,daily=immortalExpeditionDaily(),id=wastelandRouteUnlocked(w.selectedRoute)?w.selectedRoute:'dryWind',route=wastelandRoutes[id];if(!daily.remaining)return toast('今日遠征次數已盡');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_immortal_wasteland_route',{p_channel:channel,p_route:id,p_request_id:crypto.randomUUID()});applyServerImmortal(result);save();const serverW=immortalState().wasteland,progress=serverW.routeProgress[id]||0,first=progress===1,complete=progress===3;const lines=[ascensionLine('旁白',id==='dryWind'?'谷中沒有風，岩壁上的刮痕卻一夜比一夜更深。你沿痕跡找到一段凝固的世界傷口。':id==='sunken'?'坑底埋著倒置的宮闕。屋瓦朝向地心，彷彿整座城曾被某股力量從天空按落。':'每走過一尊石像，它便在你身後抬起頭，目光始終越過你，望向同一處虛空裂痕。','none'),ascensionLine('系統',`取得${route.reward}・路線進度 ${progress} / 3。`,'none')];if(id==='kneeling'&&first)lines.splice(1,0,ascensionLine('天路之主','它們不是死物。它們只是把最後一個念頭留在石頭裡：不要讓歸來者找到無聲城。','master'));if(complete)lines.push(ascensionLine('天路之主',id==='dryWind'?'這條路已被你記住。沉界坑正在遠處顯形。':id==='sunken'?'塌陷深處仍有規則流動；它可以成為界軀的材料。':'座標已完整，但無聲城仍被更古老的封鎖遮蔽。','master'));playImmortalStory({chapter:`遠征・${route.name}`,title:complete?'路標已立':'荒原留痕',lines,onDone:renderWastelandExpedition})}catch(error){toast(error.message)}}
function toggleFeature(button) {
  ...
  if(page==='root') {
    ...
    $('#featurePanel').classList.remove('feature-locked');renderImmortalFeature(page);
  } else {
...
function renderArtsPanel(view='sect',preserveScroll=false){
  ...
  const description=$('#featureDescription'),savedScrollTop=preserveScroll?description.scrollTop:0,savedScrollLeft=preserveScroll?description.scrollLeft:0,savedArtsScroll=preserveScroll?description.querySelector('#artsInner')?.scrollTop||0:0,savedTabScroll=preserveScroll?description.querySelector('.arts-book-tabs')?.scrollLeft||0:0;
...
async function forgetArt(id,view='sect'){const index=state.learnedArts.findIndex(item=>item.id===id);if(index<0)return;const art=state.learnedArts[index],kind=artKinds[art.kind],skill=art.kind==='sectSkill',lost=skill?0:artTotalEffect(art),sharp=skill?0:artSecondarySpiritualPower(art),warning=skill?'遺忘後將無法在戰鬥招式中裝配，重新加入原門派並達到學習職位後可再次習得。':`將失去 ${kind.label}+${lost.toLocaleString()}${sharp?`、銳識+${sharp.toLocaleString()}`:''}，已投入的靈氣不會返還。`;if(!await gameConfirm(`確定遺忘「${art.name}」？\n${warning}`,{title:'遺忘功法',confirmText:'確認遺忘',danger:true}))return;try{if(art.sourceSect)await serverSectArtCommand('forget',art);else if(art.source==='book'){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_book_art_forget',{p_channel:channel,p_art_id:id,p_request_id:crypto.randomUUID()});applyServerPermanentConsumables(result.effects||{});applyServerTitles(result.titles||{})}else return toast('此功法未通過伺服器登記，無法遺忘')}catch(error){toast(error.message);return}state.swordMoves=(state.swordMoves||[]).filter(moveId=>moveId!==id);if(!state.swordMoves.length)state.swordMoves=['origin'];toast(`已遺忘${art.name}${skill?'':`・${kind.label}-${lost}${sharp?`・銳識-${sharp}`:''}`}`);renderArtsPanel(view);render();save()}
...
async function expandArtsCapacity(){if(state.artsCapacity>=40)return;const cost=artsExpandCost();if(state.spiritStone<cost)return toast('靈石不足');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_arts_expand',{p_channel:channel,p_request_id:crypto.randomUUID()});applyServerSectArts(result?.arts||{});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources)}catch(error){toast(error.message);return}toast(`門派功法格擴充至${state.artsCapacity}格`);renderArtsPanel('sect');render();save()}
...
async function learnSectArt(id){const art=sectTechniqueSet().find(item=>item.id===id),skill=art?.kind==='sectSkill';if(!art||state.learnedArts.some(item=>item.id===id)||learnedSectSkills().some(item=>item.id===id))return;if(art.slot>=sectLearnLimit())return toast('目前職位不足');if(!skill&&state.learnedArts.filter(item=>item.sourceSect).length>=state.artsCapacity)return toast('門派功法格已滿');try{await serverSectArtCommand('learn',art)}catch(error){toast(error.message);return}toast(skill?`永久習得門派招式「${art.name}」・不占功法格`:`習得${art.name}`);renderSectLearning();render();save()}
...
function sectExperienceBonus(){return Math.max(0,Math.min(.5,Number(state.serverSectExperienceBonus)||0))}
...
function syncSectTaskRouteUnlocks(triggerPath=''){
  if(state.serverSectTaskRoutes&&typeof state.serverSectTaskRoutes==='object'){state.sectTaskRouteUnlocks={...state.serverSectTaskRoutes};return state.sectTaskRouteUnlocks}
...
async function joinSect(pick,bypassCooldown=false,invitationKey=null){
...
async function leaveSect(){
  ...
  processSectYears();syncCurrentSectRecord();try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sect_membership',{p_channel:channel,p_action:'leave',p_sect_name:null,p_invitation_key:null,p_request_id:crypto.randomUUID()});applyServerSect(result)}catch(error){toast(error.message);return}state.sectSearchAvailableAt=gameNow()+3*900000;
...
function applyServerEquipment(snapshot={}){serverEquipmentRevision=Number(snapshot.revision)||serverEquipmentRevision;const items=Array.isArray(snapshot.items)?snapshot.items:[],prior=state.equipmentInventory||[];for(const e of prior)state[`equipmentCount_${e.id}`]=0;state.equipmentInventory=items.map(e=>({id:String(e.id),slot:e.slot,tier:Number(e.tier),quality:e.quality,immortal:!!e.immortal,setName:e.immortal?'太荒補天仙裝':'',potential:e.potential&&typeof e.potential==='object'?e.potential:undefined,label:equipmentSlots.find(x=>x[0]===e.slot)?.[3]||'',value:Number(e.value)||0,affixes:Array.isArray(e.affixes)?e.affixes.map(a=>({element:a.element,value:Number(a.value)||0})):[]}));state.equippedItems={};items.forEach(e=>{state[`equipmentCount_${e.id}`]=e.equipped?0:1;if(e.equipped)state.equippedItems[e.slot]=String(e.id)});registerEquipmentItems();return snapshot}
function immortalMigrationPayload(){const im=immortalState();return JSON.parse(JSON.stringify({residualSource:im.residualSource,barrenTraces:im.barrenTraces,timelineStartedAt:im.timelineStartedAt,region:im.region,investigated:im.investigated,firstBattleWon:im.firstBattleWon,guideStoneStage:im.guideStoneStage,expeditionUnlocked:im.expeditionUnlocked,expedition:im.expedition,outerRouteCleared:im.outerRouteCleared,expeditionDaily:im.expeditionDaily,storyFlags:im.storyFlags,platformChapterCompleted:im.platformChapterCompleted,wasteland:im.wasteland}))}
function applyServerImmortal(snapshot={}){const im=normalizeAscension().immortalRealm,w=im.wasteland||{};state.immortalResidualSourceCount=Math.max(0,Number(snapshot.residualSource)||0);state.immortalBarrenTraceCount=Math.max(0,Number(snapshot.barrenTraces)||0);im.inventoryVersion=1;im.timelineStartedAt=Math.max(0,Number(snapshot.timelineStartedAt)||gameNow());im.region=snapshot.region==='wuxi-wasteland'?'wuxi-wasteland':'ascension-platform';im.investigated=Array.isArray(snapshot.investigated)?snapshot.investigated:[];im.firstBattleWon=!!snapshot.firstBattleWon;im.guideStoneStage=Math.max(0,Math.min(3,Number(snapshot.guideStoneStage)||0));im.restorationJob=snapshot.restorationJob||null;im.expeditionUnlocked=!!snapshot.expeditionUnlocked;im.expedition={active:!!snapshot.expedition?.active,node:Math.max(0,Number(snapshot.expedition?.node)||0),runs:Math.max(0,Number(snapshot.expedition?.runs)||0)};im.outerRouteCleared=!!snapshot.outerRouteCleared;im.expeditionDaily={date:snapshot.expeditionDaily?.date||'',used:Math.max(0,Number(snapshot.expeditionDaily?.used)||0)};im.storyFlags=snapshot.storyFlags&&typeof snapshot.storyFlags==='object'?snapshot.storyFlags:{};im.platformChapterCompleted=!!snapshot.platformChapterCompleted;im.wasteland={...w,...(snapshot.wasteland||{}),outpostJob:snapshot.wasteland?.outpostJob||null,routeProgress:{dryWind:0,sunken:0,kneeling:0,...(snapshot.wasteland?.routeProgress||{})},storyFlags:snapshot.wasteland?.storyFlags&&typeof snapshot.wasteland.storyFlags==='object'?snapshot.wasteland.storyFlags:{}};return snapshot}
async function playerImmortalBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerImmortal(await playerStateRpc('player_immortal_bootstrap',{p_channel:channel,p_legacy:immortalMigrationPayload()}))}
async function playerImmortalRestoration(kind,action){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_immortal_restoration_command',{p_channel:channel,p_kind:kind,p_action:action,p_request_id:crypto.randomUUID()});applyServerImmortal(result.immortal||{});if(result.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}save();return result}
async function playerImmortalExpeditionCommand(action){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_immortal_expedition_command',{p_channel:channel,p_action:action,p_request_id:crypto.randomUUID()});applyServerImmortal(result);save();return result}
...
async function setServerEquipment(equipment,equip){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_equipment_set',{p_channel:channel,p_item_id:equipment.id,p_equip:equip,p_request_id:crypto.randomUUID()});applyServerEquipment(result);save();return true}
async function equipInventoryItem(item){const e=item?.equipmentData;if(!e||(state[item.count]||0)<1)return false;try{await setServerEquipment(e,true);toast(`已裝備${item.name}`);render();return true}catch(error){toast(error.message);return false}}
async function unequipInventoryItem(equipment){if(!equipment||state.equippedItems?.[equipment.slot]!==equipment.id)return false;if(!canStoreItem(`equipment-${equipment.id}`,1))return toast('儲物袋空間不足，無法卸下裝備');try{await setServerEquipment(equipment,false);closeItemModal();renderBagView('character');render();toast(`已卸下${itemCatalog[`equipment-${equipment.id}`]?.name||'裝備'}`);return true}catch(error){toast(error.message);return false}}
...
async function useAttributePill(item,quantity){const p=item?.pillData;if(!p)return false;const limit=dosageLimit(),usageKey=`${p.key}_${p.tier}`,used=state.pillUsage?.[usageKey]||0,owned=state[item.count]||0,amount=Math.max(0,Math.min(limit-used,owned,Math.floor(quantity)));if(amount<1){toast(`此階此類丹藥已達服用上限 ${limit} 顆`);return false}try{const result=await useServerAttributeConsumable(item,amount),actual=Number(result?.quantity)||amount,totalGain=Number(result?.gain)||p.gain*actual;toast(`服用${item.name} ×${actual}・${p.label}+${totalGain}（${state.pillUsage?.[usageKey]||0}/${dosageLimit()}）`);render();save();return true}catch(error){toast(error.message);return false}}
async function useSpiritBrew(item,quantity){const b=item?.brewData;if(!b)return false;const limit=dosageLimit(),usageKey=`${b.key}_${b.quality}`,used=state.brewUsage?.[usageKey]||0,owned=state[item.count]||0,amount=Math.max(0,Math.min(limit-used,owned,Math.floor(quantity)));if(amount<1){toast(`此品質此類靈釀已達品飲上限 ${limit} 瓶`);return false}try{const result=await useServerAttributeConsumable(item,amount),actual=Number(result?.quantity)||amount,totalGain=Number(result?.gain)||b.gain*actual;toast(`品飲${item.name} ×${actual}・${b.label}+${formatLargeNumber(totalGain)}（${state.brewUsage?.[usageKey]||0}/${dosageLimit()}）`);render();save();return true}catch(error){toast(error.message);return false}}
async function useDosageLimitPill(key,quantity=1){const item=itemCatalog[key],gain=Math.max(0,Math.floor(item?.dosageLimitGain||0));if(!gain||(state[item.count]||0)<1)return false;quantity=Math.max(1,Math.min(state[item.count],Math.floor(quantity)));try{await useServerPermanentConsumable(item,quantity);toast(`服用${item.name} ×${quantity}・丹藥與靈釀上限提升至 ${dosageLimit()}`);render();save();return true}catch(error){toast(error.message);return false}}
function openItemModal(key){
  ...
  const sell=$('#itemModalSell');sell.disabled=count<1||item.sellable===false;sell.textContent=item.sellable===false?'不可售出':'出售';sell.onclick=item.sellable===false?null:()=>openSellModal(key,itemModalQuantity);
...
async function useTechniqueBook(key){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_book_art_learn',{p_channel:channel,p_item_key:item.count,p_request_id:crypto.randomUUID()});applyServerMailItemBalances(result?.itemBalances||{});applyServerPermanentConsumables(result?.effects||{})}catch(error){toast(error.message);return false}
...
async function useSectInvitation(key){
  ...
  return joinSect(invitation,true,item.count);
}
function isServerInventoryBundle(countKey){return /^reputation(?:spiritStone|wood|meteorIron)(?:100|1000|10000)Count$/.test(countKey)||/^market(?:wood|food|meteorIron)(?:100|1000|10000)Count$/.test(countKey)||/^marketCultivationCasket[1-5]Count$/.test(countKey)}
async function useResourceBundle(key,quantity=1){
  const item=itemCatalog[key],bundle=item?.resourceBundle;if(!bundle||(state[item.count]||0)<1)return false;
quantity=Math.max(1,Math.min(state[item.count],Math.floor(quantity)));try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_inventory_consume_bundle',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_expected_wallet_revision:serverResourceWalletRevision,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerMailItemBalances(result?.itemBalances||{});toast(`使用${item.name} ${formatLargeNumber(quantity)}個・${bundle.label}+${formatLargeNumber(Number(result?.reward)||bundle.amount*quantity)}`);render();save();return true}catch(error){toast(error.message);return false}
}
async function useCultivationBundle(key,quantity=1){const item=itemCatalog[key],amount=Number(item?.cultivationBundle)||0;if(amount<=0||(state[item.count]||0)<1)return false;quantity=Math.max(1,Math.min(state[item.count],Math.floor(quantity)));try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_inventory_consume_bundle',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_expected_wallet_revision:serverResourceWalletRevision,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerMailItemBalances(result?.itemBalances||{});const gain=BigInt(String(result?.reward||amount*quantity).split('.')[0]);state.totalEarned=BigInt(state.totalEarned||0)+gain;toast(`使用${item.name} ${formatLargeNumber(quantity)}個・修為+${formatLargeNumber(gain)}`);render();save();return true}catch(error){toast(error.message);return false}}
async function useStaminaMedicine(key,quantity=1){const item=itemCatalog[key],amount=Number(item?.staminaRestore)||0;refreshBodyState();if(amount<=0||(state[item.count]||0)<1)return false;quantity=Math.max(1,Math.min(state[item.count],Math.floor(quantity)));try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_body_consume_stamina_medicine',{p_channel:channel,p_quantity:quantity,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID()});serverBodyRevision=Number(result?.body?.revision)||serverBodyRevision;applyServerBody(result?.body||{});applyServerMailItemBalances(result?.itemBalances||{});toast(`使用${item.name} ${formatLargeNumber(result?.quantity||quantity)}個・體力+${formatLargeNumber(result?.gain||amount*quantity)}${state.bodyStamina>100?'（已溢出保留）':''}`);render();save();return true}catch(error){toast(error.message);return false}}
async function useMoralPill(key,quantity=1){const item=itemCatalog[key],gain=item?.moralGain;if(!gain||(state[item.count]||0)<1)return false;quantity=Math.max(1,Math.min(state[item.count],Math.floor(quantity)));try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerMoral(await playerStateRpc('player_moral_consume_pill',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_request_id:crypto.randomUUID()}));toast(`使用${item.name} ${formatLargeNumber(quantity)}個・${gain.label}+${formatLargeNumber(gain.amount*quantity)}`);render();save();return true}catch(error){toast(error.message);return false}}
async function useMindEmbodimentManual(){const item=itemCatalog.mindEmbodimentManual;if(hasMindEmbodiment())return toast('你已習得意念入體，本書已失效');if((state[item.count]||0)<1)return false;try{await useServerPermanentConsumable(item,1);toast('已習得「意念入體」・人物詳細屬性已開啟');render();save();return true}catch(error){toast(error.message);return false}}
...
async function confirmIdentityNameChange(){
  ...
  if(action==='protagonistName'){try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_identity_profile',{p_channel:channel,p_action:'rename',p_value:name,p_request_id:crypto.randomUUID()});applyServerIdentity(result.identity||{});applyServerMailItemBalances(result.itemBalances||{})}catch(error){$('#identityChangeError').textContent=error.message;return}}else{try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_partner_rename',{p_channel:channel,p_name:name,p_request_id:crypto.randomUUID()});applyServerPartner(result.partner);applyServerMailItemBalances(result.itemBalances||{});if(state.partnerStory)state.partnerStory.name=name}catch(error){$('#identityChangeError').textContent=error.message;return}}
...
async function useGenderRebirthMirror(key){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_identity_profile',{p_channel:channel,p_action:'gender',p_value:next,p_request_id:crypto.randomUUID()});applyServerIdentity(result.identity||{});applyServerPartner(result.partner);applyServerMailItemBalances(result.itemBalances||{})}catch(error){toast(error.message);return false}if(state.partnerStory)state.partnerStory.gender=state.partnerStory.gender==='男'?'女':'男';applyCharacterVisual();toast(established?'陰陽同轉・你與道侶皆已重塑此身':'陰陽輪轉・此身已重新顯化');render();save();return true;
}
function openSwordEmbryoReversion(){
  ...
  modal.classList.add('show');modal.querySelector('.divine-close').onclick=()=>modal.classList.remove('show');modal.querySelectorAll('[data-reversion-embryo]').forEach(button=>button.onclick=async()=>{const next=button.dataset.reversionEmbryo,embryo=swordEmbryos[next];if(!embryo||next===state.swordEmbryo)return;modal.classList.remove('show');if(!await gameConfirm(`確定將${swordEmbryos[state.swordEmbryo].name}重鑄為${embryo.name}？\n\n保留淬劍境界、養劍階數、試劍進度、劍意與劍名；養劍屬性方向和專屬劍招會改為新劍胚。`,{title:'重鑄本命劍胚',confirmText:'飲露重鑄'})){modal.classList.add('show');return}try{await serverSwordEmbryoSet(next,true);modal.classList.remove('show');toast(`本命劍已歸元重鑄・${embryo.name}`);render();if(currentFeature==='bag')renderBagView('bag');save()}catch(error){toast(error.message)}});return false;
}
async function useItem(key,quantity=1){let used=false;const item=itemCatalog[key];if(item?.equipmentData)used=await equipInventoryItem(item);else if(item?.identityAction==='gender')used=await useGenderRebirthMirror(key);else if(item?.identityAction)used=openIdentityChangeModal(key);else if(item?.swordEmbryoReversion)used=openSwordEmbryoReversion();else if(item?.dosageLimitGain)used=await useDosageLimitPill(key,quantity);else if(item?.pillData)used=await useAttributePill(item,quantity);else if(item?.brewData)used=await useSpiritBrew(item,quantity);else if(key==='mindEmbodimentManual')used=await useMindEmbodimentManual();else if(item?.techniqueBook)used=await useTechniqueBook(key);else if(item?.sectInvitation)used=await useSectInvitation(key);else if(item?.cultivationBundle)used=await useCultivationBundle(key,quantity);else if(item?.resourceBundle)used=await useResourceBundle(key,quantity);else if(item?.staminaRestore)used=await useStaminaMedicine(key,quantity);else if(item?.moralGain)used=await useMoralPill(key,quantity);if(used){closeItemModal();if(currentFeature==='bag')renderBagView('bag')}}
...
function openSellModal(key,quantity=1){
  ...
  if(item.sellable===false)return toast('此道具目前不可售出');
...
async function confirmSellItem(){
  ...
  let earned=quantity*itemSellPrice(item);if(item.equipmentData){try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_equipment_sell',{p_channel:channel,p_item_id:item.equipmentData.id,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerEquipment(result?.equipment||{});earned=Number(result?.earned)||1}catch(error){toast(error.message);return}}else if(Object.prototype.hasOwnProperty.call(state.serverMailItemBalances||{},item.count)){try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_inventory_sell',{p_channel:channel,p_item_key:item.count,p_quantity:quantity,p_expected_wallet_revision:serverResourceWalletRevision,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerMailItemBalances(result?.itemBalances||{});earned=Number(result?.earned)||quantity}catch(error){toast(error.message);return}}else{return toast('此物品尚未納入伺服器庫存，無法售出；請重新連線同步')}
  closeSellModal();closeItemModal();
  if(currentFeature==='bag')renderBagView('bag');render();save();toast(`已售出 ${item.name} ×${quantity}・靈石+${earned}`);
}
async function promoteSect(){try{await playerSectProgress('promote');syncCurrentSectRecord();toast(`晉升為${sectRanks[state.sectRank]}・本門功勳保留`);renderSectView('home');save()}catch(error){toast(error.message)}}
...
function applyServerSectSparStatus(status={}){state.serverSectSparDay=String(status.day||'');state.serverSectSparWonRoles=(Array.isArray(status.wonRoles)?status.wonRoles:[]).map(Number).filter(role=>role>=1&&role<=4);return status}
function npcDailyState(index){if(index>0)return{date:state.serverSectSparDay||'',sparWon:(state.serverSectSparWonRoles||[]).includes(index)};const key=String(sectNpcs()[index].id),record=state.npcDaily[key],today=dateKey();if(!today)return record||{date:'',sparWon:true};if(!record||record.date!==today)state.npcDaily[key]={date:today,sparWon:false};else if(typeof record.sparWon!=='boolean')record.sparWon=false;return state.npcDaily[key]}
...
async function npcAction(index,action){const n=sectNpcs()[index],daily=npcDailyState(index);if(['greet','spar'].includes(action)&&!requireTrustedTime())return;if(action==='spar'){if(daily.sparWon)return toast('今日已切磋勝利，明日再來');startNpcBattle(n);return}else if(action==='challenge'){challengeMaster();return}else if(action==='greet'){if(state.lastGreetingDay===dateKey())return toast('今日已向掌門請安');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerSect(await playerStateRpc('player_sect_greet',{p_channel:channel,p_request_id:crypto.randomUUID()}));toast('掌門頷首嘉許・門派貢獻+100')}catch(error){toast(error.message);return}}else if(action==='arts'){renderSectLearning();return}else if(action==='shop'){renderSectPanel('shop');return}renderNpcDetail(index);save()}
...
function bodyTrialPlayerStats(){
  const core=bodyOnlyCore(),hpBonus=(bodyPassiveUnlocked(1)?.1:0)+(bodyPassiveUnlocked(8)?.15:0)+(bodyPassiveUnlocked(18)?.15:0),injury=activeBodyInjury();
  return {maxHp:Math.round(combatHealth(core.rootBone)*(1+hpBonus)*(injury==='internal'?.85:1)),attack:Math.max(12,core.trueQi*5),defense:Math.max(0,core.physique*20),evasion:combatEvasion(injury==='tendon'?core.agility*.85:core.agility),accuracy:combatAccuracy(core.spiritualPower),crit:combatCritical(core.spiritualPower),damageReduction:bodyPassiveUnlocked(9)?.15:0};
}
...
async function startNpcBattle(n,mode='spar',masterTicket=null){
  let sparTicket=null;if(mode==='spar'){try{await window.syncSwordTrialProfile?.();const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';sparTicket=await playerStateRpc('player_sect_spar_begin',{p_channel:channel,p_role:n.role,p_request_id:crypto.randomUUID()})}catch(error){toast(error.message);return}}
  clearTimeout(battleTimer);
  startBgm('battle');
  const player=battlePlayerStats(),enemy=battleEnemyStats(n);
  battle={active:true,resolved:false,mode,sectSparAttemptId:sparTicket?.attemptId||null,sectMasterAttemptId:masterTicket?.attemptId||null,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp,name:n.name,npc:n,race:'human'},logs:[]};
...
function playerBattleTurn(){
  ...
  const healed=hit.dodged?0:Math.min(battle.player.maxHp-battle.player.hp,Math.max(0,Math.round(hit.damage*((technique.lifeSteal||0)+(typeof potentialSkillLifeSteal==='function'?potentialSkillLifeSteal():0)))));if(healed>0)battle.player.hp+=healed;const repeatText=hit.repeated?'，殘影返斬觸發':'';const healText=healed?`，本命回流恢復 ${formatBattleNumber(healed)} 氣血`:'';
...
function forceEndBattle(){
  ...
  if(battle.mode!=='spar')return finishBattle(false,battle.mode==='master'?'你中途認輸，本次掌門挑戰落敗。':battle.mode==='bodyTrial'?'你中途退出，本次肉身試煉落敗。':battle.mode==='immortal-sentinel'?'你撤離仙界戰場，本次沒有損失。':'你收劍退出，本次試劍落敗。');
...
async function finishBattle(won,reason){
  if(!battle||battle.resolved||battle.resolving)return;clearTimeout(battleTimer);clearSwordTrialAdvance();
  if(battle.mode==='swordTrial'){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_sword_trial_finish',{p_channel:channel,p_attempt_id:battle.swordTrialAttemptId,p_expected_progression_revision:serverProgressionRevision,p_request_id:crypto.randomUUID()});won=!!verdict.won;serverProgressionRevision=Number(verdict?.progression?.revision)||serverProgressionRevision;applyServerProgression(verdict?.progression);applyServerCombatLoadout(verdict?.sword||{});reason=won?'伺服器驗算通過，劍道幻影已被破除。':'伺服器驗算未通過，本次試劍落敗。'}catch(error){won=false;reason=`試劍結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle?.mode==='spar'){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_sect_spar_finish',{p_channel:channel,p_attempt_id:battle.sectSparAttemptId,p_request_id:crypto.randomUUID()});won=!!verdict.won;battle.sparPrestigeGain=Number(verdict.prestigeGain)||0;battle.sparIntentGain=Number(verdict.intentGain)||0;applyServerSect(verdict.sect||{});applyServerCombatLoadout(verdict.sword||{});applyServerSectSparStatus(verdict.status||{});reason=won?'伺服器裁定切磋勝利。':'伺服器裁定本次切磋落敗。'}catch(error){won=false;reason=`切磋結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle?.mode==='master'){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_sect_master_finish',{p_channel:channel,p_attempt_id:battle.sectMasterAttemptId,p_request_id:crypto.randomUUID()});won=!!verdict.won;applyServerSect(verdict.sect||{});reason=won?'伺服器裁定挑戰成功，代理掌門身分已生效。':'伺服器裁定本次掌門挑戰落敗。'}catch(error){won=false;reason=`掌門挑戰結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle?.mode==='bodyTrial'){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_body_trial_finish',{p_channel:channel,p_attempt_id:battle.bodyTrialAttemptId,p_expected_progression_revision:serverProgressionRevision,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID()});won=!!verdict.won;serverProgressionRevision=Number(verdict?.progression?.revision)||serverProgressionRevision;serverBodyRevision=Number(verdict?.body?.revision)||serverBodyRevision;applyServerProgression(verdict?.progression);applyServerBody(verdict?.body);reason=won?'伺服器驗算通過，肉身成功承受試煉。':'伺服器驗算未通過，本次肉身試煉落敗。'}catch(error){won=false;reason=`肉身試煉結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle?.mode==='mainline'){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_mainline_finish',{p_channel:channel,p_attempt_id:battle.mainlineAttemptId,p_request_id:crypto.randomUUID()});won=!!verdict.won;battle.mainlineFirstClear=!!verdict.firstClear;battle.mainlineServerRewards=verdict.rewards||{};applyServerMainline(verdict.mainline||{});applyServerMailItemBalances(verdict.inventory||{});reason=won?'伺服器驗算通過，九鎖關卡已被攻破。':'伺服器驗算未通過，本次挑戰落敗。'}catch(error){won=false;reason=`主線結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle&&(battle.mode==='ascension-delusion'||battle.mode==='ascension-road-end')&&battle.ascensionAttemptId){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_ascension_battle_finish',{p_channel:channel,p_attempt_id:battle.ascensionAttemptId,p_request_id:crypto.randomUUID()});won=!!verdict.won;applyServerAscension(verdict.ascension||{});reason=won?'伺服器驗算通過，此次飛升試煉成功。':'伺服器驗算未通過，此次飛升試煉失敗。'}catch(error){won=false;reason=`飛升結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(battle?.mode==='immortal-sentinel'&&battle.immortalAttemptId){battle.resolving=true;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',verdict=await playerStateRpc('player_immortal_battle_finish',{p_channel:channel,p_attempt_id:battle.immortalAttemptId,p_request_id:crypto.randomUUID()});won=!!verdict.won;battle.immortalFirstWin=!!verdict.firstWin;applyServerImmortal(verdict.immortal||{});reason=won?'伺服器裁定枯靈殘兵已被擊散。':'伺服器裁定本次仙界戰鬥落敗。'}catch(error){won=false;reason=`仙界戰鬥結算失敗：${error.message}`}finally{if(battle)battle.resolving=false}}
  if(!battle||battle.resolved)return;battle.active=false;battle.resolved=true;battle.won=won;
  let reward='';
  if(won&&battle.mode==='master'){reward=' 已取得代理掌門身分。'}
  else if(battle.mode==='swordTrial'){if(won){const intent=swordTrialIntentReward(state.swordTrialWins);reward=` 戰鬥感悟+1${intent?`、劍意+${intent}`:''}。`;}else reward=' 本關沒有消耗挑戰次數，可調整劍招後再戰。'}
  else if(battle.mode==='bodyTrial'){const trialLevel=battle.bodyTrialLevel||state.bodyLevel;if(won){await playerCoreAttributesGet();syncSectTaskRouteUnlocks('body');reward=` 肉身突破至${bodyRealmName(state.bodyLevel)}。`}else{const injury=activeBodyInjury(),failures=state.bodyTrialFailures[String(trialLevel)]||0;reward=` 突破物資完整保留${injury?`；留下${bodyInjuries[injury].name}`:''}。此境已失敗 ${failures} 次。`}}
  ...
  else if(battle.mode==='immortal-sentinel'){if(won&&battle.immortalSource==='domain')reward=battle.immortalFirstWin?' 荒痕+2；飛升古臺外圍遠征已開啟。':' 你再次擊散了古臺殘留的枯靈。';else if(won)reward=' 荒痕+1；遠征前路已清除。';else reward=' 凡間之力在仙界受到壓制；本次沒有損失，可再次挑戰。'}
  else if(battle.mode==='mainline'){if(won){const stage=battle.mainlineStage;if(battle.mainlineFirstClear)reward=` ${mainlineAftermath[stage.id-1]} 首通獎勵已由伺服器存入儲物袋。`;else reward=' 你已完整重溫此關劇情與戰鬥；重複通關不再獲得任何獎勵。持續取得素材需使用神念遠遊。'}else reward=' 可調整招式或提升任一修行道路後再次挑戰；本副本不消耗挑戰次數。'}
  else if(battle.mode==='spar'&&won){reward=` 聲望+${battle.sparPrestigeGain||5}${battle.sparIntentGain?`、劍意+${battle.sparIntentGain}`:''}；今日無法再與此人切磋。`}
...
async function dailyPractice(){if(!requireTrustedTime())return;try{await playerSectEconomy('start_practice');const years=state.actingLeader?20:10;toast(`練功已開啟・5倍修為持續${years}年`);renderSectView('practice');render();save()}catch(error){toast(error.message)}}
async function masterTransmission(count){try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_jade_service_purchase',{p_channel:channel,p_service:'transmission',p_value:String(count),p_request_id:crypto.randomUUID()});state.spiritJade=Math.max(0,Number(result?.jade?.balance)||0);serverJadeRevision=Number(result?.jade?.revision)||serverJadeRevision;applyServerSect(result?.sect||{});toast(`掌門傳功已開啟・${count} 次`);renderSectView('practice');render();save()}catch(error){toast(error.message)}}
async function claimSalary(){if(!requireTrustedTime())return;try{const result=await playerSectEconomy('claim_salary');toast(`俸祿・靈石+${formatLargeNumber(Number(result.salary)||0)}`);renderSectView('home');render();save()}catch(error){toast(error.message)}}
async function challengeMaster(){
  ...
  try{await window.syncSwordTrialProfile?.();const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',ticket=await playerStateRpc('player_sect_master_begin',{p_channel:channel,p_request_id:crypto.randomUUID()});applyServerSect(ticket.sect||{});save();render();startNpcBattle(sectNpcs()[0],'master',ticket)}catch(error){toast(error.message)}
}

const caveAreaMaxLevel=Number.MAX_SAFE_INTEGER;
const caveAreas = {
...
function areaWorkerMax(area,level=state[area.level]){
  ...
  if(area.value==='wood')return value*2;
  return value;
}
...
function caveFacilityEffect(key){
  ...
  return `鍛體消耗 -${Math.min(25,level*3)}%・精氣上限 +${level*150}・受傷率 -${level*2}%`;
}
...
async function craftEquipment(){const tier=Math.min(productionMaxTier(),state.craftingTier||1),slot=equipmentSlots.find(x=>x[0]===state.craftingSlot)||equipmentSlots[0],quality=state.craftingQuality==='rare'?'rare':'normal',need=forgeNeeds[tier-1],mainKey=`mainlineMaterial_${slot[4]}`,tierMat=tierMaterials[tier-1];if((state[mainKey]||0)<need[0]||lootAmount(tierMat)<need[1]||(quality==='rare'&&lootAmount('器靈精魄')<need[2]))return toast('製作素材不足');if(bagUsedSlots()>=bagCapacity())return toast('儲物袋容量不足');try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_equipment_forge',{p_channel:channel,p_slot:slot[0],p_tier:tier,p_quality:quality,p_request_id:crypto.randomUUID()});applyServerEquipment(result);applyServerMailItemBalances((await playerStateRpc('player_inventory_get',{p_channel:channel}))?.itemBalances||{});toast(`製成${quality==='rare'?'極品':'凡品'}·${equipmentSets[tier-1]}${slot[1]}`);save();renderCaveView('forge')}catch(error){toast(error.message)}}
...
async function startWeaving(){if(state.weavingJob)return;const tier=selectedWeavingTier(),recipe=weavingTiers[tier-1];if(state.wood<recipe.wood||state.spiritStone<recipe.stone)return toast('織天台所需木材或靈石不足');try{await playerWeavingCommand('start',tier);save();renderForgeProduction($('#caveInner'));render();toast(`${tier}階織法開爐・預計 ${recipe.years} 年後完成`)}catch(error){toast(error.message)}}
async function claimWeaving(){const job=weavingJobState();if(job.status!=='ready')return;try{await playerWeavingCommand('claim');save();renderForgeProduction($('#caveInner'));render();toast(`補天絲 ×${job.amount} 已收入儲物袋`)}catch(error){toast(error.message)}}
...
function applyServerCraftingDaily(daily={}){state.brewCraftDaily={date:String(daily.date||''),normal:Math.max(0,Number(daily.normal)||0),rare:Math.max(0,Number(daily.rare)||0)};return state.brewCraftDaily}
...
function studyRecordsView(){
  const cleared=Math.max(0,Math.min(mortalMainline.length,state.mainlineCleared||0)),swordWins=Math.max(0,Math.min(swordTrialMaxStage,state.swordTrialWins||0)),bodyTrials=bodyTrialLevels.filter(level=>(state.bodyLevel||0)>=level).length,bodyFailures=Object.values(state.bodyTrialFailures||{}).reduce((sum,value)=>sum+(Number(value)||0),0),encounters=(state.encounterHistory||[]).length,lastStage=cleared?mortalMainline[cleared-1]:null;
...
function renderCaveView(view){
  ...
  const cards=Object.entries(caveAreas).map(([key,a])=>{const level=state[a.level],cap=areaCapacity(a),max=areaWorkerMax(a),output=areaOutput(a),upgrade=areaUpgradeCost(a),nextCap=areaCapacity(a,level+1),stored=Math.floor(state[a.value]),full=stored>=cap,percent=Math.min(100,stored/Math.max(1,cap)*100),canUpgrade=state.wood>=upgrade.wood&&state.food>=upgrade.food;return `<article class="resource-area ${full?'storage-full':''}"><header><img src="${a.icon}" alt="${a.label}"><div><b>${a.label}</b><small>${level}級產地・可配置 ${max} 人</small></div><em>${full?'倉滿':'生產中'}</em></header><div class="resource-storage"><span><small>目前儲量</small><strong title="${stored.toLocaleString()}">${formatCaveAmount(stored)}</strong></span><i>／</i><span><small>儲存上限</small><strong title="${cap.toLocaleString()}">${formatCaveAmount(cap)}</strong></span></div><div class="resource-capacity-bar"><i style="width:${percent}%"></i></div><p>${full?'倉儲已滿，已暫停生產':`每名道童每 5 秒產出 ${formatCaveAmount(output)}${a.foodCost?`，消耗食物 ${formatCaveAmount(a.foodCost)}`:''}`}</p><div class="worker-stepper"><button data-worker="${key}" data-change="-1">−</button><span>道童 ${state[a.worker]} / ${max}</span><button data-worker="${key}" data-change="1">＋</button></div><button class="area-upgrade" data-upgrade-area="${key}" ${canUpgrade?'':'disabled'}><span>升至 ${level+1} 級・容量 ${formatCaveAmount(nextCap)}</span><small>木材 ${formatCaveAmount(upgrade.wood)}${upgrade.food?`／食物 ${formatCaveAmount(upgrade.food)}`:''}</small></button></article>`}).join('');
  ...
  if(view==='dwelling'){
    ...
    inner.innerHTML=`<section class="cave-section-title"><b>資源產地</b><small>道童上限：食物為等級×3.5取整、木材×2、隕鐵×1</small></section><section class="dao-child-yard"><img src="assets/qstyle-v2/dao-child.png" alt="道童"><div><small>可用道童</small><b>${availableChildren()} / ${state.daoChildTotal}</b><em>不限總數・招募費用平滑提高</em></div><button id="buyDaoChild" ${state.food>=cost?'':'disabled'}>招募<br>食物 ${formatLargeNumber(cost)}</button></section><div class="resource-area-grid">${cards}</div>`;
...
function renderSpiritRootView(view) {
  ...
  if(view==='pool') {
    ...
    const infusionCost=auraInfusionCost(),infusionLabels={qi:'練氣修為獲取',sword:'淬劍劍元獲取',body:'煉體三項根基收益'},infusionActive=(state.auraInfusionUntil||0)>gameNow(),infusionStatus=infusionActive?`${infusionLabels[state.auraInfusionPath]} +12%・剩餘 ${formatDuration(state.auraInfusionUntil-gameNow())}`:'目前未灌注';
    inner.innerHTML=`<div class="pool-page"><div class="pool-level">${state.spiritPoolLevel}階靈池</div><div class="pool-art small"><span></span><img src="assets/qstyle-v2/spirit-pool.png" alt="靈池"></div><div class="pool-stats"><div><small>靈氣產量・天契 +${formatLargeNumber(auraEfficiency())}</small><b>${formatLargeNumber(auraRate())} / 5秒</b></div><div><small>靈池容量・不受天契影響</small><b>${formatLargeNumber(state.aura)} / ${formatLargeNumber(auraCapacity())}</b></div></div><div class="pool-materials"><div class="pool-owned-materials"><span><img src="assets/qstyle-v2/wood-cutout.png" alt="木材"><em>木材</em><b>${formatLargeNumber(state.wood)}</b></span><i></i><span><img src="assets/qstyle-v2/meteor-iron-cutout.png" alt="隕鐵"><em>隕鐵</em><b>${formatLargeNumber(state.meteorIron)}</b></span></div><div class="pool-upgrade-cost">升至 ${nextLevel} 階：木材 ${formatLargeNumber(woodCost)}・隕鐵 ${formatLargeNumber(ironCost)}<br>產量 ${formatLargeNumber(nextRate)} / 5秒・容量 ${formatLargeNumber(nextCapacity)}</div></div><section class="aura-infusion"><div><b>靈氣灌體</b><small>消耗 ${formatLargeNumber(infusionCost)} 靈氣，持續4小時；練氣提高修為獲取、淬劍提高劍元獲取、煉體提高每輪筋骨／氣血／臟腑根基收益，各 +12%。重新選擇會覆蓋原效果。</small><em>${infusionStatus}</em></div><nav><button data-aura-infusion="qi" ${state.aura<infusionCost?'disabled':''}>灌注練氣</button><button data-aura-infusion="sword" ${state.aura<infusionCost||!state.swordPathOpened?'disabled':''}>灌注淬劍</button><button data-aura-infusion="body" ${state.aura<infusionCost||!state.bodyPathOpened?'disabled':''}>灌注煉體</button></nav></section><button id="upgradePoolBtn" class="jade-button" ${can?'':'disabled'}>靈池升階</button></div>`;
...
async function upgradeSpiritRoot(key) {
...
async function upgradeBag(){
  ...
  try{await playerWeavingCommand('upgrade');bagUpgradeDetailsOpen=false;toast(`儲物袋提升至${bagRankNames[state.bagRank-1]}階`);renderBagView('bag');save()}catch(error){toast(error.message)}
}
...
function renderWardrobeSection(section,preserveScroll=false){
  ...
  if(section==='outfits'){
    const g=state.gender==='男'?'male':'female',appearance=state.appearance||1,ascended=formalImmortalRealmEnabled&&normalizeAscension().ascended;
    const visibleOutfits=wardrobeOutfits[state.gender].filter(outfit=>!outfit.ascensionOnly||ascended);
    replaceWardrobeContent(inner,`<div class="wardrobe-intro"><b>衣閣藏衣</b><span>天工絕品需花費靈玉永久購得${ascended?'；飛升後可見仙品':''}；服裝不影響人物屬性。</span></div><div class="wardrobe-showcase-strip" data-wardrobe-strip="outfits">${visibleOutfits.map(outfit=>{const owned=masterworkOutfitOwned(outfit),selected=owned&&state.outfit===outfit.id,canBuy=(state.spiritJade||0)>=masterworkOutfitPrice;return `<article class="wardrobe-card ${outfit.effect?'mythic':''} ${outfit.animated?'animated-outfit':''} ${selected?'selected':''} ${owned?'':'locked'}" data-quality="${outfit.quality}" data-outfit-effect="${outfit.effect||'none'}"><span class="wardrobe-preview"><img src="${appearanceAsset(state.gender,appearance,outfit.id)}" alt="${outfit.name}"></span><b>${outfit.name}</b><small class="item-quality">${outfit.kind}${outfit.animated?'・本體動態':''}${outfit.quality==='masterwork'&&!owned?' ・ 未擁有':''}</small><button type="button" class="wardrobe-action" data-outfit-action="${outfit.id}" ${!owned&&!canBuy?'disabled':''}>${selected?'穿戴中':owned?'穿戴':`${masterworkOutfitPrice} 靈玉`}</button></article>`}).join('')}</div>`);
  ...
  $$('[data-title-action]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await playerTitleProfile('equip',button.dataset.titleAction);applyCharacterVisual();renderWardrobeSection('titles',true);save()}catch(error){button.disabled=false;toast(error.message)}});
  restoreWardrobeScroll(scrollTop);
}
...
inner.innerHTML=`<section class="character-sheet"><div class="sheet-header"><div><small>姓名</small><b>${state.name}</b></div><div><small>修煉歲月</small><b>${experiencedYears().toLocaleString()}年</b></div><div><small>練氣境界</small><b>${realmName(state.spiritLevel,spiritRealms)}</b></div><div><small>煉體境界</small><b>${bodyRealmName(state.bodyLevel)}</b></div><div><small>出生</small><b>${state.origin}</b></div><div><small>門派</small><b>${state.sect||'無門無派'}${state.actingLeader?'・代理掌門':''}</b></div></div><details class="sheet-fold"><summary><b>基礎白值</b><span>包含功法、裝備、丹藥等直接加值；不含百分比與臨時狀態</span></summary><div class="sheet-attributes"><div><span>命骨：${rootBone}</span><strong>氣血：${health}</strong></div><div><span>元息：${trueQi}</span><strong>攻擊：${attack}</strong></div><div><span>玄軀：${physique}</span><strong>防禦：${defense}</strong></div><div><span>游影：${agility}</span><strong><button id="evasionHelpBtn" class="attribute-help-button">閃避評級：${evasion}<i>？</i></button></strong></div><div><span>銳識：${spiritualPower}</span><strong>命中評級：${accuracy}・暴擊：${critical}%</strong></div><div><span>道悟：${comprehension}</span><strong>道悟修練效率：+${cultivationEfficiency()}</strong></div><div><span>天契：${fortune}</span><strong>天契靈氣獲取：+${auraEfficiency()}</strong></div><div><span>正氣：${Math.floor(state.righteousness)}</span><strong>邪氣：${Math.floor(state.evilQi)}</strong></div></div></details><details class="sheet-fold"><summary><b>五系功法屬性</b><span>靈根白值加成與裝備加成分開顯示</span></summary><div class="five-arts">${elements.map(([key,label,stateKey])=>`<span><b>${label}系白值＋${spiritRootBonus(state[stateKey]).toFixed(1)}%</b><small>${rootRank(state[stateKey])}・裝備＋${equippedElementBonus(key).toFixed(1)}%</small></span>`).join('')}</div></details><details class="sheet-fold"><summary><b>加成後實戰屬性</b><span>百分比、周天與當前狀態均已計入</span></summary><div class="final-attribute-grid"><span>命骨 <b>${final.rootBone}</b></span><span>元息 <b>${final.trueQi}</b></span><span>玄軀 <b>${final.physique}</b></span><span>游影 <b>${final.agility}</b></span><span>銳識 <b>${final.spiritualPower}</b></span><span>道悟 <b>${final.comprehension}</b></span><span>天契 <b>${final.fortune}</b></span><span>最終攻擊 <b>${final.trueQi*5}</b></span></div></details></section>`;
...
function openHelp(){
  $('#gameMenu').classList.add('hidden');const modal=$('#helpModal'),card=modal.querySelector('.help-card'),tabs=modal.querySelector('.help-tabs'),immortal=normalizeAscension().ascended&&normalizeAscension().currentRealm==='immortal';modal.dataset.realm=immortal?'immortal':'mortal';card.classList.toggle('immortal-help-card',immortal);card.querySelector('.eyebrow').textContent=immortal?'荒界引錄':'問道指引';card.querySelector('h2').textContent=immortal?'仙界幫助':'幫助';tabs.innerHTML=immortal?'<button class="active" data-help-tab="immortal-overview">總覽</button><button data-help-tab="immortal-domain">仙域</button><button data-help-tab="immortal-expedition">遠征</button><button data-help-tab="immortal-path">仙途</button><button data-help-tab="immortal-restoration">復界</button><button data-help-tab="immortal-bag">儲物袋</button>':'<button class="active" data-help-tab="cultivation">修行</button><button data-help-tab="spirit">練氣</button><button data-help-tab="sword">淬劍</button><button data-help-tab="encounter">因緣</button><button data-help-tab="body">煉體</button><button data-help-tab="battle">戰鬥</button><button data-help-tab="arena">問道臺</button><button data-help-tab="arts">靈根功法</button><button data-help-tab="cave">洞府</button><button data-help-tab="sect">門派</button>';tabs.querySelectorAll('[data-help-tab]').forEach(button=>button.onclick=()=>renderHelp(button.dataset.helpTab));renderHelp(immortal?'immortal-overview':'cultivation');modal.classList.remove('hidden');
}
...
function realmHelp(){const groups=[['練氣',spiritRealms,10],['煉體',bodyRealms,4],['淬劍',swordRealms,10]];return groups.map(([title,realms,stages])=>`<section class="help-realm-group"><h3>${title}境界</h3><ol>${realms.map((realm,index)=>`<li><span>${index+1}</span><b>${realm}</b><small>每境${stages===4?'四階':'十層'}</small></li>`).join('')}</ol></section>`).join('')}
function pathRealmHelp(title,realms,stages){return `<section class="help-realm-group"><h3>${title}境界</h3><ol>${realms.map((realm,index)=>`<li><span>${index+1}</span><b>${realm}</b><small>每境${stages===4?'四階':'十層'}</small></li>`).join('')}</ol></section>`}
function renderImmortalHelp(tab='immortal-overview'){const pages={
  'immortal-overview':helpCard('仙凡分界',['仙界與凡間是兩套獨立操作空間；身處仙界時不能使用凡間修行、洞府、門派等功能，返回凡間後亦不能操作仙界系統。','儲物袋為唯一跨越兩界保留的頁籤，仙界素材與凡間物品會存放在同一袋中。','仙界最終目標是修復諸域、理解枯竭真相，並找到破碎虛空之路。'])+helpCard('仙界時間',['仙界歷程按現實時間計算，首次進入新版仙界內容視為第1日。','每日額度以可信時間判定，於現實每日00:00恢復；調整裝置時間不會提前刷新。','復界工程在離線、返回凡間或關閉遊戲期間仍會持續施工。']),
  'immortal-domain':helpCard('仙域探索',['仙域頁可在已開放區域間移動，並顯示目前主線目標與探索進度。','飛升古臺完成碑錄7/7、引道碑3/3及外圍遠征後，開放第二仙域「無息荒原」。','無息荒原的枯風谷、沉界坑與跪像原會按劇情逐步發現，不會一次開放全部操作。'])+helpCard('荒界素材',['殘界源質是崩毀世界留下的法則殘質，主要用於復界、界軀與仙域建設。','荒痕來自枯靈及荒蕪遺址，是修復高階設施的關鍵素材。','兩種素材不可直接使用或出售，會由對應工程直接投入。']),
  'immortal-expedition':helpCard('每日遠征',['飛升古臺外圍採固定五節點路線；無息荒原則可在已發現的路線中選擇探索。','每日可開始3次遠征，兩個仙域共用額度，於現實00:00恢復。','無息荒原每條路需完成3次探索才會穩定；枯風谷完成後發現沉界坑，修復聚源井後發現跪像原。'])+helpCard('遠征收穫',['枯風谷偏向取得荒痕，沉界坑偏向取得殘界源質，跪像原則同時提供兩者並推進主線。','荒原材料可投入據點或淬鍊界軀，形成遠征、復界與仙途的循環。']),
  'immortal-path':helpCard('界應',['界應代表角色與仙界法則的適應程度，範圍為0至100。','界應不會取代凡間三條成長線；凡間能力仍可繼續提升，進入仙界後再按壓制比例換算。','引道碑提供最初10點界應；荒原據點每階提供2點，界軀每階提供5點。'])+helpCard('界軀',['界軀是仙界專屬成長，不等同凡間煉體，也不會覆蓋任何既有屬性。','目前開放3個初階階段，消耗殘界源質與荒痕，逐步提高仙界實效戰力。'])+helpCard('仙界戰力壓制',['界應為0時，攻擊發揮50%、防禦60%、氣血70%、命中與閃避80%。','界應提高後，各項比例會逐步回復；達到100時完全解除仙界壓制。','狀態欄與仙途頁顯示仙界實效戰力；凡間人物戰力與潛龍榜數值不受影響。']),
  'immortal-restoration':helpCard('復界工程',['飛升古臺可修復三階引道碑；無息荒原可建立聚源井、鎮風柱與引路烽。','荒原三階施工時間依序為現實8小時、24小時與72小時，離線期間照常推進。','材料在開工時立即扣除；完成後必須回到復界頁領取，才會套用進度與界應。'])+helpCard('荒原據點',['聚源井完成後會發現跪像原；後續設施建立安全邊界，並為無聲城開放預作準備。','據點不是讓仙界回到毀滅前，而是由玩家決定這片死界下一次如何活下去。']),
  'immortal-bag':helpCard('跨界儲物袋',['儲物袋是目前唯一能在仙界與凡間共同操作的系統。','仙界取得的殘界源質與荒痕會直接進入儲物袋，並占用正常格數。','復界工程會直接讀取並扣除袋內素材；不需要先手動使用。','仙界專屬素材目前不可出售，也不可在凡間直接使用。'])
};$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=pages[tab]||pages['immortal-overview']}
function renderHelp(tab='cultivation'){
  if($('#helpModal')?.dataset.realm==='immortal')return renderImmortalHelp(tab);
  if(tab==='arena'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('問道臺',['三路修行任一路達第五境一階後開放；問道臺位於凡間，對戰不套用法寶。','每日有10次免費挑戰；另可用5000靈石或10靈玉各購買最多5次。','每次顯示3名對手，名冊每5分鐘更新；同一名對手每天只能主動挑戰一次。'])+helpCard('積分與戰錄',['初始積分為1000分；擊敗較高分對手得到更多積分，落敗則依雙方分差扣分。','排行顯示前50名，超出50名仍可查看自己的名次；戰錄保留最近10場主動及被挑戰紀錄。','每週一00:00重置積分並結算前50名，獎勵會由問道臺郵件送達。']);return}
  if(tab==='cultivation'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('三路修行',['練氣累積修為、淬劍凝聚劍元、煉體消耗精氣與材料鍛鍊根基。','三條道路可同時成長；切換主修不會關閉已開啟的道路。','詳細玩法與境界名稱請查看各自頁籤。'])+helpCard('突破與飛升',['練氣跨境需渡雷劫；淬劍跨境需通過試劍境；煉體在指定境界需完成肉身試煉。','通過九鎖封天第18關，且練氣九境一階、淬劍九境一階或煉體第18境一階任一路達標，即可開啟飛升。']);return}
  if(tab==='spirit'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('練氣',['修為會在線與離線持續累積，足夠後即可突破小層。','周天可調整修煉效率、元息或渡劫成功率；每日靈機可取得修為、靈氣或渡劫加護。','跨入下一個大境界時需渡雷劫，可使用對應渡劫丹提高成功率。'])+pathRealmHelp('練氣',spiritRealms,10);return}
  if(tab==='sword'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('淬劍',['開啟淬劍後先選擇本命劍胚；劍元會在線與離線持續累積。','淬劍每逢第十層，需先擊敗對應試劍境關卡才能跨入下一境。','養劍會消耗戰鬥感悟、隕鐵、木材與靈石，並強化本命劍招。'])+pathRealmHelp('淬劍',swordRealms,10);return}
  if(tab==='body'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('煉體',['先用食物補充腹中精氣，再選擇調息養身、負重鍛骨或極境熬身。','鍛體可在線與離線持續進行；根基完成、材料不足或受傷限制時會自動停止。','完成筋骨、氣血與臟腑即可突破；進入第5、10、15、18、23境時需完成肉身試煉。'])+pathRealmHelp('煉體',bodyRealms,4);return}
  if(tab==='cave'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('洞府',['食物、木材與隕鐵產地可持續升級；道童上限依產線不同成長：食物為等級×3.5後無條件捨去、木材為等級×2、隕鐵為等級×1；儲存容量仍隨等級提升。','道童可無限制招募；木材與隕鐵生產會持續消耗食物。','食物可補充煉體精氣；木材、隕鐵則用於鍛體、養劍、擴建與製作。'])+helpCard('修行設施',['聚靈室提高修為，洗劍池提高劍元，鍛體室降低鍛體消耗與受傷率。','丹房與器室可製作的階級，依三條修行道路中的最高境界開放。']);return}
  if(tab==='arts'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('功法與靈根',['功法消耗靈氣升級；五行靈根會強化相同屬性的功法。','悟卷增加道悟，提高修煉效率；天箋增加天契，提高靈氣獲取。'])+helpCard('靈池',['靈池階級決定基礎產量與容量，天契只影響靈氣獲取速度。','靈氣可升級功法、淬鍊靈根，或用於靈氣灌體。','靈氣灌體可選擇一條修行道路，使其養成效率暫時提高；新效果會覆蓋舊效果。']);return}
  if(tab==='cultivation'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('三路修行',['練氣會在線與離線累積修為；淬劍凝聚獨立劍元；煉體則消耗食物化成的精氣與材料，按所選鍛法持續推進。','完成三途問心後選定最初道場；另外兩路可由兼修道場開啟，切換主修不會關閉已開啟的道路。','三路屬性都會計入人物與一般戰鬥，僅部分道場專修效果在兼修時降低。'])+helpCard('境界與考驗',['練氣及淬劍各大境界為十層；煉體共有23境、每境四階。','練氣跨大境界需渡雷劫；淬劍每逢第十層需先通過對應試劍境；煉體則在指定里程碑承受肉身試煉。','三路最高進度會共同決定門派、坊市、丹房、器室與九鎖封天等功能的開放階級。'])+helpCard('資源循環與飛升',['洞府食物可招募道童、維持木材與隕鐵生產，也可化為煉體精氣；木材供鍛體、擴建與養劍，隕鐵供極境鍛體、養劍及煉器。','修為、劍元與已啟動的持續鍛體皆支援離線結算；資源不足或根基完成時鍛體會自動停止。','完整通過九鎖封天第18關，且練氣九境一階、淬劍九境一階或煉體第18境一階任一路達標，才會開啟飛升。'])+realmHelp();return}
  if(tab==='body'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('持續鍛體',['煉體共有23境、每境四階；取消每日次數，選定鍛法後會在線與離線持續推進。','先在鍛體頁消耗食物補充腹中精氣；每個循環完成時才扣除精氣與材料。','根基完成、材料不足、筋傷阻止極境熬身或玩家主動停止時，持續鍛體會結束。'])+helpCard('三種鍛法',['調息養身每5分鐘一循環，只消耗精氣，偏重氣血與臟腑並降低負荷，不會受傷。','負重鍛骨每10分鐘一循環，消耗精氣與木材，偏重筋骨。','極境熬身每15分鐘一循環，消耗精氣、木材與隕鐵，三項收益最高；負荷越高越容易受傷。','消耗會依23境平滑提高；鍛體室最多降低25%消耗，並提高腹中精氣容量。'])+helpCard('突破、傷勢與飛升',['每階完成筋骨、氣血、臟腑後即可突破；進入第5、10、15、18、23境時需承受肉身試煉。','產線等級與倉儲不再是硬性突破門檻；經營產地是為了維持長期鍛體。','傷勢可等待痊癒或消耗食物、木材治療；治癒會留下破立值。','完成九鎖封天第18關，且練氣九境一階、淬劍九境一階或煉體第18境一階其中之一達標，即可開啟飛升。'])+realmHelp();return}
  if(tab==='cave'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('無限產地與容量',['食物、木材、隕鐵產地取消30級上限，可持續升級。','道童上限為食物產線等級×3.5後無條件捨去、木材產線等級×2、隕鐵產線等級×1；每名道童每5秒固定產出1份。','容量採平滑二次曲線；舊資源超出新容量時不會被刪除，但消耗至容量以下前不再生產。'])+helpCard('道童與食物收支',['道童取消總數上限，招募所需食物會隨已招募人數平滑增加。','食物線不消耗食物；木材線每名道童每5秒消耗2食物，隕鐵線消耗4食物，此比例維持不變。','食物不足時只會停止無法負擔的木材或隕鐵生產，不會產生負數。'])+helpCard('修行配合',['食物可補充煉體的腹中精氣，也是招募與兩條加工產線的底層燃料。','木材用於產地擴建、負重鍛骨與器室；隕鐵用於極境熬身、淬劍養成與器室。','聚靈室提高修為、洗劍池提高劍元；鍛體室降低持續鍛體消耗與受傷率，並提高精氣容量。','丹房與器室依三路最高境界逐階開放；儲物袋與產地倉儲是兩套不同容量。']);return}
  if(tab==='arts'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('道悟與悟卷',['悟卷仍永久增加道悟，但道悟對修煉效率改採遞減曲線；高階悟卷不再被其他倍率放大成失控速度。','人物頁會顯示道悟實際提供的每5秒基礎修煉效率；功法本身的道悟數值與既有存檔不會被刪減。'])+helpCard('天契、靈池與靈氣',['天契只增加靈氣獲取速度，不再提高靈池容量；天契的獲取效果已提高。','靈池階級獨立決定容量與基礎產量，升階不會削減目前持有的靈氣。','靈氣可淬鍊五行靈根、升級功法，也可在靈池進行靈氣灌體。'])+helpCard('靈氣灌體',['每次消耗靈池容量20%的靈氣，選擇練氣、淬劍或煉體其中一路，使該路養成效率 +12%，持續4小時。','同時間只能維持一種灌體；重新選擇會直接覆蓋原效果，不延長也不疊加。','灌體支援離線結算；尚未開啟的修行道路不能選擇。'])+helpCard('功法與靈根',['五行靈根會放大同屬性功法效果；功法可消耗靈氣升至10級。','遺忘功法不返還已投入靈氣；門派舊傳承可先免費轉換再決定是否保留。']);return}
  $$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));const helpTabs=$('.help-tabs'),activeHelpTab=helpTabs?.querySelector('.active');if(helpTabs&&!helpTabs.dataset.horizontalScroll){helpTabs.dataset.horizontalScroll='true';helpTabs.addEventListener('wheel',event=>{if(Math.abs(event.deltaY)<=Math.abs(event.deltaX))return;helpTabs.scrollLeft+=event.deltaY;event.preventDefault()},{passive:false})}if(helpTabs&&activeHelpTab)helpTabs.scrollLeft=activeHelpTab.offsetLeft-helpTabs.offsetLeft-(helpTabs.clientWidth-activeHelpTab.clientWidth)/2;const pages={
    ...
    cave:helpCard('靈脈與修行設施',['洞府靈脈提供設施運作所需供應，提升靈脈可擴充供應上限。','聚靈室提高掛機修為；洗劍池提高掛機劍元；鍛體室降低手動鍛體的材料消耗與受傷風險。','設施可啟停與升級；供應不足時無法啟用。鍛體室不會在在線或離線期間自動增加肉身進度。'])+helpCard('道童與資源生產',['道童可分配至食物、木材及隕鐵生產線；食物產線可安排等級×3.5後無條件捨去的道童，木材產線為等級×2，隕鐵產線為等級×1。','三條生產線皆可持續升級，道童配置上限會依各自倍率持續增加。','生產線等級與倉儲容量是部分煉體大境界突破條件。'])+helpCard('煉丹、煉器與儲物袋',['丹房使用神念遠遊取得的主藥與丹砂製作永久屬性丹；可製作階級取決於三路最高境界。','器室可用織天台消耗木材、靈石製作補天絲；可在目前最高境界內自由選擇較低階織法，選擇會保留，離線期間照常推進。','儲物袋初始100格、最高19階，每次升階增加50格；每格同種道具最多容納9,999個。','器室也能使用神念遠遊素材、階材與器靈精魄製作裝備。'])+helpCard('書房',['山海志只收錄親自通過的九鎖封天地域，並記載所遇生靈與地方特產；未發現項目維持封卷。','戰錄彙整九鎖封天、試劍境、肉身試煉與因緣進度，不改變任何屬性。','典故錄只供重新翻閱已完成的主線對話與結果；因緣抉擇請由因緣入口的歲月錄查看。']),
...
function openMarketPurchase(id){
  ...
  marketPurchaseOffer=offer;marketPurchaseQuantity=1;marketPurchaseRequest={key:'',id:''};
...
function closeMarketPurchase(){$('#marketPurchaseModal').classList.add('hidden');marketPurchaseOffer=null;marketPurchaseQuantity=1;marketPurchaseRequest={key:'',id:''}}
async function confirmMarketPurchase(){
  ...
  if(offer.item?.techniqueBook&&!offer.item.techniqueBook.exclusiveMarket||offer.currencyKey==='prestige'){const confirm=$('#marketPurchaseConfirm'),requestKey=`secure:${offer.id}:${quantity}`;if(marketPurchaseRequest.key!==requestKey)marketPurchaseRequest={key:requestKey,id:crypto.randomUUID()};confirm.disabled=true;confirm.textContent='購買中…';try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_secure_market_purchase',{p_channel:channel,p_offer_id:offer.id,p_quantity:quantity,p_request_id:marketPurchaseRequest.id});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverSectRevision=Number(result?.sect?.revision)||serverSectRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerSect(result?.sect||{});applyServerMailItemBalances(result?.itemBalances||{});if(offer.dailyLimit!=null){const daily=marketDailyState();daily.counts[offer.id]=Number(result.periodCount)||0}if(offer.permanentLimit!=null)state.marketPermanentPurchases[offer.id]=Number(result.periodCount)||0;if(offer.item.techniqueBook){const legacy=scriptureDailyState();if(!legacy.ids.includes(offer.id))legacy.ids.push(offer.id)}toast(result.alreadyPurchased?'購買結果已由伺服器恢復':`購得「${offer.name}」${quantity>1?` × ${quantity}`:''}`);closeMarketPurchase();renderMarket(currentMarketTab);render();save()}catch(error){confirm.disabled=false;confirm.textContent='確認購買';toast(error.message)}return}
  if(offer.currencyKey==='spiritStone'&&!offer.item?.techniqueBook){const confirm=$('#marketPurchaseConfirm'),requestKey=`stone:${offer.id}:${quantity}`;if(marketPurchaseRequest.key!==requestKey)marketPurchaseRequest={key:requestKey,id:crypto.randomUUID()};confirm.disabled=true;confirm.textContent='購買中…';try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_spirit_stone_market_purchase',{p_channel:channel,p_offer_id:offer.id,p_quantity:quantity,p_request_id:marketPurchaseRequest.id});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerMailItemBalances(result?.itemBalances||{});if(offer.dailyLimit!=null){const daily=marketDailyState();daily.counts[offer.id]=Number(result.periodCount)||0}if(offer.weeklyLimit!=null){const weekly=marketWeeklyState(),key=offer.weeklyKey||offer.id;weekly.counts[key]=Number(result.periodCount)||0}toast(result.alreadyPurchased?'購買結果已由伺服器恢復':`購得「${offer.name}」${quantity>1?` × ${quantity}`:''}`);closeMarketPurchase();renderMarket(currentMarketTab);render();save()}catch(error){confirm.disabled=false;confirm.textContent='確認購買';toast(error.message)}return}
  return toast('此商品尚未納入伺服器交易，目前無法購買');
...
$('#createBtn').onclick=async()=>{const n=$('#nameInput').value.trim(),button=$('#createBtn');if(!n){$('#nameError').textContent='請輸入暱稱';return}const now=gameNow(),candidate={...defaults,...originProfiles[createOrigin],name:n,gender:createGender,appearance:createAppearance,hair:1,outfit:createOutfit,origin:createOrigin,bornAt:now,lastSave:now,sectTechniqueMailVersion:2,updateCompensationMailVersion:2,secondUpdateCompensationMailVersion:1,firstPath:'',activePath:'',spiritPathOpened:false,tutorialCompleted:false,swordPathOpened:false,bodyPathOpened:false};candidate.mailbox=[createWelcomeMail(now)];button.disabled=true;const priorText=button.textContent;button.textContent='正在建立雲端角色…';try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';await playerStateRpc('player_new_character_initialize_v2',{p_channel:channel,p_name:n,p_origin:createOrigin,p_gender:createGender});state=candidate;startGame();save()}catch(error){$('#nameError').textContent=error.message}finally{button.disabled=false;button.textContent=priorText}};
...
function normalizeAscension(){const base=defaults.ascension,current=state.ascension&&typeof state.ascension==='object'?state.ascension:{};Object.assign(current,{...base,...current,heartAnswers:Array.isArray(current.heartAnswers)?current.heartAnswers.slice(0,3):[],delusionReplies:Array.isArray(current.delusionReplies)?current.delusionReplies.slice(0,3):[]});if(!['qi','sword','body'].includes(current.replayRoute))current.replayRoute='none';const immortalBase=base.immortalRealm,savedImmortal=current.immortalRealm&&typeof current.immortalRealm==='object'?current.immortalRealm:{};current.immortalRealm={...immortalBase,...savedImmortal,investigated:[...new Set(Array.isArray(savedImmortal.investigated)?savedImmortal.investigated:[])],expedition:{...immortalBase.expedition,...(savedImmortal.expedition&&typeof savedImmortal.expedition==='object'?savedImmortal.expedition:{})}};state.ascension=current;const a=current;if(['qi','sword','body'].includes(a.route))a.prologueCompleted=true;if(a.heartTrialCompleted&&a.pendingReward?.stage!=='heart')a.heartRewardAllocated=true;if(a.delusionTrialCompleted&&a.pendingReward?.stage!=='delusion')a.delusionRewardAllocated=true;if(a.heartRewardAllocated)a.heartTrialCompleted=true;if(a.delusionRewardAllocated){a.heartTrialCompleted=true;a.heartRewardAllocated=true;a.delusionTrialCompleted=true}if(a.roadEndRewardAllocated||a.ascended){a.heartTrialCompleted=true;a.heartRewardAllocated=true;a.delusionTrialCompleted=true;a.delusionRewardAllocated=true;a.roadEndCompleted=true;a.roadEndRewardAllocated=true;if(!a.ascendedAt)a.ascendedAt=Date.now()}return a}
const normalizeAscensionBeforeImmortalInventory=normalizeAscension;
normalizeAscension=function(){const a=normalizeAscensionBeforeImmortalInventory(),im=a.immortalRealm;if(!formalImmortalRealmEnabled)a.currentRealm='mortal';if(!im.inventoryVersion){state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(state.immortalResidualSourceCount)||0))+Math.max(0,Math.floor(Number(im.residualSource)||0));state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(state.immortalBarrenTraceCount)||0))+Math.max(0,Math.floor(Number(im.barrenTraces)||0));im.residualSource=0;im.barrenTraces=0;im.inventoryVersion=1}return a};
function ascensionEligible(){return formalImmortalRealmEnabled&&(state.mainlineCleared||0)>=18&&((state.spiritLevel||0)>=80||(state.swordLevel||0)>=80||(state.bodyLevel||0)>=bodyAscensionLevel)}
...
function ascensionRouteChoiceLocked(label){return label==='以練氣闖天路'?(state.spiritLevel||0)<80:label==='以淬劍闖天路'?(state.swordLevel||0)<80:label==='以煉體闖天路'?(state.bodyLevel||0)<bodyAscensionLevel:false}
function ascensionJourneyRoute(a=normalizeAscension()){return a.ascended&&['qi','sword','body'].includes(a.replayRoute)?a.replayRoute:a.route}
function setAscensionJourneyRoute(route,a=normalizeAscension()){if(a.ascended){lockAscensionLeaderboardRecord(a);a.replayRoute=route;if(['qi','sword','body'].includes(route))a.route=route}else a.route=route}
function restoreAscensionOfficialRoute(){const a=normalizeAscension(),record=lockAscensionLeaderboardRecord(a);if(!a.ascended||!record)return false;const changed=a.route!==record.route||a.replayRoute!=='none';a.route=record.route;a.replayRoute='none';if(changed)save();return changed}
function commitAscensionRouteChoice(label){const route={'以練氣闖天路':'qi','以淬劍闖天路':'sword','以煉體闖天路':'body','待我思考':'none'}[label];if(!route)return;const a=normalizeAscension(),replay=a.ascended;setAscensionJourneyRoute(route,a);if(!replay&&route!=='none'){a.prologueCompleted=true;ascensionRouteSyncPromise=playerAscensionRoute(route).then(()=>save()).catch(async error=>{toast(error.message);try{await playerAscensionBootstrap();renderAscensionRoad()}catch{}}).finally(()=>ascensionRouteSyncPromise=null)}save()}
document.addEventListener('click',event=>{if(event.target.closest?.('[data-story-choice]')&&normalizeAscension().ascended)lockAscensionLeaderboardRecord()},true);
...
function renderAscensionEntrances(){const a=normalizeAscension(),eligible=ascensionEligible(),realmButton=$('#realmSwitchButton'),realmImage=realmButton?.querySelector('img');$('#ascensionButton').classList.toggle('hidden',!eligible);realmButton.classList.toggle('hidden',!formalImmortalRealmEnabled||!a.ascended);const immortal=formalImmortalRealmEnabled&&a.currentRealm==='immortal'&&a.ascended,$screen=$('#gameScreen'),$scene=$('.scene-bg'),mortalNav=$('.mortal-bottom-nav'),immortalNav=$('.immortal-bottom-nav');$screen.classList.toggle('immortal-realm',immortal);mortalNav?.classList.toggle('hidden',immortal);immortalNav?.classList.toggle('hidden',!immortal);$$('.path-action-group').forEach(group=>group.classList.toggle('realm-hidden',immortal));if(!immortal&&currentFeature?.startsWith('immortal-')){currentFeature=null;$('#featurePanel').classList.add('hidden');$screen.classList.remove('feature-open');$$('.feature-tab').forEach(tab=>tab.classList.remove('active'));renderPrimarySanctum()}if(immortal){$scene.src='assets/qstyle-v2/ascension/immortal-realm-barren-v1.png';$scene.alt='靈氣枯竭的荒蕪仙界';realmImage.src='assets/qstyle-v2/ascension/entry-mortal-v2.png';realmImage.alt='凡間';realmButton.setAttribute('aria-label','返回凡間');$('#headerSpiritRealm').title='仙界沒有靈氣；凡間修練與產能照常運作'}else{const path=state.activePath||state.firstPath||a.lastMortalTrainingGround||'spirit';a.lastMortalTrainingGround=path;$scene.src=cultivationScene(path);$scene.alt=`${cultivationPathMeta[path]?.name||'練氣'}修練道場`;realmImage.src='assets/qstyle-v2/ascension/entry-immortal-v1.png';realmImage.alt='仙界';realmButton.setAttribute('aria-label','前往仙界')}}
...
function switchWorldRealm(){
  const a=normalizeAscension();if(!formalImmortalRealmEnabled)return toast('仙界尚未於正式版開放');if(!a.ascended||realmSwitching)return;
...
renderAscensionAllocation=function(reward){const a=normalizeAscension(),replay=a.ascended;if(!replay&&reward.stage==='heart'&&serverAscensionPendingStage!=='heart'){$('#ascensionContent').innerHTML='<p class="leaderboard-loading">正在請伺服器記錄問心結果……</p>';playerAscensionHeart(a.heartAnswers).then(()=>{save();renderAscensionAllocation(a.pendingReward)}).catch(error=>{toast(error.message);playerAscensionBootstrap().then(renderAscensionRoad).catch(()=>{})});return}renderAscensionAllocationBase(replay?{...reward,points:0,replay:true}:reward);if(replay)return;const c=$('#ascensionContent'),complete=c.querySelector('[data-allocation-complete]');if(!complete)return;complete.onclick=async()=>{if(complete.disabled)return;const allocation={};c.querySelectorAll('[data-allocate]').forEach(input=>allocation[input.dataset.allocate]=Math.max(0,Math.floor(Number(input.value)||0)));complete.disabled=true;complete.textContent='伺服器結算中…';try{await playerAscensionAllocate(reward.stage,allocation);save();render();reward.stage==='roadEnd'?renderAscensionSuccess(false):renderAscensionRoad()}catch(error){toast(error.message);complete.disabled=false;complete.textContent=reward.stage==='roadEnd'?'踏入仙門':'完成分配'}}};
...
const startAscensionBattleLocal=startAscensionBattle;
startAscensionBattle=async function(mode){const a=normalizeAscension();if(a.ascended)return startAscensionBattleLocal(mode);try{await window.syncSwordTrialProfile?.();const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',stage=mode==='ascension-road-end'?'roadEnd':'delusion',ticket=await playerStateRpc('player_ascension_battle_begin',{p_channel:channel,p_stage:stage,p_request_id:crypto.randomUUID()});applyServerAscension(ticket.ascension||{});startAscensionBattleLocal(mode);if(battle){battle.ascensionAttemptId=ticket.attemptId;battle.enemy.combatPower=Number(ticket.enemyPower)||battle.enemy.combatPower}}catch(error){toast(error.message);ensureAscensionModal().classList.add('show');renderAscensionRoad()}};
async function startImmortalBattle(source){ensureAscensionModal().classList.remove('show');clearTimeout(battleTimer);let start;try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';start=await playerStateRpc('player_immortal_battle_start',{p_channel:channel,p_source:source,p_request_id:crypto.randomUUID()});applyServerImmortal(start.immortal||{})}catch(error){toast(error.message);return}const player=immortalEffectiveStats(),effectivePower=immortalEffectiveCombatPower(),enemy={combatPower:Math.round(effectivePower*.55),core:{rootBone:1,trueQi:1,physique:1,agility:1,spiritualPower:1},maxHp:Math.max(120,Math.round(player.maxHp*.58)),attack:Math.max(12,Math.round(player.attack*.48)),defense:Math.max(0,Math.round(player.defense*.42)),evasion:Math.round(player.evasion*.55),accuracy:Math.round(player.accuracy*.62),crit:.03,name:'枯靈殘兵',race:'human',combatStyle:'attack-defense'};startBgm('battle');battle={active:true,resolved:false,mode:'immortal-sentinel',immortalSource:source,immortalAttemptId:start.attemptId,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className='battle-silhouette silhouette-human';$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent=enemy.name;$('.battle-arena').style.backgroundImage="linear-gradient(#161b19aa,#4c44366b),url('assets/qstyle-v2/ascension/immortal-realm-barren-v1.png')";$('#battleLog').innerHTML=`<p><b>仙界壓制生效</b>・實效戰力 ${formatCombatPower(effectivePower)}・攻擊 ${Math.round(immortalSuppression().attack*100)}%／防禦 ${Math.round(immortalSuppression().defense*100)}%／氣血 ${Math.round(immortalSuppression().hp*100)}%</p><p>枯靈殘兵已依目前實效戰力調整，這是對仙界規則的初次適應。</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700)}
...
const completeAscensionBattleLocal=completeAscensionBattle;
completeAscensionBattle=function(mode,won){if(normalizeAscension().ascended)return completeAscensionBattleLocal(mode,won);save();if(!won){ensureAscensionModal().classList.remove('show');resumeWorldBgm()}};
...
document.addEventListener('click',event=>{if(!event.target.closest?.('.ascension-close,[data-enter-immortal]'))return;const modal=$('#ascensionModal');if(modal&&!modal.classList.contains('show'))restoreAscensionOfficialRoute()});
...
setInterval(async()=>{if($('#gameScreen').classList.contains('active')){const settlement=await claimServerSettlementTicks(),ticks=settlement.ticks;if(!ticks)return;let sectChanged=false;if(state.cultivationAwakened){const immortal=normalizeAscension().currentRealm==='immortal',gains=applyServerResourceRewards(settlement.rewards);await playerSectProgress();if(state.swordPathOpened&&!immortal&&state.activePath==='sword')renderPrimarySanctum();processBodyTrainingCycles();sectChanged=processSectYears();processEncounterTriggers(5000*ticks);if(!immortal&&state.activePath==='sword'&&gains.sword>0n&&isPureCultivationView())toast(`劍元+${formatLargeNumber(gains.sword)}`,'cultivation')}processDivineRoaming();if(currentFeature==='root')renderSpiritRootView(currentRootView);if(currentFeature==='cave'&&state.cultivationAwakened&&currentCaveView!=='study')renderCaveView(currentCaveView);if(currentFeature==='sect'&&currentSectView!=='npcs'&&sectChanged)renderSectView(currentSectView);if(currentFeature==='arts')updateArtsLive();if(currentFeature==='spiritPrimary'||currentFeature==='experience'&&currentExperienceView==='spiritSide')renderQiDestination();else if(currentFeature==='experience')renderSwordPathSummary();render();save();tickStart=gameNow()}},5000);
