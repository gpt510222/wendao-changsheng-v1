...
async function playerCombatLoadoutBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';applyServerWeaving(await playerStateRpc('player_weaving_bootstrap',{p_channel:channel,p_legacy_bag_rank:state.bagRank||1,p_legacy_mending_silk:state.mendingSilk||0}));return applyServerCombatLoadout(await playerStateRpc('player_combat_loadout_bootstrap',{p_channel:channel,p_legacy_embryo:state.swordEmbryo||'',p_legacy_moves:state.swordMoves||[]}))}
...
function commitAscensionRouteChoice(label){const route={'以練氣闖天路':'qi','以淬劍闖天路':'sword','以煉體闖天路':'body','待我思考':'none'}[label];if(!route)return;const a=normalizeAscension();setAscensionJourneyRoute(route,a);if(!a.ascended&&route!=='none')a.prologueCompleted=true;save()}
