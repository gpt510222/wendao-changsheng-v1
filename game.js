...
function healBodyInjury(){const id=activeBodyInjury();if(!id)return toast('目前沒有傷勢');const costs=bodyHealCost(id),gain=bodyInjuries[id].severity*(bodyPassiveUnlocked(15)?2:1);if(state.food<costs.food||state.wood<costs.wood)return toast('療傷所需食物或木材不足');state.food-=costs.food;state.wood-=costs.wood;state.bodyInjury='';state.bodyInjuryUntil=0;state.bodyBreakthroughValue+=gain;toast(`傷勢已痊癒・破立值 +${gain}`);renderBodyDestination('body');render();save()}
...
async function upgrade(type) {
  ...
  if(spirit||sword){
    ...
    if(state.bodyBreakthroughValue>0){const bonus=Math.min(12,state.bodyBreakthroughValue);applyAttributeGain({rootBone:bonus,physique:bonus});state.bodyBreakthroughValue=0}
