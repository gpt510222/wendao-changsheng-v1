...
async function connectServerPlayerState(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';const record=await playerStateRpc('player_state_bootstrap',{p_channel:channel,p_legacy_state:recoverySaveData()});serverPlayerStateRevision=Number(record?.revision)||0;if(!serverPlayerStateRevision)throw new Error('伺服器角色狀態未建立');const wallet=await playerStateRpc('player_resource_wallet_bootstrap',{p_channel:channel,p_resources:resourceWalletMigrationPayload()});serverResourceWalletRevision=Number(wallet?.revision)||0;if(!serverResourceWalletRevision)throw new Error('伺服器資源帳本未建立');await playerStateRpc('player_resource_profile_bootstrap',{p_channel:channel,p_profile:resourceEarningProfile()});const cave=await playerStateRpc('player_cave_bootstrap',{p_channel:channel,p_cave:caveMigrationPayload()});serverCaveRevision=Number(cave?.revision)||0;if(!serverCaveRevision)throw new Error('伺服器洞府狀態未建立');return playerStateRpc('offline_reward_prepare',{p_channel:channel})}
async function playerCaveCommand(action,key=null){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_cave_command',{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_cave_revision:serverCaveRevision,p_request_id:crypto.randomUUID(),p_action:action,p_key:key});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverCaveRevision=Number(result?.cave?.revision)||serverCaveRevision;const cave=result?.cave||{};for(const key of ['daoChildTotal','daoChildBought','workerFood','workerWood','workerMeteorIron','foodAreaLevel','woodAreaLevel','meteorIronAreaLevel','spiritPoolLevel'])if(cave[key]!=null)state[key]=Number(cave[key]);return result}
