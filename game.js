...
normalizeAscension=function(){const a=normalizeAscensionBeforeImmortalInventory(),im=a.immortalRealm;if(!im.inventoryVersion){state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(state.immortalResidualSourceCount)||0))+Math.max(0,Math.floor(Number(im.residualSource)||0));state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(state.immortalBarrenTraceCount)||0))+Math.max(0,Math.floor(Number(im.barrenTraces)||0));im.residualSource=0;im.barrenTraces=0;im.inventoryVersion=1}return a};
function ascensionEligible(){return (state.mainlineCleared||0)>=18&&((state.spiritLevel||0)>=80||(state.swordLevel||0)>=80||(state.bodyLevel||0)>=bodyAscensionLevel)}
