...
function applyServerPermanentConsumables(effects={}){state.dosageLimitBonus=Math.max(0,Math.floor(Number(effects.dosageBonus)||0));if(effects.mindEmbodiment)state.mindEmbodimentUnlocked=true;const learned=new Set(Array.isArray(effects.learnedBooks)?effects.learnedBooks:[]);state.serverPermanentLearnedBooks=[...learned];for(const id of learned){const book=itemCatalog[id]?.techniqueBook;if(!book)continue;if(!state.learnedBookIds.includes(id))state.learnedBookIds.push(id);if(!state.learnedArts.some(art=>art.id===id))state.learnedArts.push({...book,level:1})}const previous=state.serverPermanentAttributeBonuses&&typeof state.serverPermanentAttributeBonuses==='object'?state.serverPermanentAttributeBonuses:{},bonuses=effects.attributeBonuses&&typeof effects.attributeBonuses==='object'?effects.attributeBonuses:{};for(const key of ['trueQi','rootBone','physique','agility']){const next=Math.max(0,Number(bonuses[key])||0),before=Math.max(0,Number(previous[key])||0);state[key]=Math.max(0,(Number(state[key])||0)+next-before)}state.serverPermanentAttributeBonuses={...bonuses};state.pillUsage={...(state.pillUsage||{}),...(effects.pillUsage||{})};state.brewUsage={...(state.brewUsage||{}),...(effects.brewUsage||{})};return effects}
...
async function useTechniqueBook(key){
  ...
  if(book.exclusiveMarket==='treasure'){try{await useServerPermanentConsumable(item,1)}catch(error){toast(error.message);return false}}else{state[item.count]--;state.learnedBookIds.push(book.id);state.learnedArts.push({...book,level:1})}
