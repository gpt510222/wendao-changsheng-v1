...
function startBodyTrial(){
  refreshBodyState();const nextLevel=state.bodyLevel+1,nextIsRealm=bodyTrialRequired(nextLevel),requirement=bodyBreakthroughRequirement(nextLevel);if(!nextIsRealm||!bodyFoundationsReady()||!bodyBreakthroughMaterialsReady(requirement))return toast('根基、產線等級或突破物資尚未備妥');const autoEquipped=ensureBodyTrialTechnique();clearTimeout(battleTimer);startBgm('bodyTrial');const player=bodyTrialPlayerStats(),targetRounds=5+bodyTrialLevels.indexOf(nextLevel),enemy=bodyTrialEnemyStats(player,targetRounds),failures=state.bodyTrialFailures[String(nextLevel)]||0,guaranteed=failures>=2&&!activeBodyInjury();
...
async function finishBattle(won,reason){
  ...
  else if(battle.mode==='bodyTrial'){const trialLevel=battle.bodyTrialLevel||state.bodyLevel+1,key=String(trialLevel);if(won){if(trialLevel===state.bodyLevel+1&&bodyFoundationsReady()&&bodyBreakthroughMaterialsReady(battle.bodyRequirement)){consumeBodyBreakthroughMaterials(battle.bodyRequirement);state.bodyLevel++;resetBodyFoundations();state.bodyTrialFailures[key]=0;applyAttributeGain(bodyAttributeGain(state.bodyLevel));syncSectTaskRouteUnlocks('body');reward=` 突破物資已投入，肉身突破至${bodyRealmName(state.bodyLevel)}。`}else reward=' 試煉資格已失效，未完成突破。'}else{state.bodyTrialFailures[key]=(state.bodyTrialFailures[key]||0)+1;const injury=Math.random()<.6?'internal':'tendon';inflictBodyInjury(injury);reward=` 突破物資完整保留；留下${bodyInjuries[injury].name}。此境已失敗 ${state.bodyTrialFailures[key]} 次。`}}
