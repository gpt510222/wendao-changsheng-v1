...
async function serverSectArtCommand(action,art){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sect_art_command',{p_channel:channel,p_action:action,p_sect:art.sourceSect,p_slot:art.slot,p_request_id:crypto.randomUUID()});applyServerSectArts(result?.arts||{});if(result?.wallet){serverResourceWalletRevision=Number(result.wallet.revision)||serverResourceWalletRevision;applyServerWalletSnapshot(result.wallet.resources)}save();return result}
