...
async function loadOwnLeaderboardRecord(session){const query=`user_id=eq.${encodeURIComponent(session.user.id)}&game_version=like.${encodeURIComponent(`${leaderboardConfig.gameVersion}*`)}&select=combat_power,player_name,game_version&limit=1`;const response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders(session.access_token)});if(!response.ok)throw new Error('ranking lookup failed');const [record]=await response.json(),ascensionRecord=parseAscensionLeaderboardRecord(record),immortalRecord=parseImmortalLeaderboardRecord(record),a=normalizeAscension();if(ascensionRecord&&a.ascended&&!['qi','sword','body'].includes(a.rankedRoute)){lockAscensionLeaderboardRecord(a,ascensionRecord);save()}leaderboardKnownPower=record?Number(record.combat_power):0;leaderboardKnownName=record?.player_name||'';leaderboardKnownAscensionKey=ascensionRecord?`${ascensionRecord.ascendedAt}:${ascensionRecord.route}`:'';leaderboardKnownImmortalKey=immortalRecord?`${immortalRecord.restorationStage}:${immortalRecord.exploration}:${immortalRecord.expeditionRuns}:${immortalRecord.outpostStage}:${immortalRecord.wastelandProgress}:${immortalRecord.bodyStage}`:''}
...
async function fetchImmortalLeaderboard(){const query=`game_version=like.${encodeURIComponent(`${leaderboardConfig.gameVersion}*`)}&select=player_name,game_version&limit=1000`,response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders()});if(!response.ok)throw new Error('immortal ranking fetch failed');return (await response.json()).map(parseImmortalLeaderboardRecord).filter(Boolean).sort((a,b)=>b.restorationStage-a.restorationStage||b.outpostStage-a.outpostStage||b.wastelandProgress-a.wastelandProgress||b.bodyStage-a.bodyStage||b.exploration-a.exploration||b.expeditionRuns-a.expeditionRuns).slice(0,leaderboardConfig.limit)}
