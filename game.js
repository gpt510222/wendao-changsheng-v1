...
async function loadOwnLeaderboardRecord(session){const query=`user_id=eq.${encodeURIComponent(session.user.id)}&game_version=like.${encodeURIComponent(`${leaderboardConfig.gameVersion}*`)}&select=combat_power,player_name,game_version&limit=1`;const response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders(session.access_token)});if(!response.ok)throw new Error('ranking lookup failed');const [record]=await response.json(),ascensionRecord=parseAscensionLeaderboardRecord(record),immortalRecord=parseImmortalLeaderboardRecord(record);leaderboardKnownPower=record?Number(record.combat_power):0;leaderboardKnownName=record?.player_name||'';leaderboardKnownAscensionKey=ascensionRecord?`${ascensionRecord.ascendedAt}:${ascensionRecord.route}`:'';leaderboardKnownImmortalKey=immortalRecord?`${immortalRecord.restorationStage}:${immortalRecord.exploration}:${immortalRecord.expeditionRuns}:${immortalRecord.outpostStage}:${immortalRecord.wastelandProgress}:${immortalRecord.bodyStage}`:''}
