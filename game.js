...
async function upgrade(type) {
  ...
  if(spirit||sword){
    ...
    try{await playerProgressionCommand('upgrade',type);applyAttributeGain(gain);toast(`已提升至${realmName(spirit?state.spiritLevel:state.swordLevel,spirit?spiritRealms:swordRealms)}`)}catch(error){toast(error.message);return}
  }
  else {
    const nextLevel=state.bodyLevel+1,gain=bodyAttributeGain(nextLevel),bonus=Math.min(12,state.bodyBreakthroughValue||0);try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_body_breakthrough',{p_channel:channel,p_expected_progression_revision:serverProgressionRevision,p_expected_body_revision:serverBodyRevision,p_request_id:crypto.randomUUID()});serverProgressionRevision=Number(result?.progression?.revision)||serverProgressionRevision;serverBodyRevision=Number(result?.body?.revision)||serverBodyRevision;applyServerProgression(result?.progression);applyServerBody(result?.body)}catch(error){toast(error.message);return}applyAttributeGain(gain);
    if(bonus>0)applyAttributeGain({rootBone:bonus,physique:bonus});
...
async function tribulate() {
  ...
  scheduleTribulation(()=>{
    ...
    if(success) {
      const gain=spiritAttributeGain(previousLevel+1);applyAttributeGain(gain);syncSectTaskRouteUnlocks('spirit');queueRealmEncounter('spirit',state.spiritLevel);
...
async function startSwordBreakthrough(cost=swordReq(state.swordLevel||0)){
  ...
  try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',result=await playerStateRpc('player_sword_breakthrough',{p_channel:channel,p_expected_wallet_revision:serverResourceWalletRevision,p_expected_progression_revision:serverProgressionRevision,p_request_id:crypto.randomUUID()});serverResourceWalletRevision=Number(result?.wallet?.revision)||serverResourceWalletRevision;serverProgressionRevision=Number(result?.progression?.revision)||serverProgressionRevision;applyServerWalletSnapshot(result?.wallet?.resources);applyServerProgression(result?.progression)}catch(error){toast(error.message);return}
...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();await playerIdentityBootstrap();await playerMoralBootstrap();await playerEncounterSync();sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await playerSectShopStatus();await syncJadeGrants({notify:false})}catch(error){sessionOnline=false;show('#titleScreen');$('#titleHint').textContent='需要連線伺服器才能進入遊戲';toast(error.message);return}
...
async function finishBattle(won,reason){
  ...
  else if(battle.mode==='bodyTrial'){const trialLevel=battle.bodyTrialLevel||state.bodyLevel;if(won){applyAttributeGain(bodyAttributeGain(state.bodyLevel));syncSectTaskRouteUnlocks('body');reward=` 肉身突破至${bodyRealmName(state.bodyLevel)}。`}else{const injury=activeBodyInjury(),failures=state.bodyTrialFailures[String(trialLevel)]||0;reward=` 突破物資完整保留${injury?`；留下${bodyInjuries[injury].name}`:''}。此境已失敗 ${failures} 次。`}}
