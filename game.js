...
function prepareBodyNutrition(){const missing=Math.max(0,bodyNutritionCapacity()-(state.bodyNutrition||0)),amount=Math.min(missing,Math.max(0,Math.floor(state.food||0)));if(amount<1)return toast(missing<1?'腹中精氣已滿':'食物不足');state.food-=amount;state.bodyNutrition=(state.bodyNutrition||0)+amount;toast(`進食調養・腹中精氣 +${formatLargeNumber(amount)}`);renderBodyDestination('training');render();save()}
function stopBodyTraining(message='已停止持續鍛體'){state.bodyTrainingMode='';state.bodyTrainingNextAt=0;if(message)toast(message);save()}
function startBodyTraining(kind){refreshBodyState();const option=bodyTrainingOptions()[kind];if(!option||!state.bodyPathOpened)return;if(bodyFoundationsReady())return toast('三項根基已足，請先完成突破');if(kind==='extreme'&&activeBodyInjury()==='tendon')return toast('筋傷未癒，無法進行極境熬身');if(!bodyTrainingCanPay(option))return toast('腹中精氣或鍛體材料不足');state.bodyTrainingMode=kind;state.bodyTrainingNextAt=gameNow()+option.minutes*60000;toast(`${option.name}已開始・離線期間照常進行`);renderBodyDestination('training');save()}
function processBodyTrainingCycles(now=gameNow()){let completed=0;while(state.bodyTrainingMode&&state.bodyTrainingNextAt&&now>=state.bodyTrainingNextAt&&completed<200&&!bodyFoundationsReady()){const kind=state.bodyTrainingMode,option=bodyTrainingOptions()[kind];if(!option||kind==='extreme'&&activeBodyInjury()==='tendon'||!bodyTrainingCanPay(option)){stopBodyTraining('');break}state.bodyNutrition-=option.nutrition;state.wood-=option.wood;state.meteorIron-=option.iron;for(const [root,gain] of Object.entries(option.roots))state.bodyFoundations[root]=(state.bodyFoundations[root]||0)+gain*partnerRouteMultiplier('body')*auraInfusionMultiplier('body',state.bodyTrainingNextAt);state.bodyTrainingLoad=Math.max(0,Math.min(100,(state.bodyTrainingLoad||0)+option.load));if(option.risk&&Math.random()*100<option.risk){const roll=Math.random();inflictBodyInjury(roll<.5?'scratch':roll<.82?'internal':'tendon')}state.bodyTrainingNextAt+=option.minutes*60000;completed++}if(bodyFoundationsReady())stopBodyTraining('根基已成・可進行突破');if(completed){syncBodyTemperFromFoundations();save()}return completed}
