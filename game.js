...
function applyServerEncounter(result={}){const encounter=result.encounter||result;serverEncounterRevision=Number(encounter.revision)||serverEncounterRevision;const partners=(state.encounterQueue||[]).filter(event=>event?.kind==='partner'),partnerHistory=(state.encounterHistory||[]).filter(entry=>(entry?.tags||[]).includes('partner'));state.encounterQueue=[...partners,...(Array.isArray(encounter.queue)?encounter.queue:[])].slice(0,12);state.encounterHistory=[...partnerHistory,...(Array.isArray(encounter.history)?encounter.history:[])].slice(0,60);return result}
...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();await playerIdentityBootstrap();await playerAscensionBootstrap();await playerTitleProfile();await playerImmortalBootstrap();await playerCoreAttributesGet();await playerMoralBootstrap();await playerEncounterSync();sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await playerSectShopStatus();await syncJadeGrants({notify:false})}catch(error){sessionOnline=false;show('#titleScreen');$('#titleHint').textContent='需要連線伺服器才能進入遊戲';toast(error.message);return}
