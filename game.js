...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();await playerIdentityBootstrap();await playerAscensionBootstrap();await playerCoreAttributesGet();await playerMoralBootstrap();await playerEncounterSync();sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await playerSectShopStatus();await syncJadeGrants({notify:false})}catch(error){sessionOnline=false;show('#titleScreen');$('#titleHint').textContent='需要連線伺服器才能進入遊戲';toast(error.message);return}
...
function upgradeWastelandBody(){const im=immortalState(),w=im.wasteland,cost=[[4,2],[8,4],[14,7]][w.bodyStage];if(!cost)return;if(im.residualSource<cost[0]||im.barrenTraces<cost[1])return toast('淬鍊界軀所需材料不足');im.residualSource-=cost[0];im.barrenTraces-=cost[1];w.bodyStage++;save();playImmortalStory({chapter:'仙途・界軀初成',title:`界軀第 ${w.bodyStage} 階`,lines:[ascensionLine('旁白','殘界源質滲入經脈，荒痕則像細小刻刀，將不屬於仙界的部分一寸寸改寫。','none'),ascensionLine('天路之主','不是捨棄凡身，而是讓凡間走出的路，學會承受另一個世界。','master'),ascensionLine('系統',`界軀提升至第 ${w.bodyStage} 階・界應 +5。`,'none')],onDone:renderImmortalPath})}
...
function runWastelandRoute(){if(!requireTrustedTime())return;const im=immortalState(),w=im.wasteland,daily=immortalExpeditionDaily(),id=wastelandRouteUnlocked(w.selectedRoute)?w.selectedRoute:'dryWind',route=wastelandRoutes[id];if(!daily.remaining)return toast('今日遠征次數已盡');im.expeditionDaily.used++;w.routeProgress[id]=Math.min(3,(w.routeProgress[id]||0)+1);if(id==='dryWind'){im.barrenTraces+=2;im.residualSource++}else if(id==='sunken')im.residualSource+=4;else{im.barrenTraces++;im.residualSource+=2}const first=w.routeProgress[id]===1,complete=w.routeProgress[id]===3;if(id==='kneeling'&&first)w.storyFlags.kneelingAwoke=true;save();const lines=[ascensionLine('旁白',id==='dryWind'?'谷中沒有風，岩壁上的刮痕卻一夜比一夜更深。你沿痕跡找到一段凝固的世界傷口。':id==='sunken'?'坑底埋著倒置的宮闕。屋瓦朝向地心，彷彿整座城曾被某股力量從天空按落。':'每走過一尊石像，它便在你身後抬起頭，目光始終越過你，望向同一處虛空裂痕。','none'),ascensionLine('系統',`取得${route.reward}・路線進度 ${w.routeProgress[id]} / 3。`,'none')];if(id==='kneeling'&&first)lines.splice(1,0,ascensionLine('天路之主','它們不是死物。它們只是把最後一個念頭留在石頭裡：不要讓歸來者找到無聲城。','master'));if(complete)lines.push(ascensionLine('天路之主',id==='dryWind'?'這條路已被你記住。沉界坑正在遠處顯形。':id==='sunken'?'塌陷深處仍有規則流動；它可以成為界軀的材料。':'座標已完整，但無聲城仍被更古老的封鎖遮蔽。','master'));playImmortalStory({chapter:`遠征・${route.name}`,title:complete?'路標已立':'荒原留痕',lines,onDone:renderWastelandExpedition})}
