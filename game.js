...
function upgradeCaveArea(key,amount=1){const a=caveAreas[key];if(!a)return;const target=amount==='max'?10000:Math.max(1,Math.floor(Number(amount)||1));let raised=0;while(raised<target){const cost=areaUpgradeCost(a);if(state.wood<cost.wood||state.food<cost.food)break;state.wood-=cost.wood;state.food-=cost.food;state[a.level]++;raised++}if(!raised)return toast('擴建所需資源不足');toast(`${a.label}區域提升 ${raised} 級・目前 ${state[a.level]} 級`);renderCaveView('production');render();save()}
