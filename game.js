...
async function claimOfflineRewards(){const vault=pendingOfflineVault,button=$('#offlineRewardClaim');if(!vault?.vault_id||button.disabled)return;button.disabled=true;button.textContent='領取中…';try{const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',receipt=await playerStateRpc('offline_reward_claim',{p_channel:channel,p_vault_id:vault.vault_id,p_request_id:crypto.randomUUID()}),rewards=receipt?.rewards||{},seconds=Math.max(0,Math.floor(Number(receipt?.elapsed_ticks)||0))*5,before=rewardSnapshot(),cultivation=toBigInt(rewards.free||0);state.free+=cultivation;state.totalEarned+=cultivation;state.swordEssence+=toBigInt(rewards.swordEssence||0);for(const key of ['aura','spiritStone','food','wood','meteorIron'])state[key]=(Number(state[key])||0)+Math.max(0,Number(rewards[key])||0);serverResourceWalletRevision=Number(receipt?.wallet_revision)||serverResourceWalletRevision;pendingOfflineVault=null;showOfflineRewards(before,seconds);button.textContent='已領取';save();render()}catch(error){button.disabled=false;button.textContent='重新領取';toast(error.message)}}
...
setInterval(async()=>{if($('#gameScreen').classList.contains('active')){const ticks=await claimServerSettlementTicks();if(!ticks)return;let sectChanged=false;if(state.cultivationAwakened){const immortal=normalizeAscension().currentRealm==='immortal';addAura(ticks*auraRate());const swordGain=state.swordPathOpened?swordEssenceRate()*ticks:0;if(state.swordPathOpened){state.swordEssence+=BigInt(swordGain);if(!immortal&&state.activePath==='sword')renderPrimarySanctum()}runSettlementTick(ticks);sectChanged=processSectYears();processEncounterTriggers(5000*ticks);if(state.spiritPathOpened)addCultivation(rate()*ticks,immortal||state.activePath!=='spirit');if(!immortal&&state.activePath==='sword'&&swordGain>0&&isPureCultivationView())toast(`劍元+${formatLargeNumber(swordGain)}`,'cultivation')}processDivineRoaming();if(currentFeature==='root')renderSpiritRootView(currentRootView);if(currentFeature==='cave'&&state.cultivationAwakened&&currentCaveView!=='study')renderCaveView(currentCaveView);if(currentFeature==='sect'&&currentSectView!=='npcs'&&sectChanged)renderSectView(currentSectView);if(currentFeature==='arts')updateArtsLive();if(currentFeature==='spiritPrimary'||currentFeature==='experience'&&currentExperienceView==='spiritSide')renderQiDestination();else if(currentFeature==='experience')renderSwordPathSummary();tickStart=gameNow()}},5000);
