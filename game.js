...
function investigateImmortalSite(id){const im=immortalState(),seen=im.investigated.includes(id),remember=()=>{if(!seen)im.investigated.push(id)};if(id==='array'){playImmortalStory({chapter:'飛升古臺・其一',title:'熄滅古陣',lines:[ascensionLine('旁白','你俯身觸碰古陣。沒有靈氣回應，陣紋深處卻滲出幾縷灰白微光。','none'),ascensionLine('天路之主','那不是靈氣，是世界崩毀後尚未散盡的「殘界源質」。在仙界，修復與前行都要靠它。','master'),ascensionLine(state.name||'修士','原來仙界不是沒有力量，只是力量早已換了形態。','player')],onDone:()=>{remember();if(!seen){im.residualSource+=3;toast('取得殘界源質 ×3')}renderImmortalDomain()}})}else if(id==='stele'){playImmortalStory({chapter:'飛升古臺・其二',title:'無字殘碑',lines:[ascensionLine('旁白','碑上無字。你繞到背面，才看見裂痕並非風化，而是從仙界內部向外斬出。','none'),ascensionLine('天路之主','仙界的枯竭不是天災。有人斬斷了諸域之間的循環，也把通往虛空的路一併藏了起來。','master'),ascensionLine(state.name||'修士','所以復甦仙界與破碎虛空，其實是同一條路。','player')],onDone:()=>{remember();renderImmortalDomain()}})}else{if(!im.investigated.includes('array')||!im.investigated.includes('stele'))return toast('先讀完古陣與殘碑留下的線索');if(seen)return playImmortalStory({chapter:'飛升古臺・戰痕',title:'傾塌界門',lines:[ascensionLine('旁白','枯靈殘兵已散，只留下一道由內向外延伸的足跡。古臺外圍的斷路仍在等待你。','none')],onDone:renderImmortalDomain});playImmortalStory({chapter:'飛升古臺・其三',title:'傾塌界門',lines:[ascensionLine('旁白','你踏近界門，沉在石縫裡的甲片同時震動。乾枯人影從沙塵裡站起。','none'),ascensionLine('天路之主','凡間之力在此會被天地壓制。別用舊有戰力估量這一戰——先學會在沒有靈氣的地方活下來。','master'),ascensionLine('系統','進入戰鬥：枯靈殘兵','none')],onDone:()=>startImmortalBattle('domain')})}}
...
function startWastelandRestoration(){if(!requireTrustedTime())return;const im=immortalState(),w=im.wasteland,item=wastelandOutpostStages[w.outpostStage];if(!item||wastelandRestorationJob())return;if(im.residualSource<item.cost[0]||im.barrenTraces<item.cost[1])return toast('荒原據點材料不足');im.residualSource-=item.cost[0];im.barrenTraces-=item.cost[1];w.outpostJob={targetStage:w.outpostStage+1,startedAt:gameNow(),completesAt:gameNow()+wastelandRestorationDurations[w.outpostStage]};save();renderWastelandRestoration();toast(`${item.name}工程已開始`)}
function claimWastelandRestoration(){if(!requireTrustedTime())return;const w=wastelandState(),job=wastelandRestorationJob();if(!job||gameNow()<job.completesAt)return;w.outpostStage=job.targetStage;w.outpostJob=null;const item=wastelandOutpostStages[w.outpostStage-1];save();playImmortalStory({chapter:'復界・無息荒原',title:`${item.name}重立`,lines:[ascensionLine('旁白',w.outpostStage===1?'第一滴灰白源質從井壁凝出時，附近跪像同時抬起了半寸。':'殘破的據點再次亮起微光，荒原終於有了一處可以辨認的方向。','none'),ascensionLine('天路之主',w.outpostStage===1?'它們聽見的不是水聲，而是世界重新流動的聲音。跪像原的路已經出現。':'每修回一處，仙界便少一分死寂；但虛空也會更容易看見你。','master'),ascensionLine('系統',`${item.name}完成・界應 +2。`,'none')],onDone:renderWastelandRestoration})}
function startGuideStoneRestoration(){if(!requireTrustedTime())return;const im=immortalState(),stage=im.guideStoneStage;if(immortalRestorationJob())return toast('目前已有復界工程進行中');if(stage===0&&im.residualSource<2)return toast('殘界源質不足');if(stage===1&&(im.barrenTraces<1||state.meteorIron<300))return toast('需要荒痕 ×1、隕鐵 ×300');if(stage===2&&(im.residualSource<4||im.barrenTraces<2||state.wood<500))return toast('需要殘界源質 ×4、荒痕 ×2、木材 ×500');if(stage===0)im.residualSource-=2;else if(stage===1){im.barrenTraces--;state.meteorIron-=300}else{im.residualSource-=4;im.barrenTraces-=2;state.wood-=500}im.restorationJob={targetStage:stage+1,startedAt:gameNow(),completesAt:gameNow()+immortalRestorationDurations[stage]};save();renderImmortalRestoration();toast(`引道碑第 ${stage+1} 階工程已開始`)}
function claimGuideStoneRestoration(){if(!requireTrustedTime())return;const im=immortalState(),job=immortalRestorationJob();if(!job)return;if(gameNow()<job.completesAt)return toast(`工程尚需 ${formatDuration(job.completesAt-gameNow())}`);const stage=im.guideStoneStage;im.guideStoneStage=job.targetStage;im.restorationJob=null;const scripts=[['碑基之下','你剝去碑基上的死灰，發現石中埋著與天路同源的古紋。','這座碑不是路標。它曾把新來者的存在，寫入仙界法則。'],['斷紋重接','凡間隕鐵一觸碑身便迅速黯淡，卻替殘紋搭起短暫的橋。','凡物在仙界會衰敗，但並非毫無作用。你的舊路，仍能成為新路的材料。'],['引道微明','最後一道殘紋亮起。無靈荒野沒有風，遠方沙塵卻第一次向兩側分開。','碑接受了你的存在。界應初生，而路的盡頭指向一片名為「無息荒原」的死地。']][stage];playImmortalStory({chapter:`復界・第 ${stage+1} 階`,title:scripts[0],lines:[ascensionLine('旁白',scripts[1],'none'),ascensionLine('天路之主',scripts[2],'master'),...(stage===2?[ascensionLine('系統','取得界應 10。仙界戰力壓制略微緩和。','none')]:[])],onDone:()=>{toast(stage===2?'引道碑修復完成・界應 +10':'引道碑修復進度提升');renderImmortalRestoration()}})}
...
function advanceImmortalExpedition(){const im=immortalState(),ex=im.expedition;if(!ex.active){if(!requireTrustedTime())return;const daily=immortalExpeditionDaily();if(!daily.remaining)return toast('今日三次遠征額度已用完，現實 00:00 恢復');im.expeditionDaily.used++;ex.active=true;ex.node=0;save();return playImmortalStory({chapter:'遠征・飛升古臺外圍',title:'離開古臺',lines:[ascensionLine('天路之主','遠征不是一路殺到底。記住你從哪裡來，也要知道什麼時候帶著所得回去。','master'),ascensionLine(state.name||'修士','先找出這片荒地仍剩下什麼，再決定下一步。','player')],onDone:()=>{ex.node=1;renderImmortalExpedition()}})}if(ex.node===1){im.residualSource+=3;ex.node=2;return playImmortalStory({chapter:'遠征・第二節點',title:'枯骨石林',lines:[ascensionLine('旁白','你從石化巨木的根部剝下三枚灰白晶片。它們沒有靈氣，卻讓周圍裂縫短暫閉合。','none'),ascensionLine('系統','取得殘界源質 ×3。','none')],onDone:renderImmortalExpedition})}if(ex.node===2)return playImmortalStory({chapter:'遠征・第三節點',title:'殘兵伏擊',lines:[ascensionLine('旁白','石林盡頭傳來甲片摩擦聲。另一具枯靈殘兵堵住了前路。','none'),ascensionLine('系統','進入戰鬥：枯靈殘兵','none')],onDone:()=>startImmortalBattle('expedition')});if(ex.node===3){im.residualSource++;ex.node=4;return playImmortalStory({chapter:'遠征・第四節點',title:'廢棄營地',lines:[ascensionLine('旁白','營地早已無人，只留下一句刻在石上的話：「若仙界已死，就替它決定下一次要活成什麼。」','none'),ascensionLine(state.name||'修士','前人沒能走完，但至少留下了方向。','player'),ascensionLine('系統','回收殘界源質 ×1。','none')],onDone:renderImmortalExpedition})}ex.active=false;ex.node=0;ex.runs=(ex.runs||0)+1;im.outerRouteCleared=true;playImmortalStory({chapter:'遠征・終點',title:'斷門界碑',lines:[ascensionLine('旁白','界碑背後，荒原延伸到視線之外。碑腳刻著四字：無息荒原。','none'),ascensionLine('天路之主','這只是仙界的第一寸路。修好引道碑，它會替你穿過封鎖，指出下一域真正的入口。','master')],onDone:()=>{toast('飛升古臺外圍通關・無息荒原線索已發現');renderImmortalExpedition()}})}
...
function applyServerImmortal(snapshot={}){const im=normalizeAscension().immortalRealm,w=im.wasteland||{};state.immortalResidualSourceCount=Math.max(0,Number(snapshot.residualSource)||0);state.immortalBarrenTraceCount=Math.max(0,Number(snapshot.barrenTraces)||0);im.inventoryVersion=1;im.timelineStartedAt=Math.max(0,Number(snapshot.timelineStartedAt)||gameNow());im.region=snapshot.region==='wuxi-wasteland'?'wuxi-wasteland':'ascension-platform';im.investigated=Array.isArray(snapshot.investigated)?snapshot.investigated:[];im.firstBattleWon=!!snapshot.firstBattleWon;im.guideStoneStage=Math.max(0,Math.min(3,Number(snapshot.guideStoneStage)||0));im.expeditionUnlocked=!!snapshot.expeditionUnlocked;im.expedition={active:!!snapshot.expedition?.active,node:Math.max(0,Number(snapshot.expedition?.node)||0),runs:Math.max(0,Number(snapshot.expedition?.runs)||0)};im.outerRouteCleared=!!snapshot.outerRouteCleared;im.expeditionDaily={date:snapshot.expeditionDaily?.date||'',used:Math.max(0,Number(snapshot.expeditionDaily?.used)||0)};im.storyFlags=snapshot.storyFlags&&typeof snapshot.storyFlags==='object'?snapshot.storyFlags:{};im.platformChapterCompleted=!!snapshot.platformChapterCompleted;im.wasteland={...w,...(snapshot.wasteland||{}),routeProgress:{dryWind:0,sunken:0,kneeling:0,...(snapshot.wasteland?.routeProgress||{})},storyFlags:snapshot.wasteland?.storyFlags&&typeof snapshot.wasteland.storyFlags==='object'?snapshot.wasteland.storyFlags:{}};return snapshot}
...
async function finishBattle(won,reason){
  ...
  else if(battle.mode==='immortal-sentinel'){const im=immortalState();if(won&&battle.immortalSource==='domain'){if(!im.firstBattleWon){im.firstBattleWon=true;im.expeditionUnlocked=true;im.barrenTraces+=2;if(!im.investigated.includes('gate'))im.investigated.push('gate');reward=' 荒痕+2；飛升古臺外圍遠征已開啟。'}else reward=' 你再次擊散了古臺殘留的枯靈。'}else if(won){im.barrenTraces++;im.expedition.node=3;reward=' 荒痕+1；遠征前路已清除。'}else reward=' 凡間之力在仙界受到壓制；本次沒有損失，可再次挑戰。'}
...
function startImmortalBattle(source){ensureAscensionModal().classList.remove('show');clearTimeout(battleTimer);const player=immortalEffectiveStats(),effectivePower=immortalEffectiveCombatPower(),enemy={combatPower:Math.round(effectivePower*.55),core:{rootBone:1,trueQi:1,physique:1,agility:1,spiritualPower:1},maxHp:Math.max(120,Math.round(player.maxHp*.58)),attack:Math.max(12,Math.round(player.attack*.48)),defense:Math.max(0,Math.round(player.defense*.42)),evasion:Math.round(player.evasion*.55),accuracy:Math.round(player.accuracy*.62),crit:.03,name:'枯靈殘兵',race:'human',combatStyle:'attack-defense'};startBgm('battle');battle={active:true,resolved:false,mode:'immortal-sentinel',immortalSource:source,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className='battle-silhouette silhouette-human';$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent=enemy.name;$('.battle-arena').style.backgroundImage="linear-gradient(#161b19aa,#4c44366b),url('assets/qstyle-v2/ascension/immortal-realm-barren-v1.png')";$('#battleLog').innerHTML=`<p><b>仙界壓制生效</b>・實效戰力 ${formatCombatPower(effectivePower)}・攻擊 ${Math.round(immortalSuppression().attack*100)}%／防禦 ${Math.round(immortalSuppression().defense*100)}%／氣血 ${Math.round(immortalSuppression().hp*100)}%</p><p>枯靈殘兵已依目前實效戰力調整，這是對仙界規則的初次適應。</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700)}
