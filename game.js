...
function queueImmortalMilestoneStory(){if(immortalMilestoneStoryCooling||immortalArrivalPending||battle?.active)return;window.setTimeout(()=>{if(immortalMilestoneStoryCooling||currentFeature?.startsWith('immortal-')!==true||$('#ascensionModal')?.classList.contains('show'))return;const im=immortalState(),story=immortalMilestoneStories().find(entry=>entry.ready&&!im.storyFlags[entry.id]);if(!story)return;immortalMilestoneStoryCooling=true;im.storyFlags[story.id]=true;if(story.complete)im.platformChapterCompleted=true;save();playImmortalStory({chapter:story.chapter,title:story.title,lines:story.lines,onDone:()=>{if(story.complete)toast('飛升古臺篇完成・無息荒原等待開放');window.setTimeout(()=>immortalMilestoneStoryCooling=false,800)}})},260)}
...
function setImmortalRegion(region){const im=immortalState();if(region==='wuxi-wasteland'&&!wastelandUnlocked())return toast('完成古臺碑錄 7/7、引道碑與外圍遠征後開放');im.region=region;save();renderAscensionEntrances();if(currentFeature==='immortal-domain'){renderImmortalDomain();if(region==='ascension-platform')renderImmortalChronicle()}else renderImmortalFeature(currentFeature);if(region==='wuxi-wasteland'&&!im.wasteland.arrivalSeen){im.wasteland.arrivalSeen=true;playImmortalStory({chapter:'無息荒原・序章',title:'風停在這裡',lines:[ascensionLine('旁白','引道碑照出的灰青道路在身後熄滅。前方沒有風，萬千跪像卻都朝著同一道虛空裂痕低首。','none'),ascensionLine(state.name||'修士','它們不是在向我跪。'),ascensionLine('天路之主','它們跪的是曾從虛空回來的東西。先在荒原立足，再決定要不要循著它們的目光走下去。','master'),ascensionLine('系統','第二仙域「無息荒原」開放。枯風谷探索路線已發現。','none')]})}}
