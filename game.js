...
function applyServerCombatLoadout(snapshot={}){state.swordEmbryo=['heavy','spirit','shadow'].includes(snapshot.swordEmbryo)?snapshot.swordEmbryo:'';state.swordMoves=(Array.isArray(snapshot.moves)?snapshot.moves:[]).map(String).filter(id=>combatTechniqueById(id)).slice(0,2);return snapshot}
...
async function chooseSwordEmbryo(id){
  ...
  try{await serverSwordEmbryoSet(id,false);if(!state.swordName)state.swordName=`${state.name||''}之劍`.slice(0,12)||'無名靈劍';toast(`已凝聚${embryo.name}`);renderSwordDestination('sword');render();save()}catch(error){toast(error.message)}
}
...
function nurtureSword(){
  ...
  state.meteorIron-=cost.iron;state.wood-=cost.wood;state.spiritStone-=cost.stone;state.swordInsight-=cost.insight;state.swordNurtureLevel++;toast(`養劍完成・本命劍提升至${state.swordNurtureLevel}階`);renderSwordDestination('sword');render();save();
}
...
async function finishBattle(won,reason){
  ...
  else if(battle.mode==='swordTrial'){if(won){const stage=state.swordTrialWins,intent=swordTrialIntentReward(stage);state.swordInsight++;state.swordIntent+=intent;reward=` 戰鬥感悟+1${intent?`、劍意+${intent}`:''}。`;}else reward=' 本關沒有消耗挑戰次數，可調整劍招後再戰。'}
