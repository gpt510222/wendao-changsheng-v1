...
function applyServerIdentity(identity={}){if(identity.origin&&originProfiles[identity.origin])state.origin=identity.origin;state.serverOriginCore=identity.baseCore&&typeof identity.baseCore==='object'?identity.baseCore:{};return identity}
async function playerIdentityBootstrap(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test';return applyServerIdentity(await playerStateRpc('player_identity_bootstrap',{p_channel:channel,p_legacy_origin:state.origin||'家族子弟'}))}
