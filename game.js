...
function renderHelp(tab='cultivation'){
  ...
  if(tab==='cave'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('洞府',['食物、木材與隕鐵產地可持續升級；每升1級增加1個道童位置與儲存容量。','道童可無限制招募；木材與隕鐵生產會持續消耗食物。','食物可補充煉體精氣；木材、隕鐵則用於鍛體、養劍、擴建與製作。'])+helpCard('修行設施',['聚靈室提高修為，洗劍池提高劍元，鍛體室降低鍛體消耗與受傷率。','丹房與器室可製作的階級，依三條修行道路中的最高境界開放。']);return}
  ...
  if(tab==='cave'){$$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));$('#helpContent').innerHTML=helpCard('無限產地與容量',['食物、木材、隕鐵產地取消30級上限，可持續升級。','每升1級增加1個道童位置；每名道童每5秒固定產出1份，避免等級與人數相乘造成後期爆量。','容量採平滑二次曲線；舊資源超出新容量時不會被刪除，但消耗至容量以下前不再生產。'])+helpCard('道童與食物收支',['道童取消總數上限，招募所需食物會隨已招募人數平滑增加。','食物線不消耗食物；木材線每名道童每5秒消耗2食物，隕鐵線消耗4食物，此比例維持不變。','食物不足時只會停止無法負擔的木材或隕鐵生產，不會產生負數。'])+helpCard('修行配合',['食物可補充煉體的腹中精氣，也是招募與兩條加工產線的底層燃料。','木材用於產地擴建、負重鍛骨與器室；隕鐵用於極境熬身、淬劍養成與器室。','聚靈室提高修為、洗劍池提高劍元；鍛體室降低持續鍛體消耗與受傷率，並提高精氣容量。','丹房與器室依三路最高境界逐階開放；儲物袋與產地倉儲是兩套不同容量。']);return}
