-- An arena reward is only final after its attachment is claimed in the server
-- mail ledger. Until then, return it again so a lost local mailbox can recover.
  ...
  where r.user_id = uid
    ...
    and not exists (
      ...
      where c.user_id = uid
        ...
        and c.mail_id = 'arena-week-' || p_channel || '-' || r.week_start::text
    )
