...
declare uid uuid:=(select auth.uid());m text[];tier integer;star integer;required_floor integer:=1;o record;quantity integer;prior private.player_state_events;w private.player_resource_wallets;s private.player_sect_states;balance bigint:=0;v_period_key text;period_count integer:=0;result jsonb;
begin
 ...
 perform pg_advisory_xact_lock(hashtext(uid::text||'-secure-market-'||p_channel||'-'||p_offer_id));select * into prior from private.player_state_events where user_id=uid and channel=p_channel and request_id=p_request_id;
 if prior.id is not null then
  ...
  v_period_key:=case when o.daily_limit is not null then'day:'||((now()at time zone'Asia/Taipei')::date)::text else'all'end;
  select * into w from private.player_resource_wallets where user_id=uid and channel=p_channel;select * into s from private.player_sect_states where user_id=uid and channel=p_channel;select amount into balance from private.player_mail_item_balances where user_id=uid and channel=p_channel and item_key=o.item_key;
  select pm.purchase_count into period_count from private.player_market_purchases pm where pm.user_id=uid and pm.channel=p_channel and pm.offer_id=p_offer_id and pm.period_key=v_period_key;
  return jsonb_build_object('wallet',private.wallet_snapshot(w),'sect',private.sect_progress_snapshot(s),'itemBalances',jsonb_build_object(o.item_key,coalesce(balance,0)),'offerId',p_offer_id,'quantity',quantity,'periodKey',v_period_key,'periodCount',coalesce(period_count,0),'permanent',o.permanent_limit is not null,'alreadyPurchased',true);
 end if;
 result:=public.player_secure_market_purchase_without_floor_guard(p_channel,p_offer_id,quantity,p_request_id);return result||jsonb_build_object('alreadyPurchased',false);
...
returns jsonb language plpgsql security definer set search_path='' as $$
declare uid uuid:=(select auth.uid());m text[];required_floor integer:=1;o record;quantity integer;prior private.player_state_events;w private.player_resource_wallets;balance bigint:=0;v_period_key text;period_count integer:=0;result jsonb;
begin
 if uid is null then raise exception '需要重新登入';end if;if p_channel not in('formal','test')or p_request_id is null then raise exception '版本不正確';end if;
 ...
 perform pg_advisory_xact_lock(hashtext(uid::text||'-market-'||p_channel||'-'||p_offer_id));select * into prior from private.player_state_events where user_id=uid and channel=p_channel and request_id=p_request_id;
 if prior.id is not null then
  ...
  v_period_key:=case when o.daily_limit is not null then'day:'||((now()at time zone'Asia/Taipei')::date)::text when o.weekly_limit is not null then'week:'||(date_trunc('week',now()at time zone'Asia/Taipei')::date)::text else'all'end;
  ...
  select pm.purchase_count into period_count from private.player_market_purchases pm where pm.user_id=uid and pm.channel=p_channel and pm.offer_id=p_offer_id and pm.period_key=v_period_key;
  return jsonb_build_object('wallet',private.wallet_snapshot(w),'itemBalances',jsonb_build_object(o.item_key,coalesce(balance,0)),'offerId',p_offer_id,'quantity',quantity,'periodKey',v_period_key,'periodCount',coalesce(period_count,0),'alreadyPurchased',true);
 end if;
 result:=public.player_spirit_stone_market_purchase_without_floor_guard(p_channel,p_offer_id,quantity,p_request_id);return result||jsonb_build_object('alreadyPurchased',false);
exception when invalid_text_representation or numeric_value_out_of_range then raise exception '購買請求資料不正確';end$$;
