...
declare uid uuid:=(select auth.uid());account public.player_accounts;clean_name text:=left(coalesce(nullif(trim(p_player_name),''),'無名修士'),20);was_created boolean:=false;
begin
  ...
  if account.user_id is null then
    ...
    insert into public.player_accounts(user_id,public_uid,player_name,release_channel,updated_at) values(uid,'WD1-'||upper(uid::text),clean_name,'v1',now()) returning * into account;
    was_created:=true;
