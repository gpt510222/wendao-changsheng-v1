...
declare uid uuid:=(select auth.uid()); game_id uuid; new_game_no bigint;
begin
 ...
 return jsonb_build_object('id',game_id,'game_no',new_game_no,'wager',p_wager);
end $$;

drop function if exists public.cangji_open_games(text);
create or replace function public.cangji_open_games(p_channel text)
returns table(id uuid,game_no bigint,creator_name text,wager integer,created_at timestamptz)
language sql security definer set search_path='' as $$
 select g.id,g.game_no,g.creator_name,g.wager,g.created_at
...
begin
 ...
 where id=g.id;
 return jsonb_build_object('id',g.id,'game_no',g.game_no,'creator_name',g.creator_name,'creator_choice',g.creator_choice,
...
returns table(id uuid,game_no bigint,status text,opponent_name text,creator_choice text,responder_choice text,wager integer,outcome text,payout integer,fee integer,claimed boolean,created_at timestamptz,resolved_at timestamptz)
language sql security definer set search_path='' as $$
 select g.id,g.game_no,g.status,g.responder_name,g.creator_choice,
